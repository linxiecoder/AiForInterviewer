...
| DD-004 | 目标产品代码结构默认采用 monorepo | proposed | 当前仓库继续以“根目录全局文档 + `docs/` + `tools/doc_governor/` + `tests/doc_governor/`”作为治理与工具布局；`W10-A / W10-B / W10-C` 已完成首切片边界与关系层冻结，但这不构成创建 `apps/web`、`apps/api`、`infra` 的依据；是否进入最小业务骨架继续后置到总控对 `W10-D` 的二次判定 | M01-M10 | 待 OQ-001 确认后更新为 confirmed |
