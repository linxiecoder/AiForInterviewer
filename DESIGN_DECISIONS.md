...
| DD-025 | 一期至少包含打磨模式和模拟模式 | confirmed | 用户已确认一期至少包含打磨模式和模拟模式：打磨模式支持基于薄弱项、自定义主题、岗位 / 简历自动推荐开启，且薄弱项不是必填；模拟模式更贴近真实面试节奏，过程中弱化中途打断并在结束后输出整场判断。两种模式的一期深度和反馈保存粒度仍未确认 | M06、M07、M08、M09、W13-C、W13-D | `W13-C` 补 `PolishModeSession`、`SimulationModeSession`、题级反馈和结束报告对象；`W13-D` 补评分 / 复盘 / 导出 DoD |
