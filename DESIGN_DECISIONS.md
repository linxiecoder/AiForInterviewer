...
| DD-025 | 一期至少包含打磨模式和压力面模式 | confirmed | 打磨模式由 `ProgressTree` 驱动，用户决定继续或结束；压力面由 `InterviewQuestionSet` 驱动，题目完成后结束。 | M06、M07、M08、M09 | 评分 / 复盘 / DoD 按两种模式分别承接。 |
| DD-026 | 一期复盘来源包括真实面试材料和模拟面试结果 | confirmed | 真实面试支持逐字稿原文上传、系统自动切分并逐题拆解；模拟面试复盘展示整场判断、多维评分、岗位匹配度、通过概率、逐题点评和改进建议。 | M08、M09、M10 | 唯一事实源见评分 / 复盘 / 导出 / DoD 文档。 |
