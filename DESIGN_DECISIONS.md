...
| DD-020 | W13-B 一期工作台 IA 草案采用“工作台总览 + 岗位 / 简历 / 面试 / 评分 / 复盘 / 导出”主闭环 | proposed | `W13-B` 已新增 `docs/superpowers/plans/2026-04-25-workbench-mvp-ia-user-journey.md`，将一期必须页面收敛为登录页、工作台首页、岗位列表与编辑、简历列表与编辑、发起模拟面试、面试进行、反馈 / 评分详情、历史记录 / 复盘列表、复盘详情、复制 / Markdown 下载入口和用户账号入口；资产库、RAG / 知识库、管理台、训练中心、高级多轮、更完整导出、运维 / 配置仅作为后续占位 | W13-B、W13-C、W13-D、全局产品 IA | 用户确认 `OQ-035` 至 `OQ-042` 后，再由 `W13-C / W13-D` 吸收为对象模型、评分复盘导出和 MVP DoD 输入 |
