...
| 4. 通过反馈意图创建打磨题目 | `POST /api/v1/polish-sessions/{session_id}/feedback/{feedback_id}/next-question` | async | `InterviewSession`、`PolishSessionDetail`、latest `Feedback`、对应 `Answer` / `Question`、`ResumeVersion`、`JobVersion`、`SessionSummary`、recent turns、可选 RAG evidence | `AiTask`、`Question`、`TraceRef`、`LowConfidenceFlag` | 是，`task_type=question_generation` | 单一路径：`feedback_next_question_intent` 调用 `create_question_task`，再进入 `question_generation_service` | 使用反馈授权上下文、selected_progress_node_ref、exclude_question_refs、completed_focus_refs 和 output_schema | feedback 不是最新、answer 未满足执行条件、source unavailable、上下文裁剪高风险 -> validation failed / low confidence | question result + trace | 202 accepted；result 后产生 `Question` | `flow.polish_feedback_next_question.intent_path` |
