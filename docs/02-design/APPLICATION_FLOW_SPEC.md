...
### 2.3 Polish Execution Minimal Model

本节承接 `ADR-0005` 的 2026-06-19 addendum。Polish question / feedback formal execution 采用 Execution Minimal Model：
...
| 层 | 职责 | 输出 | 禁止事项 |
|---|---|---|---|
| Execution Authority | 按 domain 校验 intent、session canonical state、progress canonical state 和输入 payload 是否允许执行 | `allowed` / `rejected`、`execution_target`、`reason_codes`、`decision_ref` | 执行业务动作、写入状态、调用 provider 生成业务结果 |
| Execution Snapshot | 在 authority allowed 后冻结决策、session / progress state、输入 payload、runtime flags 和 asset refs | immutable `ExecutionSnapshot` | 重新决策、重建 context、读取 runtime mutable state 派生新目标 |
| Execution Executor | 通过 domain handler 消费 snapshot，执行 question / feedback / progress canonical write 并持久化结果 | persisted result、safe response、trace metadata | 决策、改写 `execution_target`、选择另一条执行路径、承载所有 domain 细节 |

Polish domain authority 拆分如下：

| Authority / Policy | 允许目标 | 说明 |
|---|---|---|
| `QuestionAuthority` | `generate_question`、`retry_question_generation` | 只能由后端根据 feedback intent、session state、progress state 和 backend-owned refs 决定下一题执行目标 |
| `FeedbackAuthority` | `evaluate_feedback`、`retry_feedback_evaluation` | 只能由后端根据 answer / question / session canonical state 决定反馈执行目标 |
| `ProgressCanonicalAuthority` | `update_progress_canonical` | 只处理真实改变进展树或会话进度事实的写入 |
| `ProgressProjectionPolicy` | `refresh_progress_projection` | 只刷新派生读模型 / 展示状态，不产生 question / feedback 业务授权 |

frontend 只提交用户意图，不提交或覆盖 `execution_target`、`selected_progress_node_ref`、`completed_focus_refs` 等执行目标字段。LLM、graph、fallback、provider 和 legacy mapper 只能提供 recommendation、candidate、adapter status、availability 或 transport result；它们不能成为 formal write 的 authority。`decision_ref` 只用于追踪 authority decision、snapshot、日志、结果关联和调试排查，不是 durable idempotency、running task lifecycle、resume / cancel / deadline recovery 的长期契约。
...
| 4. 通过反馈意图创建打磨题目 | `POST /api/v1/polish-sessions/{session_id}/feedback/{feedback_id}/next-question` | async | `InterviewSession`、`PolishSessionDetail`、latest `Feedback`、对应 `Answer` / `Question`、`ResumeVersion`、`JobVersion`、`SessionSummary`、recent turns、可选 RAG evidence | `AiTask`、`Question`、`TraceRef`、`LowConfidenceFlag` | 是，`task_type=question_generation` | 单一路径：feedback intent -> `QuestionAuthority` -> `ExecutionSnapshot` -> `QuestionExecutionHandler` -> question generation / persistence | frontend request 只提交 intent 和可选 exclude refs；后端 authority 从 feedback / progress canonical state 决定 execution target；Prompt 输入只消费 snapshot refs 和 output_schema | feedback 不是最新、answer 未满足执行条件、source unavailable、上下文裁剪高风险 -> validation failed / low confidence / safe rejection | question result + trace；persisted metadata 可关联 `decision_ref` / `execution_target`，但不形成 durable idempotency contract | 202 accepted；result 后产生 `Question` | `flow.polish_feedback_next_question.intent_path`、`flow.polish_question.authority_snapshot_executor` |
...
| 6. 每次回答后的打磨反馈生成 | `POST /api/v1/polish-sessions/{session_id}/feedback` | async | `InterviewSession`、`PolishSessionDetail`、当前 `Question`、当前 `Answer`、`ResumeVersion`、`JobVersion`、`SessionSummary`、最近若干轮 turn refs、可选 `JobMatchAnalysis`、可选 `AssetVersion`、可选 `Weakness`、可选 RAG evidence | `AiTask`、`Feedback`、`ScoreResult(polish_answer)`、`LossPoint`、candidate refs、`LowConfidenceFlag`、`SessionSummary` update | 是，`task_type=feedback_generation` | feedback intent -> `FeedbackAuthority` -> `ExecutionSnapshot` -> `FeedbackExecutionHandler`；MVP 默认 1 call 合并 `P-POLISH-003`、`P-POLISH-004`、`P-POLISH-005`、`P-POLISH-009`；`P-POLISH-006/007/008` 默认按用户点击 on-demand 独立 task；`P-POLISH-010/011` 默认只生成 candidate refs 或入口建议 | 模板字段全部适用；必须包含 current_question、current_answer、recent_turns_summary、session_summary、forbidden_repeat_refs、score rule ref；LLM output 不能改写 execution target | 输出必须分别保留每个 contract_id、validation_result_ref、evidence_refs、trace_refs；answer too short、evidence missing、score mismatch -> low confidence | feedback、score、loss point、candidate refs、summary snapshot、trace；persisted metadata 可关联 `decision_ref` / `execution_target`，但不形成 durable idempotency contract | 202 accepted；result 返回 feedback / score / candidates / `feedback_payload.next_recommended_actions` | `flow.polish_feedback.merged_call_contract_ids`、`flow.polish_feedback.on_demand_reference_answer`、`flow.polish_feedback.candidate_not_formal`、`flow.polish_feedback.authority_snapshot_executor` |
...
`next_recommended_actions` 的权威来源是反馈规则 / 校验链路中的受控 action id 集合。feedback storage handoff 只允许补充 `feedback_id`、owner-scoped refs、metadata、trace / candidate refs 等持久化上下文，不得在 final validation 之后追加、改写或合成新的 action 文案；API response 只能通过 `feedback_payload.next_recommended_actions` 暴露该集合，不再保留顶层 mirror 字段，也不得把 storage-time 后处理作为推荐动作决策层。
