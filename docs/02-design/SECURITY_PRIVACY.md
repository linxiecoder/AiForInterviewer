...
- 本轮按 `AR-F4-FULL-001` 处置安全隐私侧 F4 阻断项：owner、visibility、LLM 输入最小化、provider payload、system prompt、隐藏评分规则、日志、trace、copy boundary、第三方隐私和用户确认审计边界已冻结；不标记 `AIFI-ARCH-002` 或 `AIFI-SEC-001` 完成。
...
|---|---|---|---|
| 简历 | `Resume`、`ResumeVersion`、`ResumeMarkdownOutline` | 求职者 / 面试准备用户为 owner；创建者记录到 `OwnerRef`；outline 仅为 derived / non-persistent 识别视图 | 默认仅 owner 可见；管理员 / 内容维护者不得默认查看个人简历正文 |
...
| 多维评分 | 0-100 展示值、维度、解释、规则版本、可信度、风险提示 | 否 | 是 | 否 | 是 | 是 | 是 | 条件 | 仅状态和 trace | 是 | 关联对象 owner | 必须保留 `score_version`、`rubric_version`、`ScoreRuleVersion`、证据和低置信度；不承诺精确通过概率；不暴露隐藏评分规则 |
...
- 服务端只保存用户确认后的 Markdown / text 内容、`ResumeVersion` 和系统识别出的 `ResumeMarkdownOutline` / 识别文本，不保存文件名、MIME、文件大小或原始文件二进制。
- 系统不得跟随 Markdown 中的远程链接抓取外部资源，不得调用 shell 或外部文件转换器处理简历输入。
- 模块识别失败不创建资产候选；可保留失败审计和错误分类，但不得记录简历正文。

### 8.3 原文、识别文本和删除绑定

- MVP 没有上传临时文件、对象存储对象、原始文件下载或文件解析 worker；持久化的是用户确认后的简历 Markdown / text 原文、`ResumeVersion` 和系统识别出的 `ResumeMarkdownOutline` / 识别文本。
...
- scoring candidate 只有在结构化校验、业务语义校验、evidence binding、低置信度分类和版本记录通过后，才可成为正式 `ScoreResult`；validation failed 时不得落正式报告评分。
...
6. 打磨自定义主题：`custom_topic_text` 只能作为用户输入标签进入上下文；必须做长度限制、敏感信息裁剪、prompt injection 检查和指令中和，不得作为系统指令或 developer 指令执行。
7. 删除 / 重建 / 失效：删除或禁用文档后，chunk、embedding 引用和检索索引立即标记 invalid；后续检索排除；后台重建或清理失败时保持不可检索。
...
### 11.1 AI Runtime raw-off / checkpoint / timeline 可见性

本节补齐 PR3 / PR4 Runtime Contracts 的安全隐私边界。AI Runtime rows、timeline、interrupt drawer、checkpoint refs 和 LLM call summaries 必须遵守 raw-off default。

强制规则：

- 不保存 raw prompt、raw completion、provider request / response payload、system prompt、token、cookie、secret、hidden scoring rule 或未经脱敏正文到普通 runtime row、日志、API response、timeline、copy content、checkpoint metadata 或普通 trace 可见正文。
- `agent_checkpoint_refs` 只保存 checkpoint namespace、thread_id、checkpoint_id、version、hashes、timestamps 和 sanitized metadata summary；checkpoint payload 不进入 API、timeline、normal trace 或 Core Business read model。
- Agent run timeline 只返回 sanitized summary、status、refs、hashes、validation result、low confidence flags、failure category 和 user-visible next action。
- Interrupt drawer payload 只能包含 candidate refs、allowed actions、safe prompt summary、base_version_ref、validation summary 和 owner-scoped display summary；不得包含 AgentState、checkpoint payload、Prompt、completion 或 provider payload。
- LLM debug raw capture 默认关闭。未来如需开启，必须同时满足 feature flag、encryption、TTL、owner/admin access control、access audit、redaction scan 和 release accepted risk；PR3 / PR4 不默认启用。
- PR3 / PR4 不启用 real provider default-on；real provider gate 独立于 graph enablement，默认 false。
- runtime config、graph descriptor、health summary、timeline 和 graph configuration API 不得暴露 provider secrets、model key、raw provider payload 或 LangGraph debug internals。

F7 / PR4+ 验证至少覆盖：

- timeline / interrupt detail / run status 不含 `AgentState`、checkpoint payload、raw prompt、raw completion、provider payload、raw_payload_ciphertext_ref 或 encryption_key_ref。
...
- 复盘复制内容不得包含 system prompt、provider payload、completion、隐藏评分规则、内部校准细节或未脱敏第三方 / 公司 / 面试官 / 他人隐私；真实面试复盘复制前必须完成第三方隐私和公司敏感信息脱敏。
...
- 内容沉淀目标：`target_type` / `target_ref` 必须 owner scoped；允许目标只限 `asset`、`weakness`、`training_suggestion`、`polish_input`、`pressure_input`、`next_interview_input`、`review_note`、`none` / `skip`；目标不可用、目标冲突或来源不可用时不得静默写入。
- 低置信校对保存：用户修正内容必须先通过 owner 校验、敏感信息处理和 validation；审计只记录 `CandidateCorrection` / `UserCorrectionRef`、结果和摘要，不记录过量正文，不得把校对内容直接作为 Prompt source。
...
### 17.1 评分、通过倾向、风险提示和校准隐私边界

- 评分和风险提示不得泄露隐藏评分规则、完整内部权重表、校准样例正文、系统 Prompt、provider payload、模型参数或内部校准细节。
- 用户可见分数是 0-100 产品评分刻度，不是精确通过概率、录取概率、offer 概率或通过率百分比。
- 用户可见通过倾向只允许“偏低 / 中等 / 偏高 / 需谨慎 / 证据不足，无法判断倾向”这类分档；低置信度、证据不足、source unavailable 或 validation failed 时不得给出确定性倾向。
- 风险提示必须绑定证据、`score_version`、`rubric_version`、`ScoreRuleVersion`、`confidence_level`、`validation_status` 和低置信度状态；证据不可展示时只返回可展示摘要或 ref，不返回原始敏感正文。
- 风险提示不得把第三方公司、面试官、真实反馈中的个人信息或未脱敏公司敏感材料作为可见风险依据；如需要引用，只能使用脱敏摘要和来源可信度说明。
- 风险提示不得使用恐吓式、确定性、歧视性或不可解释表达，不得把岗位匹配缺口直接包装成稳定能力缺陷。
- 用户可见评分、通过倾向和风险提示必须带可信度说明和非决策性免责声明，说明结果基于当前材料、规则版本和证据，仅用于面试准备辅助，不代表真实招聘决定。
- 日志、错误追踪、trace 和复制事件不得记录原始 provider payload、原始 Prompt、completion、隐藏评分规则、内部校准细节或复制正文。
- 报告复制 / copy content 不得包含隐藏评分规则、内部校准细节、provider payload、原始 Prompt、completion、无权限来源正文或未经脱敏的第三方 / 公司 / 个人敏感信息。
...
## 20. 验证清单（checklist）/ 完成定义（Definition of Done）
...
- 评分 / 风险提示检查：0-100 score 正常生成但不出现精确通过概率；low confidence 时不输出确定倾向；source unavailable 时报告降级；validation failed 时不落正式报告评分；`risk_level` 与 `evidence_refs` 同步存在；`score_version` / `rubric_version` 存在；copy content 不包含隐藏评分规则或内部校准细节。
...
| `PROMPT_SPEC.md` 与 `prompt-contracts/*.md` | 承接 AI Task Contract、上下文装配、检索依赖、validation、low confidence、evidence、trace、failure handling、source unavailable、scoring candidate 校验、评分 / 风险提示降级和 LLM payload 交接要求 | 不写 Prompt 模板全集，不选择 provider、模型参数、向量数据库、embedding 模型或搜索服务，不暴露隐藏评分规则 |
| F5 实现 | 交接密钥管理、日志保留、脱敏实现、审计事件落库、删除策略、trace 存储和访问控制 | 不写 ORM、DDL、migration 或平台选型 |

### 21.1 Candidate / Confirmation 安全边界

- `CandidateRef`、`SuggestionRef`、`UserConfirmationRef`、`UserCorrectionRef` 和 `DepositTarget` 不是敏感正文容器，只能保存最小必要引用、状态、摘要、目标类型、目标引用和审计关联。
...
- Report / Review / Asset / Training 的 `CopyableContent` 不得包含无权限来源正文、原始 Prompt、completion、provider payload、隐藏评分规则、内部校准细节或未脱敏第三方 / 公司 / 个人敏感信息。
...
| SP-DEF-005 | 企业级 KMS、自动轮换、泄露演练和 DPA | MVP 后端 secret、fake 示例、日志禁止、泄露后废弃 / 手动轮换 | 运维成熟度有限，供应商协议和企业级 DPA 为 deferred_non_blocking | 发布前按部署环境补 secret 平台、轮换周期和 provider 协议审查 |
...
| 2026-05-16 | 同步 AR-F4-FULL-002 后 API handoff 当前性 | 明确 endpoint、API schema、async / retry / idempotency、rate limit 由 `API_SPEC.md` 承接；本文只同步安全隐私 enforcement 交接口径，不定义 Prompt 模板、物理 schema 或实现方案 |
| 2026-05-16 | 同步 API / DATA / Prompt 交接边界当前性 | 同步 `API_SPEC.md` / `DATA_MODEL.md` / `PROMPT_SPEC.md` 已创建后的安全交接口径；补 candidate / confirmation、source unavailable、trace / evidence / audit、LLM payload 边界；不定义 endpoint、schema、Prompt 模板或实现方案 |
...
| 2026-05-15 | 初始化 F4 安全隐私规范草案 | 建立 `AIFI-SEC-001` 的 ownership、角色权限、LLM 隔离、RAG 来源、trace、审计、脱敏、保留、复制 non-goal、真实面试敏感信息、用户确认和密钥边界 |
