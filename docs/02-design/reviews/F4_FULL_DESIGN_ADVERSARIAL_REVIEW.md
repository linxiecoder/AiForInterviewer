...
Source Documents:
- `docs/03-implementation/DELIVERY_PLAN.md` §1 F4 / M4 退出标准
- `docs/03-implementation/BACKLOG.md` §1 `AIFI-ARCH-002`
...
- `docs/03-implementation/DELIVERY_PLAN.md` §1 F4 / M4 退出标准要求不得遗留 `F4_TECH_DESIGN` UNKNOWN。
- `docs/03-implementation/BACKLOG.md` §1 `AIFI-ARCH-002` 仍为 `NOT_STARTED`。
...
- `docs/03-implementation/DELIVERY_PLAN.md` §1 F4 / F5 / F7
Affected Downstream:
...
`rg -n "download|export|导出|下载|filename|MarkdownExport" docs/02-design docs/03-implementation` 的 active 命中只能出现在 non-goal、deferred、禁止说明或本审计 finding 中；F7 能验证无文件生成、无文件下载、无批量导出入口。
...
Source Documents:
- `docs/03-implementation/DELIVERY_PLAN.md` §3 当前完成状态
...
- `docs/03-implementation/DELIVERY_PLAN.md` §3 将 F3 标为 `DONE`。
