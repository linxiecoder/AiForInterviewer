...
| AR-F4-FULL-002 | High | `API_SPEC.md` 仍为骨架，不能支撑 F5/F6/F7 接口交接 |
| AR-F4-FULL-003 | High | 评分、通过倾向、风险提示和校准口径仍未冻结 |
| AR-F4-FULL-004 | High | `REPORT_CONTRACTS.md` 重新引入 Markdown 下载 / 导出语义，违背 MVP non-goal |
...
- Critical = 0。
- High = 4。
- 根据 F4 退出判定规则，只要 High > 0，F4 不允许退出，F5 不允许启动；可以进入 remediation，但不能进入 implementation。

## 7. 人工决策项

1. 是否允许任何 Markdown 下载 / export / filename / export snapshot 语义进入 MVP；当前审计建议不允许。
2. API 任务同步 / 异步边界如何划分，尤其报告、复盘、训练建议是否必须异步。
...
- `REPORT_CONTRACTS.md` 不再出现下载 / 导出实现语义。
- `API_SPEC.md` 从语义骨架补齐到可实现、可测试 contract。
...
不为本验收记录创建最终 acceptance approval。原因：当前 `Status: Pending`，且 High findings 尚未 remediation / verification。

## 10. 当前结论

本验收记录不是 Accepted。High findings 关闭前，F4 全量设计只能进入 remediation，不能进入 F5 implementation。
