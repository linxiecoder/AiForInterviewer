...
- 6B 完成后，Pressure / Report / Review / Weakness / Asset / Training contracts 仍应保持 Stub，等待后续阶段授权填充。
...
当前已将 Shared contracts、Job match contracts 和 Polish `P-POLISH-001` 至 `P-POLISH-011` 填充为 Draft。Polish 001-011 完成 Draft 后，下一批不应继续重复填充 Polish；应先进入 Polish 只读验收，或在另行授权后进入 Pressure / Report / Review 等后续阶段。

1. Polish mode `P-POLISH-001` 至 `P-POLISH-011` 只读验收。
