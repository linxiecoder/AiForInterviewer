...
| `P-POLISH-009` | Next Round Suggestion | 生成下一轮改进建议 | Draft |
| `P-POLISH-010` | Asset Candidate | 生成资产候选 | Draft |
| `P-POLISH-011` | Weakness Candidate | 生成薄弱项候选 | Draft |

### 9.4 Pressure Mode Contracts

| Contract ID | 名称 | 目标 | 状态 |
|---|---|---|---|
| `P-PRESSURE-001` | Opening Strategy | 生成压力面开场策略 | Draft |
| `P-PRESSURE-002` | First Question Generation | 生成首题 | Draft |
| `P-PRESSURE-003` | Answer Quality Assessment | 判断回答质量 | Draft |
| `P-PRESSURE-004` | Follow-up Strategy | 选择追问策略 | Draft |
...
本节填充打磨模式主链路的 AI 子任务 contract。Polish contracts 只定义主题规划、题目生成或选择、回答诊断、每轮 0-100 得分、反馈解释链路和回流候选链路的输入输出、检索依赖、上下文装配、校验、低置信度、证据、trace、持久化交接和安全边界；不写完整生产 Prompt 文案，不冻结题目推荐算法、评分公式、权重、阈值、校准方法、模型供应商、模型参数、RAG 实现、API endpoint 或物理数据库 schema。
...
Polish 6B 只负责回流候选链路：下一轮建议、资产候选和薄弱项候选。6B 不负责正式创建 `Asset`、正式创建 `Weakness`、正式创建 `TrainingRecommendation`、压力面追问、最终面试报告、真实面试复盘或训练计划生成，也不关闭同题结束阈值、资产质量算法、薄弱项合并算法、训练推荐算法、评分公式或 RAG 实现 UNKNOWN。
...
- Goal: 基于当前题目、用户回答、诊断、得分、失分点、参考回答、考点解析、技术原理扩展和 session summary 生成下一轮建议；只生成建议，不关闭同题结束阈值 UNKNOWN，不生成正式训练计划。
- Required Inputs:
  - `OwnerRef`
  - `polish_session_ref`
  - `polish_turn_ref`
...
- Validation Rules:
  ...
  - 不得关闭同题打磨结束阈值 UNKNOWN；`same_question_end_threshold_status` 只能表达 `unknown`、`not_evaluated`、`suggestion_only` 或等价非关闭状态。
  - 不得把建议动作写成正式训练计划。
  - 不得把低置信度结果包装为确定建议。
...
- Test Strategy: 使用 fixture 覆盖评分后建议、失分后建议、参考回答后建议、考点 / 技术原理后建议、用户请求下一步、继续同题、换题、进入压力面仅作为建议、session summary 缺失、低置信度不包装为确定建议和不关闭同题结束阈值 UNKNOWN。
- Open Questions: 同题结束建议阈值、下一轮推荐算法、建议排序算法、进展树推荐算法和训练计划映射仍待后续 contract / API / UX 收敛，不在本 contract 关闭。
...
- Open Questions: 资产质量算法、资产分类算法、资产合并算法、版本替代规则和正式资产入库 API 字段仍待后续 Asset / API / UX 收敛，不在本 contract 关闭。
...
- Open Questions: 薄弱项合并算法、严重度算法、状态生命周期、自动消减规则、训练推荐算法和正式 Weakness API 字段仍待后续 Weakness / Training / API / UX 收敛，不在本 contract 关闭。
...
- `P-POLISH-009` 不得关闭同题结束阈值 UNKNOWN，不得生成正式训练计划。
...
当前已将 Shared contracts、Job match contracts、Polish `P-POLISH-001` 至 `P-POLISH-011` 和 Pressure `P-PRESSURE-001` 至 `P-PRESSURE-004` 填充为 Draft。Pressure 第一组完成后，下一批不应继续重复填充 Opening / First Question / Answer Quality / Follow-up Strategy；应先进入 Pressure 第一组只读验收，或在另行授权后进入 Pressure 后续 contracts / Report / Review 等后续阶段。

1. Pressure 第一组 `P-PRESSURE-001` 至 `P-PRESSURE-004` 只读验收。
2. Pressure mode `P-PRESSURE-005` 至 `P-PRESSURE-009`。
3. Report contracts。
4. Review contracts。
