...
- API path、request / response schema、错误码、鉴权语义、异步任务语义、幂等、rate limit 和重试语义已由 `API_SPEC.md` 作为 F5/F6/F7 handoff 承接；后续 F5 实现不得绕过该 API contract。
...
1. 继续迭代 `DATA_MODEL.md`，同步 Asset / Training / Candidate / Confirmation / AI output handoff 的逻辑对象与状态边界。
2. 基于 `API_SPEC.md` 当前 API handoff，后续只在授权 remediation 或 F5 实现准备中做 targeted 复核，不得重新回退为未冻结 endpoint 占位。
3. 继续迭代 `PROMPT_SPEC.md` 与 `prompt-contracts/*.md`，维护 AI Task Contract registry、子文档和跨 contract 一致性。
4. 继续迭代 `SECURITY_PRIVACY.md`，同步 API / Prompt / Data 新增边界下的 owner / scope、trace、evidence、audit、source unavailable、复制和 LLM payload 风险。
5. 回到 `TECH_DESIGN.md` 汇总跨文档决策，并在证据充分时评估 `F4_TECH_DESIGN` UNKNOWN 关闭状态。
...
- 本轮同步只修正 TECH / SECURITY 当前性，不表示实现完成。
