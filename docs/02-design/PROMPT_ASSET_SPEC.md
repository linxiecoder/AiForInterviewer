...
|---|---|---|---|---|---|---|---|
| `prompt_asset.polish.question.001` | `P-POLISH-002` + shared validation | `polish_question_generation` | `polish_question_surface.phase1` | `QuestionGenerationService.generate`; `build_question_blueprint`; `render_blueprint_question` | `validate_question_grounding` | `eval.polish.question.v1` | Phase 2A runtime fact: question generation is blueprint + grounding based; old question LLM / pattern / repair chain removed |
| `prompt_asset.polish.feedback.001` | `P-POLISH-003` reserved placeholder | `polish_feedback_generation` | `polish_feedback_reserved.phase2b` | `build_reserved_feedback_artifacts`; `build_reserved_feedback_payload` | reserved payload shape；raw-off / no-candidate / no-score assertions | `eval.polish.feedback.v1` deferred | Phase 2B runtime fact: feedback currently returns reserved placeholder only; no LLM feedback, candidate generation, reference answer or score result |
...
|---|---|---|---|---|---|---|
| `P-POLISH-002` | `polish_question_generation` | `polish_question_surface.phase1` | `QuestionGenerationService.generate`; `build_question_blueprint`; `render_blueprint_question` | `validate_question_grounding` | `tests/api/test_polish_question_refactor_phase1.py` covers progress-node resolution, grounding pass/fail and no question persistence on failure | closed for Phase 2A docs; implementation source of truth is `question_generation_service.py` |
| `P-POLISH-003` | `polish_feedback_generation` | `polish_feedback_reserved.phase2b` | `build_reserved_feedback_artifacts`; `build_reserved_feedback_payload` | reserved payload shape；candidate refs empty；score / reference answer absent；raw-off metadata | `tests/api/test_polish_api.py` covers reserved feedback placeholder and no candidate extraction | closed for Phase 2B placeholder; full feedback generation is deferred |
| `P-POLISH-004` | deferred | N/A in Phase 2B reserved flow | N/A | N/A | covered indirectly by reserved feedback no-score assertions | deferred until feedback generation is re-authorized |
| `P-POLISH-005` | deferred | N/A in Phase 2B reserved flow | N/A | N/A | covered indirectly by reserved feedback no-loss-point assertions | deferred until feedback generation is re-authorized |
| `P-POLISH-009` | deferred | N/A in Phase 2B reserved flow | N/A | N/A | covered indirectly by reserved feedback no-retry assertions | deferred until feedback generation is re-authorized |
...
Polish progress / feedback runtime builder 仍具备 compact prompt bundle 和校验闭环；Phase 2A 后 Polish question generation 由 blueprint / grounding path 接管。AIFI-PROMPT-002 的设计结论是：Polish prompt migration 不再只依赖 builder，而是按本文 asset registry 和 `PROMPT_EVALUATION_SPEC.md` fixture set 管理。

| Asset | 输入块 | Evidence 格式 | 输出 / 禁止输出 | Release Gate |
|---|---|---|---|---|
| `prompt_asset.polish.question.001` | resolved progress node、`EvidenceScope`、`QuestionBlueprint`、compact evidence、recent questions、schema | `evidence_refs` 必须来自 selected progress evidence chunk id；不得用 source ref / object ref 替代 | 只输出 grounding passed 后可持久化的单题 draft；grounding failed 不生成、不持久化 question；不得输出参考答案、raw prompt、provider payload、编造组件 | `eval.polish.question.v1` 全部通过 |
| `prompt_asset.polish.feedback.001` | current question、answer、question metadata | answer / question refs 只进入 reserved payload；不装配评分、候选或参考答案 evidence | 当前只输出 reserved feedback payload；不得写 ScoreResult / Weakness / Asset；不得调用 LLM | `eval.polish.feedback.v1` deferred until feedback generation is re-authorized |
