...
|---|---|---|---|---|---|---|---|
| `prompt_asset.polish.question.001` | `P-POLISH-002` + shared validation | `polish_question_generation` | `polish_question_surface.phase1` | `QuestionGenerationService.generate`; `build_question_blueprint`; `render_blueprint_question` | `validate_question_grounding` | `eval.polish.question.v1` | Current runtime fact: question generation uses blueprint + compact provider request, `source_support_level` four-state path (`direct_project_evidence` / `adjacent_project_evidence` / `job_gap_only` / `insufficient_context`) and blocking/warning grounding gate; old question LLM / pattern / repair chain removed; Phase 6B follow-up coverage matrix not executed |
| `prompt_asset.polish.feedback.001` | `P-POLISH-003` + `P-POLISH-004` + `P-POLISH-005` 初始 generated payload | `polish_feedback_generation` | `polish_feedback_agent_prompt.v1` | `FeedbackGenerationService.generate`; `build_feedback_prompt_asset`; `FeedbackGenerationAgent.generate` | `validate_generated_feedback_payload`；raw-off / legacy-field compatibility / failure-to-`generation_failed` assertions | `eval.polish.feedback.v1` pending | Current runtime fact: `create_feedback_task` calls `FeedbackGenerationService.generate()` and persists readable generated feedback; provider / validation failures do not return reserved success; `CanonicalEvidencePack.canonical_project_assets` feeds feedback context; only `asset_confirmed` is active canonical evidence and `asset_archived` is historical/reference-only unless restored; Phase 4 backend policy / validation enforces `asset_consistency_check`, `answer_coverage`, `answer_change_analysis`, `feedback_cards`; Phase 5 Fake cleanup and Phase 6B follow-up coverage matrix not executed; real provider E2E not verified |
| `prompt_asset.polish.progress.001` | `P-POLISH-001` + shared validation | active initial：`polish_progress_quality_first_menu`；状态刷新：`polish_progress_tree_state` | `polish_progress_quality_first_menu_prompt_v1`; `polish_progress_tree_state_prompt_v1` | `build_progress_quality_first_menu_prompt`; `build_progress_tree_state_refresh_prompt` | canonical Progress Tree generator normalization / structured status parser / quality gates / metadata trust boundary | `eval.polish.progress.v1` | `polish_progress_quality_first_menu` 是唯一初始生成链路；`deferred_candidates` 仅作为质量策略 metadata |
...
| `prompt_asset.jobmatch.analysis.001` | `P-JOBMATCH-001` | `job_match_analysis` | `JOB_MATCH_PROMPT_VERSION` | `LlmJobMatchAnalyzer.analyze` | `_normalize_job_match_payload`; scoring / evidence normalization | `eval.jobmatch.analysis.v1` | PR8 deferred / conditional: direct path retention / descriptor first; graph only if still needed after PR7 configuration console |
| `prompt_asset.jobmatch.score.001` | `P-JOBMATCH-002` | `job_match_analysis` | `JOB_MATCH_PROMPT_VERSION` | `LlmJobMatchAnalyzer.analyze` | scoring normalization | `eval.jobmatch.score.v1` | PR8 deferred / conditional: direct path retention / descriptor first; graph only if still needed after PR7 configuration console |
| `prompt_asset.jobmatch.points.001` | `P-JOBMATCH-003` | `job_match_analysis` | `JOB_MATCH_PROMPT_VERSION` | `LlmJobMatchAnalyzer.analyze` | points / evidence normalization | `eval.jobmatch.points.v1` | PR8 deferred / conditional: direct path retention / descriptor first; graph only if still needed after PR7 configuration console |
| `prompt_asset.jobmatch.weakness.001` | `P-JOBMATCH-004` | `job_match_analysis` | `JOB_MATCH_PROMPT_VERSION` | `LlmJobMatchAnalyzer.analyze` | weakness candidate boundary checks | `eval.jobmatch.weakness.v1` | PR8 deferred / conditional: direct path retention / descriptor first; graph only if still needed after PR7 configuration console |

## 6. Required Mapping Table

| Contract | Runtime task_type | Prompt Version | Code Builder | Validator | Tests | Gap |
|---|---|---|---|---|---|---|
| `P-POLISH-002` | `polish_question_generation` | `polish_question_surface.phase1` | `QuestionGenerationService.generate`; `build_question_blueprint`; `render_blueprint_question` | `validate_question_grounding` | `tests/api/test_polish_question_refactor_phase1.py` covers progress-node resolution, compact provider request, `source_support_level`, grounding pass/fail and no normal question persistence on blocking failure | closed for current question hardening; implementation source of truth is `question_generation_service.py` |
| `P-POLISH-003` | `polish_feedback_generation` | `polish_feedback_agent_prompt.v1` | `FeedbackGenerationService.generate`; `build_feedback_prompt_asset`; `FeedbackGenerationAgent.generate` | `validate_generated_feedback_payload`；generated status / schema / legacy fields / raw-off metadata / Phase 4 feedback fields | local Phase 2 and Phase 4 tests cover service call, generated payload persistence/readback, failure semantics, canonical assets context, forbidden-key redaction and feedback rule validation | closed for generated-feedback real path and Phase 4 feedback rules; legacy reserved compatibility must not be returned as success |
| `P-POLISH-004` | `polish_feedback_generation` | `polish_feedback_agent_prompt.v1` | `FeedbackGenerationService.generate` | generated payload score fields are accepted only through validator; complete scoring calibration remains F4/F7 governed | local Phase 2 audit covers compatibility fields and failure handling | partially represented in generated payload; complete scoring policy is not closed by Phase 2 |
| `P-POLISH-005` | `polish_feedback_generation` | `polish_feedback_agent_prompt.v1` | `FeedbackGenerationService.generate` | generated payload loss point / reference answer fields are accepted only through validator; advanced explanation rules remain future | local Phase 2 audit covers compatibility fields and failure handling | partially represented in generated payload; full explanation policy is not closed by Phase 2 |
| `P-POLISH-009` | partially covered by `polish_feedback_generation` guardrails | `polish_feedback_agent_prompt.v1` for generated feedback guardrails | `FeedbackGenerationService.generate` | `validate_generated_feedback_payload` + Phase 4 feedback rules | current generated payload carries compatible `next_recommended_actions`; Phase 4 policy blocks unsafe next-question action on asset conflict | standalone candidate confirmation and Phase 6B follow-up coverage matrix remain future |
...
| `P-JOBMATCH-001` | `job_match_analysis` | `JOB_MATCH_PROMPT_VERSION` | `LlmJobMatchAnalyzer.analyze` | `_normalize_job_match_payload` | target parity fixture: analysis summary and evidence refs | PR8 deferred / conditional descriptor parity |
| `P-JOBMATCH-002` | `job_match_analysis` | `JOB_MATCH_PROMPT_VERSION` | `LlmJobMatchAnalyzer.analyze` | scoring normalization | target parity fixture: score range, confidence, no exact probability | PR8 deferred / conditional descriptor parity |
| `P-JOBMATCH-003` | `job_match_analysis` | `JOB_MATCH_PROMPT_VERSION` | `LlmJobMatchAnalyzer.analyze` | points / evidence normalization | target parity fixture: match / mismatch / improvement points | PR8 deferred / conditional descriptor parity |
| `P-JOBMATCH-004` | `job_match_analysis` | `JOB_MATCH_PROMPT_VERSION` | `LlmJobMatchAnalyzer.analyze` | weakness candidate boundary checks | target parity fixture: candidate, confirmation required, no formal write | PR8 deferred / conditional descriptor parity |
...
Polish progress / question / feedback runtime builder 仍具备 compact prompt bundle 和校验闭环；Polish question generation 已具备 compact provider request、`source_support_level` 四态路径和 blocking / warning grounding gate；feedback real path 已接通 `FeedbackGenerationService.generate()`，并在 question / feedback context 中集成 `CanonicalEvidencePack.canonical_project_assets`。AIFI-PROMPT-002 的设计结论是：Polish prompt migration 不再只依赖 builder，而是按本文 asset registry 和 `PROMPT_EVALUATION_SPEC.md` fixture set 管理。

| Asset | 输入块 | Evidence 格式 | 输出 / 禁止输出 | Release Gate |
|---|---|---|---|---|
| `prompt_asset.polish.question.001` | resolved progress node、`EvidenceScope` / `CanonicalEvidencePack`、`QuestionBlueprint`、compact evidence、canonical project assets、recent questions、schema | `evidence_refs` 必须来自 selected progress evidence chunk id；`canonical_project_assets` 只使用 `asset_confirmed`；`asset_archived` 只作历史 / reference-only，除非显式恢复；不得用 source ref / object ref 替代 | 只输出 blocking grounding passed 后可持久化的单题 draft；warning grounding 写入 metadata；blocking failed 不生成、不持久化正常 question；不得输出参考答案、raw prompt、provider payload、编造组件 | `eval.polish.question.v1` 全部通过；Phase 6A question hardening 已实现；Phase 6B follow-up coverage matrix 未执行 |
| `prompt_asset.polish.feedback.001` | current question、answer、question metadata、`CanonicalEvidencePack.canonical_project_assets`、最小 session / progress / evidence 摘要 | answer / question refs 进入 generated payload 与 trace refs；`asset_confirmed` 是 active canonical evidence；`asset_archived` 是 historical/reference-only unless explicitly restored；不得传 raw prompt、provider payload、完整简历或完整 JD | 输出 `polish_feedback_generated_v1` generated/partial/low_confidence payload，以及 `asset_consistency_check`、`answer_coverage`、`answer_change_analysis`、`feedback_cards`；provider unavailable、exception 或 validation failed 落为 `generation_failed` / `validation_failed`，不得返回 legacy reserved success；不得写正式 Weakness / Asset / Training object | `eval.polish.feedback.v1` 待补；Phase 4 feedback rules 已由 backend policy / validation 实现；Phase 5 Fake cleanup 未执行；real provider E2E 未验证 |
| `prompt_asset.polish.progress.001` | resume / JD / match summary、selected evidence chunks、turn summaries、schema | evidence chunk ids 稳定且可追踪 | 输出 progress tree plan / grounded plan；不得把 evidence 原句直接当 display title | `eval.polish.progress.v1` 通过后进入 PR5 Polish migration |
