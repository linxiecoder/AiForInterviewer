...
| `prompt_asset.polish.progress.001` | `P-POLISH-001` + shared validation | active initial：`polish_progress_quality_first_menu`；历史刷新 / 读取兼容：`polish_progress_tree_state` 和 historical V1 plan payloads | `polish_progress_quality_first_menu_prompt_v1`; `polish_progress_tree_state_prompt_v1` | `build_progress_quality_first_menu_prompt`; `build_progress_tree_state_refresh_prompt` | quality-first normalization / quality gates / legacy read-only compatibility | `eval.polish.progress.v1` | active initial generator 仅保留 quality-first；四阶段 `v2_pipeline` 已删除 |
