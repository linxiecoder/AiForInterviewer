...
|---|---|---|---|---|---|---|
| `P-POLISH-002` | `polish_question_generation` | `polish_question_generation_prompt.v1` | `QuestionGenerationService.generate`; `build_question_blueprint`; `render_blueprint_question` | `validate_question_grounding` | `tests/api/test_question_semantic_collapse.py` locks removed legacy symbols; focused execution coverage is the feedback intent API path | closed for current question hardening; implementation source of truth is `question_generation_service.py` |
