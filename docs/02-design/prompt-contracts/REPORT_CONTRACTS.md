...
- Open Questions: 隐藏评分公式实现细节、复杂分项权重调参、RAG 实现、报告生成 readiness 阈值和最终 UX 文案润色仍待后续 contract / API / UX 收敛，为 deferred_non_blocking；通过倾向展示已收敛为分档表达，风险提示已收敛为 `risk_level` / `risk_reason` / evidence / confidence / validation 绑定，不再作为本 contract 的开放问题。
...
- Trigger:
  ...
  - 用户请求解释某个分项得分。
  - 用户重新生成报告或评分解释。
- Goal: 解释报告中的分项评分；本 contract 冻结正式报告评分解释必须具备的 0-100 产品刻度、rubric / rule version、证据、低置信度、trace、免责声明和禁止精确概率边界，不暴露隐藏评分规则。
- Required Inputs:
  - `OwnerRef`
  - `report_input_package_ref`
  - `pressure_session_ref`
...
- Low Confidence Rules:
  ...
  - 分项解释与总分冲突。
  - 上下文裁剪影响评分依据。
- Evidence Requirements: 每个 section 的分数、解释、positive evidence、negative evidence、uncertainty reasons、规则版本、可信度和免责声明必须绑定 source refs、evidence refs、validation result refs 和 trace refs；证据不足或规则版本缺失时必须保留低置信度。
...
- Security Notes: 分项解释只使用当前 owner 的授权评分、题答摘要和可展示证据摘要；不得暴露无权限来源正文、原始 Prompt、completion、provider payload、隐藏评分规则或内部校准细节。
- Test Strategy: 使用 fixture 覆盖 0-100 范围、分项展示刻度、低分解释、高分解释、评分规则版本缺失、分项与总分冲突、禁止虚构公式 / 权重 / 阈值、禁止精确通过概率、禁止必过 / 必挂、validation failed 不落正式评分和不得创建正式回流对象。
- Open Questions: 隐藏评分规则实现细节、真实招聘结果校准、复杂调参和运营发布流程仍待后续 API / F5 / F7 收敛；本 contract 已冻结 MVP 可展示分、版本、证据、低置信度和禁止概率边界。
...
- Low Confidence Rules:
  ...
  - 风险和评分解释冲突。
  - 通过倾向无法从证据支持。
- Evidence Requirements: 每个 risk item、risk summary、pass tendency wording、uncertainty disclaimer、non decision disclaimer、forbidden wording flags 和 user visible caveats 必须绑定 source refs、evidence refs、validation result refs、score version、rubric version 和 trace refs；证据不足或来源不可用时必须保留低置信度和用户可见 caveat。
...
- API State Mapping: 只定义状态语义，包括 `risk_wording_available`、`risk_wording_partial`、`risk_wording_low_confidence`、`risk_wording_validation_failed`、`pass_tendency_available` 和 `pass_tendency_insufficient_evidence`；不定义 endpoint 或 schema。
- Security Notes: 风险和通过倾向表达只使用当前 owner 的授权评分、风险信号和可展示证据摘要；不得暴露无权限来源正文、原始 Prompt、completion、provider payload、隐藏评分规则、内部校准细节、未脱敏第三方公司 / 面试官 / 个人敏感信息，不得输出恐吓、歧视或攻击性表达。
- Test Strategy: 使用 fixture 覆盖有风险信号、无风险信号、source unavailable、score explanation 缺失、风险和评分解释冲突、低置信度不输出确定倾向、禁止精确通过概率、禁止必过 / 必挂 / 稳过 / 一定失败、候选 Weakness 不得写成正式诊断、risk_level 与 evidence_refs 同步存在、score_version / rubric_version 存在和不得创建正式回流对象。
- Open Questions: 真实招聘结果校准、复杂风险排序和最终 UX 展示文案润色仍待后续 API / UX / F7 收敛；本 contract 已冻结 MVP 通过倾向分档、风险等级、证据绑定、免责声明和禁止概率边界。
...
- Trigger:
  ...
  - 系统需要生成剪贴板可用的报告内容结构。
- Goal: 从报告、评分解释、风险提示、证据摘要和用户可见内容中组装可复制内容结构；本 contract 只组装剪贴板可用内容，不改变报告结论，不生成新的评分，不生成新的风险判断，不生成文件。
- Required Inputs:
  - `OwnerRef`
  - `report_ref`
  - report sections
...
- Security Notes: 可复制内容必须遵守当前 owner、展示权限、脱敏和来源可用性边界；不得包含无权限来源正文、`system prompt`、原始 Prompt、completion、provider payload、隐藏评分规则、密钥、token、cookie、未经脱敏的第三方 / 公司 / 个人敏感信息或不可展示敏感原文。
- Test Strategy: 使用 fixture 覆盖完整报告复制、单 section 复制、缺少分项解释、缺少风险提示、source unavailable、敏感字段脱敏、禁止生成文件、禁止新增报告结论 / 评分 / 风险判断、禁止包含 `system prompt` / 原始 Prompt / completion / provider payload / 隐藏评分规则和用户复制偏好冲突。
- Open Questions: 复制反馈 UI、脱敏策略细节、剪贴板格式提示和用户设备副本责任边界仍待后续 API / UX / 安全设计收敛，为 deferred_non_blocking。
