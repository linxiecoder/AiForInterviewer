...
Pressure 7B 只负责生成连续追问题目、控制节奏和压力强度、判断是否建议结束整场、生成整场评分，以及组装报告输入包。Pressure 7B 不生成最终面试报告正文，不生成正式复盘，不正式创建 Weakness，不正式创建 Asset，不正式创建 TrainingRecommendation，也不关闭压力强度算法、追问深度算法、结束条件、整场评分公式、通过倾向展示或 RAG / 检索实现 UNKNOWN。
...
- Open Questions: 追问深度算法、压力强度算法、题目排序、最大连续追问轮数和技术追问知识证据策略仍待后续 contract / API / UX 收敛，不在本 contract 关闭。
...
- Open Questions: 压力强度算法、压力强度阈值、疲劳信号阈值、追问深度调节和节奏状态机细节仍待后续 contract / API / UX 收敛，不在本 contract 关闭。
...
- Trigger:
  ...
  - 连续多轮低置信度或输入不足。
  - 系统需要决定继续、暂停、结束或转入评分。
- Goal: 根据 session summary、已问问题、focus coverage、回答质量、追问状态、节奏控制和低置信度判断是否建议结束整场压力面；本 contract 只生成结束条件判断，不生成最终报告，不关闭结束条件 UNKNOWN。
- Required Inputs:
  - `OwnerRef`
  - `pressure_session_ref`
  - recent Pressure turns
...
- Open Questions: 结束条件阈值、最小题量、coverage 计算、低置信度占比阈值和自动结束策略仍待后续 contract / API / UX 收敛，不在本 contract 关闭。
...
- Trigger:
  ...
  - 用户重新生成整场评分。
- Goal: 基于整场 Pressure turns、回答质量、追问过程、风险信号、coverage 和 evidence 生成整场 0-100 评分与解释；本 contract 不冻结整场评分公式、权重、阈值或校准方法，只冻结评分输出必须具备的结构、解释、证据、低置信度和 trace。
- Required Inputs:
  - `OwnerRef`
  - `pressure_session_ref`
  - Pressure turns
...
- Retrieval Sources:
  ...
  - 条件读取必须经过 `P-SHARED-002`。
  - 互联网检索不默认启用。
  - 无公共评分口径或 RAG 时仍可生成基础整场评分，但必须保留评分规则 UNKNOWN 或低置信度边界。
- Context Assembly:
  - 必须继承 `P-SHARED-001` 的最小必要上下文、owner 校验、来源可用性、裁剪、omitted refs 和 trace 规则。
  - 上下文至少包含整场 turns 摘要、answer quality summaries、追问链路、coverage、risk signals、end condition、评分目标、评分规则版本或 UNKNOWN 占位和输出 schema。
...
- Excluded Inputs:
  ...
  - 精确通过概率、必过 / 必挂预测、通过倾向文案和真实面试结果预测。
...
- Evidence Requirements: 分数、维度分、正向证据、负向证据、风险信号、质量趋势、coverage、低置信度影响和不确定性原因必须绑定整场题目、回答、回答质量判断、追问链路、end condition、session summary 或授权 evidence；证据不足时必须输出低置信度或要求补充输入。
- Trace Requirements: 必须记录 `TraceRef`，覆盖条件 Retrieval Planning、Input Evidence Selection、Context Assembly、评分规则版本选择或 UNKNOWN 占位、整场评分生成、Output Evidence Binding、Output Validation、Low Confidence Classification、Session Summary Update handoff、ScoreResult handoff、Persistence handoff 和 AuditEvent。
- Persistence Targets:
...
- Retry / Fallback:
  ...
  - 评分规则版本或公共评分口径未冻结时可使用 UNKNOWN 占位生成基础评分，并标记低置信度和不确定性原因。
  - 重试不得默认启用互联网检索、虚构评分公式、输出精确通过概率或创建正式回流对象。
- API State Mapping: 只定义状态语义，包括 `session_score_available`、`session_score_partial`、`session_score_low_confidence`、`session_score_validation_failed`、`score_rule_unknown` 和 `report_input_ready`；不定义 endpoint 或 schema。
- Security Notes: 整场评分只使用当前 owner 的 pressure session、评分规则引用和授权 evidence；用户可见摘要不得暴露无权限来源正文、原始 Prompt、completion 或 provider payload，不得包装出未验证的通过倾向。
- Test Strategy: 使用 fixture 覆盖 0-100 范围、低分解释、高分解释、评分规则 UNKNOWN、禁止精确通过概率、禁止必过 / 必挂、禁止复用 Job Match / Polish score、无 RAG 降级和不得创建正式回流对象。
- Open Questions: 整场评分公式、权重、阈值、校准方法、维度分定义和通过倾向展示仍待后续 contract / API / UX 收敛，不在本 contract 关闭。
...
- Open Questions: Report contracts 的正式报告结构、分项解释、风险提示、通过倾向表达、复制内容结构和报告生成 readiness 阈值仍待后续授权填充，不在本 contract 关闭。
...
- Pressure `P-PRESSURE-001` 至 `P-PRESSURE-009` 不得关闭压力强度、追问深度、结束条件、整场评分公式、通过倾向或 RAG 实现 UNKNOWN。
- Report / Review / Weakness / Asset / Training contracts 仍保持 Stub，等待后续阶段授权填充。
