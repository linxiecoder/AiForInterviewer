...
- 当前版本是骨架草案，用于先对齐 `DATA_MODEL.md` 与 `PROMPT_SPEC.md` 已出现的候选态、正式态、建议态、用户确认流、状态语义和 response envelope 边界。
...
| 建议对象 | `WeaknessMergeSuggestion`、`WeaknessSeverityAssessment`、`WeaknessStatusUpdateSuggestion`、后续 Asset / Training suggestion | 表示推荐动作、提示或评估；不等于实际业务写入动作 |
