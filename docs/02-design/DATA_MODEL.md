...
MVP 不要求保存 `original_file`、`parsed_file`、MIME、文件名或文件大小；`file_upload` / PDF 类来源如未来需要，只能作为 Deferred enum 扩展，不能成为 MVP required path。简历版本创建触发、版本保留策略和历史引用细节按 §8 / §12 的 F4 处置表承接：保存 Markdown 正文时创建新 `ResumeVersion`，历史结果引用生成时版本或快照，回滚以新版本写入而不改写历史。
...
- 项目经历表达版本：项目经历仍是 `ResumeModule(type=project_experience)`；打磨结果先进入 `AssetCandidate` 或 `AssetVersionSuggestion`，用户确认后可更新简历模块或资产版本，并保留来源引用。
