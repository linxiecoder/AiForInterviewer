...
其中 `<postgresql_database_url>` 的格式为 `postgresql+psycopg://<user>:<password>@localhost:5432/<database>`，真实值只写入本地 `.env`，不要写入文档或命令历史。
...
上述 compose 只用于本地开发，并强制从本地 `.env` 读取 `POSTGRES_USER`、`POSTGRES_PASSWORD` 和 `POSTGRES_DB`。不要把真实数据库密码或真实生产连接串写入 `.env.example`、文档、测试或日志。
...
其中 `<postgresql_test_database_url>` 的格式为 `postgresql+psycopg://<user>:<password>@localhost:5432/<test_database>`，真实值只写入本地 `.env`。
