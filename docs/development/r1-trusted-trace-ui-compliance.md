...
2026-05-01 补充：根路径 `/` 已从旧 W10 mock 原型切换为 R1 工作台首页，旧 mock 迁移到 `/legacy-mock` 和 `/mock`。首页只做工作台级入口、最近记录、可信能力摘要和风险空态，不新增后端能力，不改变可信详情页 API 读取逻辑。
...
- 根路径 `/` 已通过真实浏览器 E2E 保护，可展示 R1 工作台首页，并能从最近记录进入 `/interviews/:sessionId`。
...
- `App.tsx` 只做轻量 route 分发：`/` 指向 R1 工作台首页，`/interviews/:sessionId` 指向可信详情页，`/legacy-mock` 和 `/mock` 指向旧 mock 原型。
...
- 引入 `antd` 且新增 R1 首页后，当前 Vite build 输出主 JS chunk 约 `740 kB`，触发 `Some chunks are larger than 500 kB` warning。
...
- R1 首页当前是静态工作台入口和示例最近记录，真实历史列表、发起表单、岗位 / 简历 / RAG 管理仍需后续 R1 slice 接入后端数据。
