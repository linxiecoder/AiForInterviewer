...
- 当前授权已从 `R1-DEV-02-ST13_20-SCHEMA-MIGRATION-TRACEABILITY-SLICE` 推进到 `R1-DEV-03-ST13_20-TRACEABILITY-INTEGRATION-SLICE`，仅放行 R1 主业务边界的最小 traceability integration，不放行完整数据层、完整 ORM、复杂 RBAC、R2 训练闭环、资产归档或批量导出。
...
## 20. R1-DEV-03 gate 输入与 packet 授权修正

本节修正 official implementation packet 将提取的范围输入。当前窗口目标已从 R0 最小保存边界和 R1 数据追踪基础推进到 R1 traceability integration；本节只授权 schema SQL、既有 sqlite persistence pattern、最小 traceability domain 代码、现有 API / service 调用点和 targeted tests，不授权完整数据层重构。
...
- 将已完成的 TraceabilityStore 接入 R1 主业务边界，使 interview、RAG、score、review、export 和 history 的最小业务链路开始产生并读取 traceability record。

### 20.2 实施范围边界

R1 traceability 最小切片只覆盖关系可保存、可读取和可测试，不重写主业务链路，不实现完整 RAG、评分、复盘或导出能力：

- job / resume / interview session / turn / answer 关系。
- score / review / export 对 session、turn、answer、evidence 和 source snapshot 的引用。
- RAG retrieval query / result / citation / evidence gap 的引用边界。
- degraded、failed、retryable、archived 等状态的持久化表达。
- interview / RAG / score / review / export / history 现有服务函数中的最小 traceability 写入点。
- 最小读取 helper 或接口，使 targeted tests 能验证一条业务链路的 trace 记录可被 owner / visibility 边界过滤后读取。
...
上述 allowed / forbidden paths 只作为本次 R1 traceability slice 的 implementation packet 输入。Phase A 允许通过 official command 重新生成 packet；Phase B 只能在 packet 授权路径内修改代码和 targeted tests，且 packet 禁止范围优先级高于允许范围。
...
- 检查 packet allowed paths 覆盖 TraceabilityStore、schema、API router、interview flow、RAG、scoring、review、export 和 targeted traceability tests 的最小集成路径。
- 检查实现只沿用已有 sqlite3 / schema SQL 模式，不新增 ORM、Alembic、数据库依赖或 provider 依赖。
- 检查 targeted tests 至少覆盖 interview 主流程 session / turn / answer trace、RAG empty 或 degraded evidence gap trace、score / review reference、export / history reference、degraded / failed / retryable 状态和 permission / visibility 过滤。
...
- targeted persistence tests：创建并读取岗位、简历、session、turn、answer 的追踪关系。
- targeted integration tests：通过现有 API / service 边界验证 interview、RAG、score、review、export 和 history 会写入并读取 traceability record。
- targeted RAG tests：保存 retrieval query / result、citation 和 evidence gap reference。
...
- R1 数据追踪基础的最小关系、interview / RAG / score / review / export / history trace reference 和失败 / 降级 / 空状态可被 targeted tests 验证。
- git diff --check 通过。
