...
- 当前授权仅限 `R1-DEV-02-ST13_20-SCHEMA-MIGRATION-TRACEABILITY-SLICE` 的 R1 数据追踪基础，不放行完整数据层、完整 ORM、复杂 RBAC、R2 训练闭环、资产归档或批量导出。
...
## 20. R1-DEV-02 gate 输入与 packet 授权修正

本节修正 official implementation packet 将提取的范围输入。当前窗口目标已从 R0 最小保存边界推进到 R1 数据追踪基础；本节只授权 schema SQL、既有 sqlite persistence pattern、最小 traceability domain 代码和 targeted tests，不授权完整数据层重构。
...
R1 traceability 最小切片只覆盖关系可保存、可读取和可测试，不实现完整业务 API：
...
- 最小 repository / persistence adapter，使 targeted tests 能验证写入和读取。

本 slice 不创建业务 endpoint，不实现完整 API contract，不保存完整 prompt 原文、完整 LLM response 原文、embedding 向量、provider secret 或对象存储真实路径。
...
- failure-state tests：保存 degraded、failed、retryable 和 archived 状态，确认不会破坏读取。
...
4. 需要实现完整 migration 体系、完整 ORM 体系、完整业务 API、LLM / RAG provider、RAG 检索、训练闭环、资产归档或批量导出。
