...
type: note
permalink: ai-for-interviewer/docs/modules/m08-review-and-replay/module-open-questions
---
...
| OQ-010 | 归档粒度是整份资产、片段还是题目级 | confirmed | M08 / M05 / M10 | 已由 `FC-14` 覆盖：支持整份和单题归档到资产库 | 否 | `OPEN_QUESTIONS.md`、对象模型 / RAG / 多轮 / 后端边界输入 |
...
| OQ-015 | 真实面试输入是结构化问答、自由 transcript 还是混合 | confirmed | M08 | 已由 `FC-11` 覆盖：真实面试支持上传逐字稿原文，由大模型识别问答边界并在低置信度时提示校对 | 否 | `OPEN_QUESTIONS.md`、评分 / 复盘 / 导出 / DoD 输入 |
