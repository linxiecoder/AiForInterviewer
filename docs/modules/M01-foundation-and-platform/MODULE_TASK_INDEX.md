...
| --- | --- | --- | --- | --- | --- | --- | --- |
| ST01_01 | 运行环境与仓库基线 | 收敛 monorepo 目录、环境模板、本地基础设施占位、FastAPI 最小入口与健康检查 | runtime baseline implementation 已完成；`maturity=L4`；`readiness=downstream_ready`；scoped `formal_window_status=open`；`candidate_status=none`；`implementation_approval_status=approved`；`implementation_ready=true`；`can_generate_implementation_packet=true` | `MODULE_REQUIREMENTS.md`、`MODULE_DESIGN.md`、`MODULE_API_DESIGN.md`、`SUBTASK_DESIGN.md`、`SUBTASK_IMPLEMENTATION.md` | 无新增 gate blocker；runtime baseline 实施范围已落地，输出作为 ST01_02 前置。 | runtime baseline implementation 已完成 | 是，作为 runtime baseline implementation packet 输入；当前窗口不代表 runtime code implementation |
...
- ST01_01 scoped formal window 已打开，implementation approval 已写入，当前窗口已完成 runtime baseline implementation；后续不得将现有表述替代正式实现授权边界。
