...
- `ST01_01` 双文档已由历史骨架重建为 formal window preparation 输入；正式 `DOC_STATE.yaml` 已存在 `subtasks.ST01_01` entry，且 `implementation_doc_state=active_working_doc`、`candidate_status=none`、`readiness=blocked`、`formal_window_open=false`、`implementation_ready=false`。
- `ST01_02` / `ST01_03` 的旧 `SUBTASK_DESIGN.md` / `SUBTASK_IMPLEMENTATION.md` 若仍为骨架或模板，不因被本索引链接而获得正式入口、candidate 或开窗资格。

| 子任务文档 | 父级索引 | 当前用途 |
| --- | --- | --- |
| [`ST01_01/SUBTASK_DESIGN.md`](sub_modules/ST01_01-runtime-and-repo-baseline/SUBTASK_DESIGN.md)、[`ST01_01/SUBTASK_IMPLEMENTATION.md`](sub_modules/ST01_01-runtime-and-repo-baseline/SUBTASK_IMPLEMENTATION.md) | 本文件第 1 节 `ST01_01` | 已存在正式状态 entry；`implementation_doc_state=active_working_doc`；仍 blocked 且 formal window closed |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| ST01_01 | 运行环境与仓库基线 | 收敛 monorepo 目录、环境模板、本地基础设施占位、FastAPI 最小入口与健康检查 | formal window preparation 输入；`candidate_status=none`；`readiness=blocked`；`implementation_ready=false` | `MODULE_REQUIREMENTS.md`、`MODULE_DESIGN.md`、`MODULE_API_DESIGN.md`、`SUBTASK_DESIGN.md`、`SUBTASK_IMPLEMENTATION.md` | `formal_window_open=false`；当前未打开 formal window；不得创建 `apps/api/**`、`infra/**` 或运行时代码 | 由独立 formal window open 窗口处理开窗；之后才可进入实施窗口 | 是，仅限 ST01_01 文档入口与 formal window preparation；不代表 implementation-ready |
...
- ST01_01 已存在正式状态 entry，可作为后续 formal window preparation 输入；当前 `candidate_status=none`、`readiness=blocked`、`implementation_ready=false`。
- ST01_01 不代表 `candidate_status=candidate`，不代表 `formal_window_open=true`，不代表可生成 implementation packet，也不代表 `implementation_ready`。
