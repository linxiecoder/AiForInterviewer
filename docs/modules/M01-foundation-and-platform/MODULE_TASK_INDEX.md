...
| --- | --- | --- | --- | --- | --- | --- | --- |
| ST01_01 | 运行环境与仓库基线 | 收敛 monorepo 目录、环境模板、本地基础设施占位、FastAPI 最小入口与健康检查 | runtime packet input correction；`maturity=L4`；`readiness=downstream_ready`；scoped `formal_window_status=open`；`candidate_status=none`；`implementation_approval_status=approved`；`implementation_ready=true`；`can_generate_implementation_packet=true` | `MODULE_REQUIREMENTS.md`、`MODULE_DESIGN.md`、`MODULE_API_DESIGN.md`、`SUBTASK_DESIGN.md`、`SUBTASK_IMPLEMENTATION.md` | 无当前 gate blocker；当前旧 packet 需替换；新 packet 应允许 `apps/api/**`、`.env.example`、`infra/**`、`package.json`、`requirements.txt`、`pytest.ini` 的最小 runtime baseline 范围，并继续禁止业务实现、`tools/**`、`tests/**` 与重型基础设施 | 本轮输入文档修正后重新生成 packet；packet 审查通过后才进入 runtime baseline implementation | 是，作为 runtime baseline implementation packet 输入；当前窗口不代表 runtime code implementation |
...
- ST01_01 已存在正式状态 entry，当前 `maturity=L4`、`readiness=downstream_ready`、scoped `formal_window_status=open`、`candidate_status=none`、`implementation_approval_status=approved`、`implementation_ready=true`、`can_generate_implementation_packet=true`。
- ST01_01 scoped formal window 已打开，implementation approval 已写入，当前旧 implementation packet 已生成但需替换；输入文档已转为 runtime baseline packet 输入，后续 regenerated packet 通过审查前不代表 runtime code implementation 授权。
