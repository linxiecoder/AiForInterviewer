...
| ST01_02 | 工作台壳层与 i18n 基线 | 收敛 Dashboard 壳层、Page Header、i18n seed、列表原语与基础页面样式 | 等待模块 L5 | `MODULE_REQUIREMENTS.md`、`MODULE_DESIGN.md`、`MODULE_API_DESIGN.md`、`MODULE_SCHEMA_DESIGN.md`、`MODULE_LOGIC_DESIGN.md` | 共享页面原语虽已形成 `proposed-default` 口径，但列表查询状态的实现级接口、locale 规则和视觉范围仍未冻结 | 吸收共享页面原语默认口径，并继续冻结列表状态 / locale / 页面状态边界 | 否 |
...
- 其中最关键的上游缺口是：
  - 根目录脚本与最小 CI 命令矩阵未冻结。
  - `PageHeader`、Dashboard 摘要区虽已形成默认冻结候选，但列表查询状态的完整实现级接口仍未冻结。
