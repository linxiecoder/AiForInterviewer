...
- 当前用途：作为后续重新生成 ST01_01 runtime baseline implementation packet 的输入文档，并用于本轮 packet input correction。
- 当前限制：本窗口只修正 packet 输入文档；本文档不修改 `docs/governance/DOC_STATE.yaml`，不修改 packet 文件，不进入 runtime code 实现。
...
ST01_01 只负责运行环境与仓库基线，目标是把 M01 中已经收敛的最小工程边界整理为 runtime baseline implementation packet 输入，并明确本窗口文档修正与后续 runtime implementation 的边界。

本子任务覆盖：

- workspace baseline：根目录脚本、包管理入口、环境样例和 monorepo 目录边界。
- API runtime baseline：后续 regenerated packet 通过审查后，可授权最小 `apps/api/**` 入口、`/api/v1/health` 和 API lane 验证口径。
...
2. 为后续 regenerated packet 明确最小运行环境与仓库基线 allowed / forbidden / DoD / validation 输入。
3. 明确当前 `apps/web` 只能作为 W10 mock / 前端原型资产和历史参考，不是 Workbench MVP 的正式实现基础。
4. 明确 ST01_01 继续承接 runtime baseline，不另开新 task；后续必须重新生成 packet，不能使用当前旧 packet 绕过 forbidden paths。
5. 明确 `/api/v1/health` 的最小契约：只返回 `{ "status": "ok" }`，不访问数据库、Redis、对象存储或 LLM。
6. 将当前窗口限制在 packet 输入文档修正，不把 Markdown 修正误读为 runtime code implementation 授权。
...
- 不重新生成 implementation packet，不修改已生成 packet 文件，不把当前旧 packet 扩张为 runtime code implementation 授权。

## 6. 当前仓库基线判断

当前仓库已具备 ST01_01 packet generation gate，但当前已生成 packet 仍是旧的 docs/index-only packet，必须在本轮输入文档修正后重新生成：
...
- 当前已生成 implementation packet 的 `allowed_modify_paths` 仍仅覆盖 ST01_01 双文档、`TASK_INDEX.md`、`MODULE_TASK_INDEX.md` 与 `MODULE_DEPENDENCIES.md`，与 canonical runtime baseline target 不匹配。
- 当前窗口仍不得创建 `apps/api/**`，不得修改 runtime code，不得启动 M02/M03。
- 当前 `apps/web` 若存在，只能视为 W10 mock / 前端原型资产；它可以作为历史观察材料，但不能作为 Workbench MVP 正式实现基线。

本轮结论：ST01_01 已通过 packet generation gate，且仍应继续承接 runtime baseline。当前旧 packet 不能用于 runtime implementation；后续应在本轮输入文档修正后重新生成 packet 并替换旧 packet。

## 7. 设计范围

### 7.1 工作区基线

后续 regenerated packet 若通过审查，可围绕以下最小边界设计执行；当前窗口不执行 runtime code：

- 根目录脚本命名保持 API / Web 双 lane 可区分。
- 根目录脚本不得隐式启动真实数据库、缓存、对象存储或 LLM provider。
- `.env.example` 只允许声明最小本地开发变量和占位说明，不得写入真实密钥。
- 目标目录拓扑为 `apps/web`、`apps/api`、`infra`；本次 runtime baseline allowed paths 不包含 `apps/web/**`，仅候选包含 `apps/api/**` 与 `infra/**` 本地占位。
- 任何脚本、环境变量或依赖入口都必须能被后续测试命令验证，而不是只停留在 README 说明。

### 7.2 API 运行时基线

后续 regenerated packet 若通过审查，可创建最小 API runtime，但必须满足：
...
后续 regenerated packet 可对 Web lane 做最小不破坏验证，但必须先处理当前事实边界：
...
后续 regenerated packet 可补充最小环境说明，但必须满足：
...
- 一个可供 regenerated runtime baseline packet 使用的最小 runtime / repo baseline 设计入口。
- 对 `apps/web` 原型资产与正式 Workbench MVP 实现基线的明确区分。
- 对后续 regenerated packet 候选范围中 `apps/api/**`、根脚本、`.env.example`、`infra/**` 和健康检查的最小建议范围；该建议不构成本窗口 runtime code 授权。
...
- 新的 implementation packet；当前输入文档被修改后，后续应重新生成 packet。

## 9. 风险与约束

- 状态误读风险：当前 `implementation_ready=true` 容易被误读为 runtime code 已授权；必须继续以后续 regenerated packet 的 allowed / forbidden paths 控制实施边界。
...
- 已明确后续 regenerated packet 候选范围应包含最小 `apps/api/**`、根脚本、`.env.example`、`infra/**` 占位和健康检查。
- 已明确 `/api/v1/health` 只返回 `{ "status": "ok" }`，不访问数据库、Redis、对象存储或 LLM。
- 已明确不做 M02 auth、不做业务路由、不做数据库、不做 RAG、不做评分复盘导出。
- 已明确本文档可作为 runtime packet input correction 输入；当前 official state 已 approved 且 `implementation_ready=true`，但不代表本窗口 runtime code implementation 授权。
...
- 当前已生成 implementation packet 仍是旧的 docs/index-only packet；本轮输入文档修正后应重新生成 packet。

本文档不得把 `implementation_ready=true` 或当前旧 packet 改写为 runtime code implementation 授权。

## 12. 当前不进入实现说明

本窗口只完成 ST01_01 runtime packet input correction。即使 official state 已 approved、`implementation_ready=true` 且当前旧 packet 已生成，也不代表：
...
- 已授权使用当前旧 packet 执行 runtime code implementation。
- 已授权绕过后续 regenerated packet 的 allowed / forbidden paths。
- M02 / M03 已可启动业务实现。

下一步若要推进 runtime baseline，必须重新生成 ST01_01 implementation packet；本文档修复不改变正式状态，也不修改当前已生成 packet 文件。
