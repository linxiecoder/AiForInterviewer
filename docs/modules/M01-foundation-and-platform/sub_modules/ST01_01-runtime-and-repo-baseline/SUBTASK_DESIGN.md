...
- 当前用途：为后续 ST01_01 formal window 提供设计输入和边界说明。
- 当前限制：本文档不修改 `docs/governance/DOC_STATE.yaml`，不打开 formal window，不生成 implementation packet，不声明 `implementation_ready`。
- 状态口径：`docs/governance/DOC_STATE.yaml` 是正式真值；本文档只记录当前事实和后续审查输入，不自封 `candidate_status`、`readiness` 或 `implementation_ready`。

## 2. 子任务定位

ST01_01 只负责运行环境与仓库基线，目标是把 M01 中已经收敛的最小工程边界整理为后续 formal window 可执行的入口范围。
...
2. 为后续 formal window 明确最小运行环境与仓库基线范围。
3. 明确当前 `apps/web` 只能作为 W10 mock / 前端原型资产和历史参考，不是 Workbench MVP 的正式实现基础。
4. 明确未来 formal window 可建议创建最小 `apps/api/**`、最小 `infra/**`、根脚本与健康检查，但本轮不创建。
5. 明确 `/api/v1/health` 的最小契约：只返回 `{ "status": "ok" }`，不访问数据库、Redis、对象存储或 LLM。
6. 将 ST01_01 的输出限制在 formal window preparation，不把文档重建误读为 `implementation_ready`。
...
- 不打开 formal window，不生成 implementation packet，不标记 implementation-ready。

## 6. 当前仓库基线判断

当前仓库已具备 ST01_01 formal window preparation 的文档输入，但尚未具备实施授权：
...
- ST01_01 当前仍是 `candidate_status=none`、`readiness=blocked`、`implementation_ready=false`。
- 全局 `formal_window_open=false`，因此任何 formal window、implementation packet 或代码实现仍保持关闭。
- 当前 `apps/web` 若存在，只能视为 W10 mock / 前端原型资产；它可以作为历史观察材料，但不能作为 Workbench MVP 正式实现基线。

本轮结论：ST01_01 可重建为正式子任务入口并进入 formal window preparation，但不能据此创建运行时代码或声明实现准备完成。

## 7. 设计范围

### 7.1 工作区基线

未来 formal window 可围绕以下最小边界设计执行，但本轮不执行：
...
未来 formal window 可创建最小 API runtime，但必须满足：
...
未来 formal window 可对 Web lane 做最小验证，但必须先处理当前事实边界：
...
未来 formal window 可补充最小环境说明，但必须满足：
...
未来 formal window 的最小验证应分为治理验证和工程验证：
...
- 一个可供 formal window 使用的最小 runtime / repo baseline 设计入口。
- 对 `apps/web` 原型资产与正式 Workbench MVP 实现基线的明确区分。
- 对未来 `apps/api/**`、`infra/**`、根脚本、`.env.example` 和健康检查的最小建议范围。
...
- 下游抢跑风险：ST01_01 的 formal preparation 不代表 M02 / M03 可以开始业务实现。
...
- 已明确未来 formal window 可建议创建最小 `apps/api/**`、最小 `infra/**`、根脚本和健康检查，但本轮不创建。
- 已明确 `/api/v1/health` 只返回 `{ "status": "ok" }`，不访问数据库、Redis、对象存储或 LLM。
- 已明确不做 M02 auth、不做业务路由、不做数据库、不做 RAG、不做评分复盘导出。
- 已明确本文档可作为 formal window 输入，但不代表 formal window 已打开，也不代表 implementation-ready。
...
本轮只完成 ST01_01 formal window preparation。即使本设计文档已经重建，也不代表：
...
下一步若要推进开窗，必须由独立 formal window open 窗口处理；本文档修复不改变正式状态。
