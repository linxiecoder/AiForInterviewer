...
type: note
permalink: ai-for-interviewer/docs/modules/m03-jobs-resumes-and-documents/module-requirements
---
...
- 当前状态：高 `L4`，本轮按 `MR-15` 执行“`OQ-024` 过时表述清理 + 真实剩余条件回写”，继续维持“已吸收但未放行”的白名单观察状态，不登记为正式 `L5` 候选。
- 本轮判断：
  - 模块级边界、最小字段面、同步/异步职责，以及 `OQ-021` / `OQ-025` 的最低位吸收口径已收口到足够支撑模块设计输入对齐与白名单观察的程度，但仍不足以外推上传 / 导出微任务 readiness。
  - 当前真实剩余条件已收口为：正式开窗层当前为空、最低位文档仍是高 `L4` 的 `MODULE_API_DESIGN.md`、当前阶段仍关窗、上传 / 导出链依赖未变；因此不得把本模块误写成“已正式进入子任务设计候选”。
...
- 为 M04-M06 提供最小可引用的岗位正文、结构化要求、当前简历正文、版本记录和文件产物引用。
...
- 领域对象最小字段面已收紧：
  ...
  - `resume_documents.markdown_content` 与 `resumes.current_document_id` 已明确为下游最小可引用输入。
  - `jobs.requirement_items_json` 已按 `OQ-025` 的 `proposed-default` 口径吸收到模块层：JSON array item 至少含 `item_key` / `text`，`null` 表示未完成结构化，`[]` 表示已结构化但无项，数组顺序即消费顺序；该口径当前只足够支撑 M03 / M04 / M06 的模块设计输入对齐、模块级收口、白名单观察与最小下游引用，不等于完整岗位链 / 下游链已 ready，更不等于上传 / 导出相关微任务条件已齐。
- 关键业务规则已收口：
...
### 10.2 仍留给后续微任务级细化，当前不应被误读为正式候选已 ready
...
- `OQ-021` 的共享最小映射当前只覆盖 `page / page_size / q / status / sort / order`；`updated_after / updated_before` 只能在 M03 内部作为模块级扩展单独登记，不得被误写为共享最小映射或白名单 readiness 前提。
- `OQ-024` 的模块内引用已按全局固定映射吸收：旧 `ST03_*` 只保留为历史容器、`MT03_01` / `MT03_03` 只保留为白名单观察蓝本、正式开窗层当前为空；因此当前状态只能写成“已吸收但未放行”。

### 10.3 当前剩余阻塞与待确认

- `OQ-024` 已完成模块内吸收；当前只保留固定口径：正式开窗层当前为空。
- 当前模块最低成熟度文档仍是高 `L4` 的 `MODULE_API_DESIGN.md`；最低位未跨过 `L5` 前，不得外推为正式候选已放行。
- 当前阶段仍关闭子任务设计窗口；`MT03_01` / `MT03_03` 只能继续作为白名单观察面。
- 上传 / 导出相关微任务的依赖判断未变：仍主要受 `OQ-007` 与 M01 共享下载能力 / 对象存储口径阻塞，`OQ-025` 的稳定吸收不改变 `MT03_06` / `MT03_07` / `MT03_08` 的判断。
- `MT03_01`、`MT03_03` 当前只保留为白名单观察面：它们对应的模块层输入虽已基本收紧，但正式开窗层为空、最低位 API 仍是高 `L4`、当前阶段仍关窗，且上传 / 导出链依赖未变，因此尚不满足“正式进入子任务设计候选”的最小条件。
