...
- 当前状态：高 `L4`，重点明确接口边界、同步/异步职责、最小查询键和跨模块归属；本轮按 `MR-25` 做最低位候选前复核，并把结构性主阻塞收紧为“正式开窗层为空 + 当前阶段关窗 + 上传 / 导出链依赖未变”；`OQ-021` / `OQ-025` 的最低位吸收口径保持不变。
- 鉴权基线引用 M02：
  - 所有接口默认要求已登录用户。
...
- `OQ-024` 的三层映射引用已按全局口径吸收到模块内：旧 `ST03_*` 只保留为历史容器、`MT03_01` / `MT03_03` 只保留为白名单观察蓝本、正式开窗层当前为空；这意味着当前没有任何 `MT03_*` 被总控提升为正式候选入口，因此本最低位文档当前只能维持“已吸收但未放行”的判断。
- 当前模块最低成熟度文档仍是高 `L4` 的 `MODULE_API_DESIGN.md`；其主因已不是 `OQ-021` / `OQ-025` 未吸收，而是本模块 API 文档仍覆盖 `upload-pdf`、`export-pdf`、`original-pdf` 等上传 / 导出链接口，而这些接口的放行前提仍未发生实质变化。
- 当前阶段仍关窗；在关窗状态解除前，即使 `MT03_01` / `MT03_03` 已具备局部可拆输入，也只能继续停留在白名单观察面，不得据此升级为正式候选。
...
- 总控把正式开窗层从“当前为空”推进到至少一个明确的正式候选入口，而不是继续停留在观察蓝本层。
- 总控明确结束当前阶段关窗状态，允许重新评估 `MT03_01` / `MT03_03` 的正式子任务设计候选资格。
- 上传 / 导出链依赖判断发生实质收口，至少不再让 `upload-pdf`、`export-pdf`、`original-pdf` 这些接口继续作为整份 API 文档的结构性阻塞。
