...
- 当前状态：高 `L4`，重点明确接口边界、同步/异步职责、最小查询键和跨模块归属；本轮继续修正 `OQ-021` 漂移与 `OQ-025` 吸收表述。
- 鉴权基线引用 M02：
  - 所有接口默认要求已登录用户。
...
- `OQ-021` 已形成 `proposed-default`，但模块内仍需持续防止把 `updated_after / updated_before` 漂移回共享最小映射；列表页实现级 callback 签名、复杂筛选编码与路由细节仍属于子任务级待确认项。
- `OQ-025` 已形成 `proposed-default`，但岗位写模型当前只承诺最小 item 契约与整体替换责任；扩展字段与派生摘要仍不得被 API 文档写成稳定下游输入。
