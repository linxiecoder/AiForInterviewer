...
- 以下内容必须保留在模块层，不允许再下放到子任务里补洞：
  - `MT02_02` 只允许消费并固化 `CurrentUserContext` 的字段、来源、`401 / 403 / 404` 语义与 `logout=204` 成功口径；不得接管 `/(dashboard)/admin/**`、`/api/v1/admin/**` 的 route group 判定，也不得把这些边界再下放到子任务层。
  - `/members` 的字段投影、跨团队 `403`、软删除过滤，以及与 `OQ-021` 的列表集合对齐边界。
    - 其中共享列表契约只闭合到 `page/page_size/q/status/sort/order` 与统一分页骨架；callback、URL 序列化与 request adapter 细节继续留在页面 / 实现层。
...
| 旧入口 | 映射后的蓝本 | 仍保留在模块层的内容 | 当前放行结论 |
| --- | --- | --- | --- |
| `ST02_01` | `MT02_01`、`MT02_02`、`MT02_05` | `CurrentUserContext`、`logout=204`、`401/403` 语义、`/login` 对共享页面 / i18n 的消费边界 | 旧目录仅保留为历史容器；`MT02_01/MT02_02` 仅进入本轮白名单观察面，`MT02_05` 继续后置 |
| `ST02_02` | `MT02_03`、`MT02_04`、`MT02_06` | `/members` 字段投影、跨团队 `403`、软删除过滤、列表集合与页面承接口径 | 旧目录仅保留为历史容器；`MT02_03` 不进入本轮白名单，仅保留后续观察顺序；`MT02_04/MT02_06` 继续阻塞 |
| `ST02_03` | `MT02_07`、`MT02_08` | route group 权限矩阵、`M02/M10` 边界、鉴权验证基线 | 旧目录仅保留为历史容器；`MT02_07` 不进入本轮白名单，仅保留后续观察顺序；`MT02_08` 等待上游稳定后再判断 |

本轮所谓“白名单观察面”，只表示总控可以继续跟踪最小非页面变化面，并不表示这些蓝本已经获得正式子任务入口或可以开窗。
...
- 当前仍未解除的外部阻塞：
  ...
  - `MQ-205` / `OQ-021`：`/members` 列表已在模块内闭合到共享最小层：沿用 `page/page_size/q/status/sort/order` 与统一分页骨架；`M02` 不再私自补分页协议。当前剩余未定项只限于 callback、URL 序列化与 request adapter 等页面 / 实现细节，因此最低位阻塞已收敛到 `MODULE_API_DESIGN.md` 对该共享口径的吸收引用；这里的“闭合”只代表模块停止误补协议，不代表 `MT02_04 / MT02_06` 已进入正式候选判断。
  - `MQ-206` / `OQ-022`：`/login` 与 `/(dashboard)/members` 已获得 `M01` 集中 i18n 入口的最小 namespace / fallback 默认口径，但模块尚未完成吸收；页面子任务设计暂不宜先开。
  - `MQ-207` / `OQ-024`：全局主文档已把映射写死为“历史容器固定、观察蓝本固定、正式开窗层当前为空”；模块本轮只吸收该引用，不把任何 `MT02_*` 写成已放行子任务入口。
  - `MQ-209`：本轮继续收紧的治理边界是“白名单观察面”与“正式开窗状态”必须分离；若不显式写明，`MT02_01/MT02_02` 会被误解为可直接创建子任务窗口，或把 `MT02_02` 误扩张成 route group 权限定义入口。
- `MQ-203` 已与全局 `OQ-023` 对齐，不再构成当前模块的新增阻塞。
- 当前 readiness 收紧后的保守判断：
  - 本轮白名单观察面仅为 `MT02_01`、`MT02_02`，用于继续观察 auth 基座与 `CurrentUserContext` 边界是否足以进入正式候选复核
  - `MT02_01`、`MT02_02` 继续只属于观察面，不是因为 auth 接口内容又回退，而是因为 `MODULE_API_DESIGN.md` 仍是模块最低位高 `L4`、`OQ-024` 的正式开窗层当前仍为空，且遗留 `ST02_01` 仍只有骨架 / 模板，不能充当正式入口证明
  - `MT02_03`、`MT02_07` 仅保留后续观察顺序，不纳入本轮白名单
  - 白名单观察面不等于正式子任务入口；即使全局 `OQ-024` 映射已写死，当前正式开窗层仍为空，因此不得开窗，也不得补写子任务双文档
  - `MT02_04` 继续受“`/members` 只在模块内闭合到共享最小层 + API 最低位仍未跨过 `L5`”双重约束，不能从“已闭合”外推 ready
  - `MT02_05`、`MT02_06` 继续保留在页面后置队列；其中 `MT02_06` 继承 `MT02_04` 全部阻塞，并额外受 `OQ-020 / OQ-022` 与 request adapter 仍留在页面层的约束
  - `MT02_08` 必须等待 `MT02_04`、`MT02_07` 稳定后再判断
