...
- 当前仍未解除的外部阻塞：
  ...
  - `MQ-207` / `OQ-024`：旧 `ST02_*` 目录虽已按历史容器处理，但总控尚未正式下发 `MT02_*` 的入口映射；当前任何候选微任务都不应被写成“已放行子任务入口”。
  - `MQ-209`：本轮新增的治理边界是“白名单观察面”与“正式开窗状态”必须分离；若不显式写明，`MT02_01/MT02_02` 会被误解为可直接创建子任务窗口。
...
- 当前 readiness 收紧后的保守判断：
  ...
  - 白名单观察面不等于正式子任务入口；在总控未下发 `OQ-024` 正式映射前，不得开窗，也不得补写子任务双文档
  - `MT02_04` 继续受列表共享契约阻塞
  - `MT02_05`、`MT02_06` 继续保留在页面后置队列
