...
| --- | --- | --- | --- |
| `ST02_01` | `MT02_01`、`MT02_02`、`MT02_05` | `CurrentUserContext`、`logout=204`、`401/403` 语义、`/login` 对共享页面 / i18n 的消费边界 | 旧目录仅保留为历史容器；`MT02_01/MT02_02` 只列候选，`MT02_05` 继续后置 |
| `ST02_02` | `MT02_03`、`MT02_04`、`MT02_06` | `/members` 字段投影、跨团队 `403`、软删除过滤、列表集合与页面承接口径 | 旧目录仅保留为历史容器；`MT02_03` 只列候选，`MT02_04/MT02_06` 继续阻塞 |
| `ST02_03` | `MT02_07`、`MT02_08` | route group 权限矩阵、`M02/M10` 边界、鉴权验证基线 | 旧目录仅保留为历史容器；`MT02_07` 只列候选，`MT02_08` 等待上游稳定后再判断 |
...
- 当前 readiness 收紧后的保守判断：
  - 候选前排仅为 `MT02_01`、`MT02_02`，其次是 `MT02_03`、`MT02_07`
  - `MT02_04` 继续受列表共享契约阻塞
