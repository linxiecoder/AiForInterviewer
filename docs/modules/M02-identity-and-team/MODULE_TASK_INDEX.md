...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `MT02_01` | 身份源与 token resolver | `ST02_01` 拆分 | 固定 `Bearer token`、演示身份映射、resolver 输入输出 | 模块核心文档通过 `L5` 复核、`OQ-004` | 无新增共享闸门 | 否，作为 auth 基座优先冻结 | 下一轮最先启动 |
| `MT02_02` | `CurrentUserContext` 与 `/auth/*` 边界 | `ST02_01` 拆分 | `login/me/logout` payload、`CurrentUserContext`、`logout=204` | `MT02_01` | 无新增共享闸门 | 完成后可解锁多个并行面 | 与 `MT02_01` 串行 |
| `MT02_03` | `Team/User` schema 与安全投影 | `ST02_02` 拆分 | `teams/users` 字段、`team_id` 关系、`MemberDirectoryItem` projection | 模块 schema / design 稳定 | 无新增共享闸门 | 可与页面 / policy 面错峰并行 | 先于成员目录 read API |
| `MT02_04` | 成员目录 read API | `ST02_02` 拆分 | `GET /api/v1/members`、`GET /api/v1/members/{member_id}`、过滤与错误语义 | `MT02_02`、`MT02_03` | `OQ-021` | 可与 admin policy 面并行 | 等 `OQ-021` 继续吸收后启动 |
| `MT02_05` | `/login` 页面 adapter | `ST02_01` 拆分 | 登录页表单、登录态承接、共享 i18n 消费 | `MT02_02` | `OQ-020`、`OQ-022` | 可与 schema / policy 面并行 | 不要与 auth backend 再绑在一起 |
| `MT02_06` | `/(dashboard)/members` 页面 adapter | `ST02_02` 拆分 | 列表页容器、`PageHeader` 承接、列表请求 adapter | `MT02_04` | `OQ-020`、`OQ-021`、`OQ-022` | 可与验证面并行 | 等成员目录 API 稳定后再开 |
| `MT02_07` | admin 路由组 policy | `ST02_03` 拆分 | `/(dashboard)/admin/**`、`/api/v1/admin/**` 的角色判断与边界消费 | `MT02_02` | `OQ-005`、`OQ-023` | 可与成员目录 read API、登录页 adapter 并行 | 不承接完整成员管理实现 |
| `MT02_08` | 鉴权验证矩阵 | `ST02_03` 拆分 | `401/403/404`、跨团队、软删除、角色矩阵验证 | `MT02_04`、`MT02_07` | 无新增共享闸门 | 可与 `MT02_06` 并行 | 作为收口型微任务最后启动 |
...
- 当前不建议直接开启任何 `ST02_01 ~ ST02_03` 子任务设计窗口。
- `OQ-020` 已能作为页面承接口径输入，但 `OQ-021` 的实现级细节承接与 `OQ-022` 的页面吸收仍决定页面类微任务何时能真正启动。
- `OQ-023` 已足以支撑职责重切，因此 `MT02_07` 可继续作为独立 policy 面，而不是继续挂在管理台实现里。

## 5. 下一轮开启子任务设计前必须满足

- 模块评审通过，并确认 `MODULE_REQUIREMENTS / DESIGN / API / SCHEMA / LOGIC` 已达到治理要求的下游输入成熟度。
- 总控确认是否沿用现有 `ST02_01 ~ ST02_03` 目录重写，还是按 `MT02_01 ~ MT02_08` 建立新的子任务入口。
