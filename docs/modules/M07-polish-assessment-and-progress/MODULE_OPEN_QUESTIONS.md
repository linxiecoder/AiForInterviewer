...
| OQ-008 | 匹配分析与评估规则是否需要版本化 | confirmed | M07 / M08 / M09 / M10 | 已由 `FC-17` 覆盖：匹配分析与评估采用规则版本化、共享核心评估框架与规则推荐优先 | 否 | `OPEN_QUESTIONS.md`、`DESIGN_DECISIONS.md`、评分 / 复盘 / 导出 / DoD 事实源 |
| OQ-013 | 打磨主题推荐是规则、LLM 还是混合 | confirmed | M07 / M09 | 已由 `FC-13`、`FC-17` 覆盖：打磨主题以规则推荐优先，可结合岗位与薄弱项自动推荐 | 否 | `OPEN_QUESTIONS.md`、对象模型 / RAG / 多轮 / 后端边界事实源 |
| OQ-014 | 模拟面试、打磨模式和复盘是否共用同一评估口径 | confirmed | M07 / M08 / M09 | 已由 `FC-17` 覆盖：共享核心评估框架，具体模式按 W13 评分事实源承接 | 否 | `OPEN_QUESTIONS.md`、评分 / 复盘 / 导出 / DoD 事实源 |
