...
type: note
permalink: ai-for-interviewer/docs/modules/m05-assets-and-retrieval/module-open-questions
---
...
| OQ-009 | Embedding 与向量化来源如何确定 | historical | `MODULE_REQUIREMENTS.md`、`MODULE_TASK_INDEX.md`、`ST05_03` | 已由 Workbench MVP 当前设计输入和 `FC-05` / `DD-021` 覆盖：RAG / 知识库进入一期，支持混合检索，失败时降级继续并标注证据缺口 | 否 | 已在本模块文档改为历史引用 |
| OQ-010 | 归档粒度是整份资产、片段还是题目级 | historical | `MODULE_REQUIREMENTS.md`、`MODULE_TASK_INDEX.md`、`ST05_02` | 已由 Workbench MVP 当前设计输入和 `FC-14` / `DD-027` 覆盖：一期支持整份和单题归档到资产库，归档时选择资产类型，类型带 schema 时动态渲染字段表单 | 否 | 已在本模块文档改为历史引用 |
