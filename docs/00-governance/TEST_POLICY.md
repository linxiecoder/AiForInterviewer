...
- 测试必须使用 `ManagedTempArtifacts` / `ManagedTempArtifactsTestCase` 管理临时目录。
- 测试不得直接调用 `tempfile.gettempdir()`、`tempfile.mkdtemp()`、`TemporaryDirectory()`、`NamedTemporaryFile()`、`mkstemp()` 或 `TemporaryFile()`。
- 测试不得直接使用 `tmp_path` / `tmp_path_factory`、`pytester` 或 `runpytest` 类夹具。
...
- `tests/test_temp_artifact_policy.py` 负责扫描测试代码中的禁止临时目录模式，并验证本文档仍描述受管策略。
