...
| F4 技术设计草案 | `docs/02-design/TECH_DESIGN.md` | AIFI-ARCH-001 当前产物；阶段 F4；状态 DRAFT；类型 active technical design draft；基于 `docs/01-product/PRD.md`、`docs/02-design/UX_SPEC.md` 和 `docs/02-design/UI_DESIGN_SYSTEM.md` 冻结技术架构边界；并承接 G-003 / G-004 / Composition Layer 的架构边界；不替代 `DATA_MODEL.md`、`SECURITY_PRIVACY.md`、`API_SPEC.md` 或 `PROMPT_SPEC.md` |
...
| `docs/03-implementation/` | active | 当前 `DELIVERY_PLAN.md`、`BACKLOG.md` 是阶段与任务入口；`F5_BACKEND_IMPLEMENTATION_NOTES.md` 和 `agent-io-shared-abstractions-summary.md` 是既有实现记录；不再承载已去层的 capability、spec 或 planning standalone 容器 |
