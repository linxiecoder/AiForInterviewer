...
| P2-GAP-CTX-002 | `CTX-002` SourceSupportSummary | `open` | `TEST_RESULT` | W1 added `SourceSupportSummary`, allowed levels, validation, serialization and pack compatibility. A shared context-level builder/service for Question/Feedback is not yet implemented. | Add required fields: level, primary/adjacent/job_gap refs, missing_context, reason_codes, confidence, policy_version, computed_at. | P2-W1, P2-W2 | High | W1 contract closed; W2 must centralize deterministic summary construction across loaded evidence refs. |
