...
| P2-GAP-CTX-003 | Source support single interpretation | `open` | `TEST_RESULT` | W3 removed the Question caller dependency on `question_generation_service._source_support_level()` and routes `EvidenceScope.source_support_level` through `SourceSupportSummaryService`; Feedback routing remains W4. | Question/Feedback consume summary from unified context entry; legacy level remains derived only. | P2-W2, P2-W3, P2-W4 | High | W3 covered Question with `tests/api/test_polish_interview_context_question.py`; keep open until W4 proves Feedback routing. |
| P2-GAP-CTX-004 | Interview Context | `open` | `TEST_RESULT` | W3 added `InterviewContextBuilder.build_question_context()` under `application/polish/context` and routes `create_question_task()` through it before graph/provider context snapshots; Feedback context remains W4. | Add context-level unified entry or compatibility adapter without API/provider behavior changes. | P2-W2, P2-W3, P2-W4 | Medium | Keep service pure and feed already-loaded app data only; close only after W4 extends the same entry to Feedback. |
...
| P2-GAP-TEST-003 | Question/Feedback context tests missing | `open` | `TEST_RESULT` | W3 added `tests/api/test_polish_interview_context_question.py`; W4 feedback context and expected-point tests still remain. | Prove real callers consume unified context entry. | P2-W3, P2-W4 | High | Use focused application/service-level tests without provider rewrite. |
