...
3. Execution Executor：统一执行入口，按 domain handler 消费 snapshot 并持久化结果。
...
| graph/fallback | adapter only；只返回可用性、调用结果或 adapter status |

任何非 backend authority 的输出都不能直接生成执行授权、执行目标或持久化写入。所有执行必须先得到 authority 输出，再冻结为 snapshot，再交给 executor。

## 1.1 Human Decisions And Breaking Change Policy

本轮重构按单人本地使用场景处理，不以长期兼容、分布式可靠性或 durable idempotency 为优先目标。

本轮接受破坏式清理：

- 旧 direct path 默认删除，不作为长期 fallback 保留。
- 旧 API compat fields / mirror payload 默认删除，不做长期双写。
- 旧 contract tests 默认按新契约重写，不保留旧断言作为兼容保护。
- `decision_ref` 只作为决策追踪引用，不承担完整 durable idempotency / running task lifecycle。
- 如某个旧路径无法立刻删除，必须显式登记为 temporary exception，并写明删除条件。
- ADR-0005 addendum 在重构完成后回写最终决策；实施前只记录 expected ADR delta。
...
| Execution Executor | 统一入口；按 domain handler 消费 snapshot 并执行业务动作 | persisted result、safe response | 决策、重建 context、选择另一条执行路径、承载所有 domain 细节 |
...
### 2.3 Decision Reference Boundary

`decision_ref` 是本次 authority decision 的追踪引用，用于日志、结果关联和调试排查。本轮不要求实现 durable idempotency、running task recovery、resume / cancel / deadline lifecycle。

重复提交在本轮只做 best-effort guard 或测试层约束；不作为 durable runtime contract。后续如果要把 `decision_ref` 绑定到完整幂等记录、running task 或恢复语义，必须另行回写 ADR-0005 addendum / active design docs。

## 3. Authority Layer

Execution Authority 是唯一决策层，按 domain 拆分。Progress 只有在真实改变 canonical progress state 时才进入 authority；普通 projection refresh 只走派生读模型策略和 handler，不被提升为与 question / feedback 同级的业务 execution authority。

| Component | Domain | Allowed targets |
| --- | --- | --- |
...
| `ProgressCanonicalAuthority` | 真实改变会话进度或题目完成状态 | `update_progress_canonical` |
| `ProgressProjectionPolicy` | 派生读模型或展示状态刷新 | `refresh_progress_projection` |
...
- graph/fallback availability 或 adapter status。
...
- `ProgressCanonicalAuthority` 只处理真实改变会话进度、题目完成状态或 canonical progress state 的 intent。
- canonical progress write 必须校验其依附于已完成的 backend execution result。
- `ProgressProjectionPolicy` 只处理派生读模型或展示状态刷新，不产生新的业务授权。
- projection refresh 不得改写 question / answer / feedback canonical state。
- canonical write 输出 `execution_target=update_progress_canonical`。
- projection refresh 输出 `execution_target=refresh_progress_projection`，但其语义是派生更新，不等同于 question / feedback execution。
...
| `decision_ref` | 决策引用，贯穿日志、结果关联和调试排查 |
...
- Snapshot 必须能支持确定性检查和调试复盘；本轮不要求实现 durable idempotency replay。
...
Execution Executor 是统一执行入口，不是单体大执行器。Executor 只接收 immutable snapshot，不接收 frontend intent、authority 输入或 mutable context。

Executor 只负责边界校验和分派，实际执行由 domain handler 完成：

- `QuestionExecutionHandler`
- `FeedbackExecutionHandler`
- `ProgressCanonicalHandler`
- `ProgressProjectionHandler`

handler 之间不得共享隐式授权，不得互相调用形成隐藏执行路径。
...
| `update_progress_canonical` | 基于 backend execution result 更新 canonical progress state |
| `refresh_progress_projection` | 基于 source refs 刷新派生 progress projection，不产生新的业务授权 |
...
- 本轮不要求同一 `decision_ref` 的重复执行具备 durable no-op 语义；重复提交只做 best-effort guard。
...
Canonical progress write 路径：

```text
backend canonical progress intent
→ ProgressCanonicalAuthority
→ ExecutionSnapshot(execution_target=update_progress_canonical)
→ ExecutionExecutor
→ ProgressCanonicalHandler
→ persist canonical progress result
```

Projection refresh 路径：

```text
backend projection refresh request
→ ProgressProjectionPolicy
→ ProgressProjectionHandler
→ persist or return derived projection state
```

拒绝路径：

```text
backend progress intent
→ ProgressCanonicalAuthority / ProgressProjectionPolicy(rejected, reason_codes)
→ safe rejection response
```
...
- 为 `QuestionAuthority`、`FeedbackAuthority`、`ProgressCanonicalAuthority` 建立统一 `AuthorityDecisionResult`。
- `ProgressProjectionPolicy` 输出必须能被 handler 安全消费，但不被升级为完整业务 authority。
...
- 所有 question / feedback / canonical progress write 统一进入 `ExecutionExecutor`。
- projection refresh 进入 `ProgressProjectionHandler`，不得产生新的业务授权。
...
- `decision_ref` 能关联 authority decision 与持久化结果，但不被声明为 durable idempotency contract。

## 8. Hard Rules

1. 本轮主执行路径只允许三层结构：Execution Authority、Execution Snapshot、Execution Executor。
2. 所有执行必须使用 `intent → authority → snapshot → executor → persist result`。
3. Authority 是唯一决策层，且必须 backend-only。
4. Authority 必须按 domain 分为 `QuestionAuthority`、`FeedbackAuthority`、`ProgressCanonicalAuthority`；projection refresh 不得伪装为完整业务 authority。
...
19. Persist result 必须绑定 `decision_ref` 和 `execution_target`，但 `decision_ref` 不声明 durable idempotency。
20. 任何绕过 authority、snapshot 或 executor 的旧执行路径默认删除；只有 temporary exception 可以短期隔离，且必须写明删除条件。
21. 旧 API compat fields / mirror payload 默认删除，不做长期双写。
22. 旧 direct path / legacy mapper / fallback success 测试默认废弃，按新契约重写。
