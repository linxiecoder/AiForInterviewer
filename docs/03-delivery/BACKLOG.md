...
| AIFI-UX-001 | F2 | M2 | MUST | 准备 F2 低保真设计输入 | 已创建 `docs/02-design/UX_SPEC.md`；基于 PRD 的用户故事、核心业务数据流、功能逻辑、输入 / 输出、状态异常和验收标准完成低保真 UX 规范；F2.7 已按最新产品交互口径修正简历、岗位、模拟面试、面试复盘、薄弱项场景，覆盖简历 Markdown 编辑、岗位手动录入表单、岗位详情模块化展示、模拟面试列表与详情、面试报告无一级入口、面试复盘列表与新增复盘、薄弱项多选发起打磨模式、资产库、回流确认和低置信度校对；每个核心功能都具备布局、流程、交互、状态和 Figma 交付要求，可直接指导 Figma 低保真；未新增页面代号体系，未设计精确通过概率展示 | `docs/02-design/UX_SPEC.md` | F1 完成 | DONE |
| AIFI-UX-002 | F2 | M2 | MUST | 关闭 F2 UX-blocking UNKNOWN | 已在 `docs/02-design/UX_SPEC.md` 中关闭所有 `F2_UX_BLOCKING` 和必须在 F2 确认的 `HUMAN_CONFIRMATION`；F2.7 后低保真交互不再依赖分散表格，功能场景能直接指导 Figma；覆盖多简历多岗位绑定、岗位状态、岗位手动录入表单、打磨主题完成、沉淀确认、低置信度校对、资产更新确认、缺失增强输入提示、压力面节奏、进展树展示层级、报告复盘展示、报告复制反馈和回流确认；OQ-F1-006 已在 F1 关闭，F2 只设计岗位手动录入表单；OQ-F1-035 已在回流确认功能场景中完整关闭，回流目标覆盖资产库、薄弱项、训练建议、打磨模式输入、压力面模式输入和下一次模拟面试输入，目标级状态可测试，且明确不得自动静默写入；OQ-F1-040 保留给 F4/F7；未新增页面代号体系 | `docs/02-design/UX_SPEC.md` | AIFI-UX-001、AIFI-PROD-010 | DONE |
