...
| AIFI-UX-001 | F2 | M2 | MUST | 准备 F2 低保真设计输入 | 已创建 `docs/02-design/UX_SPEC.md`；基于 PRD 的用户故事、核心业务数据流、功能逻辑、输入 / 输出、状态异常和验收标准完成低保真 UX 规范，覆盖简历、简历详情中的项目经历模块、岗位 / JD、岗位绑定简历、岗位匹配分析、资产库、打磨模式多主题、压力面模式、进展树、面试报告、报告内容复制、模拟面试复盘、真实面试复盘、薄弱项、训练建议和反馈回流；F2.3 已在原结构中修复 UX_SPEC 审计 MAJOR，补齐 Figma 低保真交付清单中的关键抽屉和状态，统一列表、抽屉、操作列、气泡、复制、回流确认规则，可支撑 Figma 低保真；已包含全局布局、抽屉交互、表格筛选 / 排序 / 分页、气泡提示、页面级低保真规格和关键交互矩阵；未新增页面代号体系，未设计精确通过概率展示 | `docs/02-design/UX_SPEC.md` | F1 完成 | DONE |
| AIFI-UX-002 | F2 | M2 | MUST | 关闭 F2 UX-blocking UNKNOWN | 已在 `docs/02-design/UX_SPEC.md` 的统一结构中关闭所有 `F2_UX_BLOCKING` 和必须在 F2 确认的 `HUMAN_CONFIRMATION`；覆盖多简历多岗位绑定、岗位状态、导入格式、打磨主题完成、沉淀确认、低置信度校对、资产更新确认、缺失增强输入提示、压力面节奏、进展树展示层级、报告复盘展示、报告复制反馈和回流确认；OQ-F1-035 已形成回流确认抽屉、完整回流目标和可测试的目标级状态，覆盖资产库、薄弱项、训练建议、打磨模式输入、压力面模式输入和下一次模拟面试输入，且明确不得自动静默写入；OQ-F1-040 保留给 F4/F7；未新增页面代号体系 | `docs/02-design/UX_SPEC.md` | AIFI-UX-001、AIFI-PROD-010 | DONE |
