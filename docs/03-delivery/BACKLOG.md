...
| AIFI-UX-002 | F2 | M2 | MUST | 关闭 F2 UX-blocking UNKNOWN | 基于 PRD §10 关闭所有 `F2_UX_BLOCKING` 和必须在 F2 确认的 `HUMAN_CONFIRMATION`，覆盖多简历多岗位绑定、岗位状态、导入格式、项目打磨结束、沉淀确认、低置信度校对、资产更新确认、缺失增强输入提示、压力面节奏、进展树展示层级、报告复盘展示和回流确认 | `docs/02-design/UX_SPEC.md` | AIFI-UX-001、AIFI-PROD-010 | NOT_STARTED |
...
| AIFI-QA-002 | F7 | M7 | MUST | 验证 UNKNOWN 关闭状态 | 在测试计划和测试报告中确认 PRD §10 中所有影响验收的 UNKNOWN 已关闭、已转为明确测试断言或已按产品原因排除出 MVP 阻塞范围 | `docs/03-delivery/TEST_PLAN.md`、测试报告 | AIFI-QA-001、AIFI-UX-002、AIFI-ARCH-002 | NOT_STARTED |
