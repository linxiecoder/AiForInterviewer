...
| AIFI-UX-001 | F2 | M2 | MUST | 准备 F2 低保真设计输入 | 基于 PRD 的用户故事、核心业务数据流、功能逻辑、输入 / 输出、状态异常和验收标准编写低保真 UX 规范，覆盖简历、简历详情中的项目经历模块、岗位 / JD、岗位绑定简历、岗位匹配分析、资产库、打磨模式多主题、压力面模式、进展树、面试报告、报告内容复制、模拟面试复盘、真实面试复盘、薄弱项、训练建议和反馈回流；不得设计精确通过概率展示 | `docs/02-design/UX_SPEC.md` | F1 完成 | NOT_STARTED |
| AIFI-UX-002 | F2 | M2 | MUST | 关闭 F2 UX-blocking UNKNOWN | 基于 PRD §10 关闭所有 `F2_UX_BLOCKING` 和必须在 F2 确认的 `HUMAN_CONFIRMATION`，覆盖多简历多岗位绑定、岗位状态、导入格式、打磨主题完成、沉淀确认、低置信度校对、资产更新确认、缺失增强输入提示、压力面节奏、进展树展示层级、报告复盘展示、报告复制反馈和回流确认 | `docs/02-design/UX_SPEC.md` | AIFI-UX-001、AIFI-PROD-010 | NOT_STARTED |
