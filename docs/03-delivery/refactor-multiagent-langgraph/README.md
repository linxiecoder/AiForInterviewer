...
PR1.6 已新增 `19_AI_PRODUCT_PROMPT_SKILL_PRESSURE_REMEDIATION_PLAN.md` 作为 blocker identification / remediation planning 文档。该文档不替代 active docs；它冻结 PR2 blocked 状态。PR2 code implementation 只有在 `BACKLOG.md` 中 `AIFI-BE-004`、`AIFI-PROMPT-002`、`AIFI-ARCH-007`、`AIFI-ARCH-008` 和 `AIFI-BE-005` 关闭，或主 Agent 显式接受风险后，才可获得新授权。
...
PR1.5 曾定义 implementation-ready 判定标准；PR1.6 findings 已覆盖该结论，因此 PR2 不能仅凭 PR1.5 进入代码实现。PR1.5 的判定标准只能作为后续 remediation 的历史输入：
...
## 3.3 PR1.6 blocker freeze

PR1.6 冻结以下后续设计修复任务为 PR2 blocker：

| Backlog task | 修复主题 | PR2 影响 |
|---|---|---|
| `AIFI-BE-004` | Pressure Mode mode-level spec | Pressure graph 不得进入 PR2；Pressure runtime spec 未关闭前不得进入业务 graph PR |
| `AIFI-PROMPT-002` | Prompt Asset / Prompt Evaluation 设计 | PR5-PR8 runtime prompt migration 不得仅依赖 Python builder 或 fake transport |
| `AIFI-ARCH-007` | Skill / Capability Model 设计 | 未登记 active Skill Model 前，任何 graph 不得生成 `Skill*` formal object 或把 `Weakness` / `ScoreDimension` 当 Skill |
| `AIFI-ARCH-008` | AI/Core 目录边界与 ADR 更新 | PR2 不得在目录命名分裂状态下创建业务 graph 或扩大 LangGraph import 边界 |
| `AIFI-BE-005` | PR2 Runtime Data Model preflight readiness gate | PR2 code implementation 只有在前述 blocker 关闭或人工接受风险后，才能获得新授权 |
...
| 19 | `19_AI_PRODUCT_PROMPT_SKILL_PRESSURE_REMEDIATION_PLAN.md` | 登记 PR1.6 findings、PR2 blocker、Pressure / Prompt Asset / Skill Model / Directory Boundary 修复计划 |

推荐阅读顺序：README -> 00 -> 01 -> 02 -> 03 -> 04 -> 10 -> 11 -> 06-09 -> 05 -> 12 -> 13-15 -> 16 -> 17 -> 18 -> 19。
...
| PR | 使用方式 |
|---|---|
| PR2 | 当前 blocked。只能做 repo-state preflight、只读 recon 和受权文档回写；`AIFI-BE-004`、`AIFI-PROMPT-002`、`AIFI-ARCH-007`、`AIFI-ARCH-008`、`AIFI-BE-005` 关闭或人工接受风险后，才可获得新的 runtime foundation 代码授权 |
...
| 检查项 | 通过标准 | 未通过处理 |
|---|---|---|
| PR1.6 blockers | `AIFI-BE-004`、`AIFI-PROMPT-002`、`AIFI-ARCH-007`、`AIFI-ARCH-008`、`AIFI-BE-005` 已关闭，或主 Agent 显式接受风险 | PR2 保持 blocked；只能 preflight / recon / docs 回写 |
