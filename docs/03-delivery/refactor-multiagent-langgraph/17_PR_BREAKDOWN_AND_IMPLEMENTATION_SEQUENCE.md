...
PR1 只创建文档登记与设计包骨架。PR1.5 已补 implementation-ready spec，但 PR1.6 findings 已覆盖“PR2 可启动”结论。PR2-PR8 均未启动；任何实现必须先进入受权 AIFI 任务或明确 PR scope。AIFI-BE-004 已由 `docs/02-design/PRESSURE_MODE_SPEC.md` 接受并关闭 Pressure Mode blocker；AIFI-ARCH-007 已由 `docs/02-design/SKILL_MODEL_SPEC.md` 接受并关闭 Skill Model blocker；AIFI-PROMPT-002 已由 `docs/02-design/PROMPT_ASSET_SPEC.md` 和 `docs/02-design/PROMPT_EVALUATION_SPEC.md` 接受并关闭 Prompt Asset / Evaluation 设计 blocker；AIFI-ARCH-008 已在 `BACKLOG.md` 同步为 `ACCEPTED` 并关闭 AI/Core directory blocker；AIFI-BE-005 / AIFI-BE-006 将 PR2 code implementation 更新为 `CONDITIONAL GO`，仅允许按 `20_PR2_PREFLIGHT_READINESS_REPORT.md` exact scope lock 实施 inert runtime data model / repository / backend tests。
...
| PR1.6 findings 登记与 blocker freeze | 登记 AI 产品设计、Prompt Asset / Evaluation、Skill / Capability Model、Pressure Mode 和 AI/Core directory findings | 把 PR2 code implementation 重新冻结为 blocked | docs only | `BACKLOG.md`, `DOCS_INDEX.md`, `README.md`, `17_PR_BREAKDOWN_AND_IMPLEMENTATION_SEQUENCE.md`, ADR-0005 as needed | `apps/**`, `tests/**`, dependencies, migrations, CI, real LLM | doc checks | git status/stat/check + doc governor | revert docs-only PR1.6 diff | AIFI-BE-004 已由 `PRESSURE_MODE_SPEC.md` 接受；AIFI-ARCH-007 已由 `SKILL_MODEL_SPEC.md` 接受；AIFI-PROMPT-002 已由 `PROMPT_ASSET_SPEC.md` / `PROMPT_EVALUATION_SPEC.md` 接受；AIFI-BE-005 等剩余 blocker 未关闭时 PR2 blocked statement 清晰 |
| PR2 AI Runtime 基础模型 | `CONDITIONAL GO`；只能在 `20_PR2_PREFLIGHT_READINESS_REPORT.md` exact scope lock 内实现 runtime data model、repository、schema/bootstrap/redaction/idempotency tests | Core tables / runtime tables / checkpoint tables 分离；runtime default-off | backend models/repos/tests only | `apps/api/app/infrastructure/db/models/ai_runtime.py`; `apps/api/app/infrastructure/db/repositories/ai_runtime/**`; `tests/api/test_model_imports.py`; `tests/api/test_db_schema_bootstrap.py`; `tests/api/test_agent_run_repository.py`; `tests/api/test_agent_interrupt_repository.py`; `tests/api/test_llm_call_repository.py`; `tests/api/test_sensitive_payload_redaction.py`; `tests/api/test_agent_side_effect_idempotency.py`; `tests/api/test_agent_replay_resume_policy.py`; `tests/api/test_architecture_boundaries.py` PR2 boundary assertions only | `apps/api/app/application/**`; `apps/api/app/infrastructure/ai_runtime/langgraph/**`; `apps/api/app/application/ai_runtime/graphs/**`; frontend; dependencies; migrations; CI; real provider; business graph; facade / runner / adapter / runtime flag enablement; LangGraph concrete graph migration | repository/schema/bootstrap/redaction/idempotency/import-boundary tests from `13_TEST_PLAN_BACKEND.md` and PR2 scope lock | pytest subset + forbidden LangGraph / real-provider import scan + `git diff --check` + doc governor for governance diffs | no runtime enablement rollback because PR2 remains inert; rollback code diff by reverting PR2 models/repos/tests | AIFI-ARCH-007 / AIFI-PROMPT-002 / AIFI-BE-004 / AIFI-ARCH-008 accepted；ADR-0005 remains Proposed with PR2-only accepted risk；source-of-truth backfill deferred before PR3/PR4 runtime exposure；raw-off、owner、retention、checkpoint ref、idempotency replay 通过；不得新增架构设计事实 |
...
| Gate | 关闭标准 | 验证 |
|---|---|---|
| G0 PR1.6 blockers closed or risk accepted | `AIFI-BE-004` Pressure Mode 已由 `PRESSURE_MODE_SPEC.md` 接受；`AIFI-ARCH-007` Skill Model 已由 `SKILL_MODEL_SPEC.md` 接受；`AIFI-PROMPT-002` Prompt Asset / Evaluation 已由 `PROMPT_ASSET_SPEC.md` / `PROMPT_EVALUATION_SPEC.md` 接受；`AIFI-ARCH-008` 已由 `BACKLOG.md` 同步为 `ACCEPTED`；`AIFI-BE-005` / `AIFI-BE-006` 已登记 PR2 `CONDITIONAL GO` 与 exact scope lock | `BACKLOG.md` 状态、PR1.6 remediation evidence、accepted risk 记录、`PRESSURE_MODE_SPEC.md`、`SKILL_MODEL_SPEC.md`、`PROMPT_ASSET_SPEC.md`、`PROMPT_EVALUATION_SPEC.md`、`20_PR2_PREFLIGHT_READINESS_REPORT.md` |
...
| G6 feature flag frozen | PR2 default-off 行为已冻结为禁止启用：PR2 不创建 / 修改 runtime enablement flag、per-graph flag、real-provider gate 或 `runtime_flags.py`，不执行 graph，不调用真实 provider；runtime flag implementation 延后到 PR3 scope | ADR-0005 `PR2 Default-Off Rule`；`20_PR2_PREFLIGHT_READINESS_REPORT.md` exact scope lock；forbidden import/provider scan |
| G7 active docs sync decision | PR2 runtime foundation 接受有限风险：PR2 可依赖专题包 + ADR + readiness report 实施 inert models/repos/tests；`DATA_MODEL.md` / `PERSISTENCE_MODEL.md` / `SECURITY_PRIVACY.md` backfill 不进入 PR2 code scope，必须在 PR3 / PR4 runtime exposure 前回写或重新登记 accepted risk | ADR-0005 follow-up table；`20_PR2_PREFLIGHT_READINESS_REPORT.md` source-of-truth closure |

若 PR2 实现窗口不能完整采用 `20_PR2_PREFLIGHT_READINESS_REPORT.md` exact scope lock，PR2 仍只能做只读 recon、repo-state preflight 或受权文档修复，不能修改 `apps/**`、`tests/**`、依赖、migration，也不能调用真实 LLM。
...
- PR 顺序清晰且可分批回滚。
- PR2 启动门槛清晰；AIFI-BE-004 只关闭 Pressure Mode blocker，AIFI-ARCH-007 只关闭 Skill Model blocker，AIFI-PROMPT-002 只关闭 Prompt Asset / Evaluation blocker，AIFI-ARCH-008 只关闭 AI/Core directory blocker；PR2 code implementation 只能按 `CONDITIONAL GO` exact scope lock 启动。
