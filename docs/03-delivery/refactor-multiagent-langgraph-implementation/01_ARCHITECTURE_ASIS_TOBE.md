...
当前 AI 能力以直接 application use case、LLM transport、local blueprint / grounding validators 和显式 failure states 为主。Polish 是当前唯一具备 first real business graph migration 成熟度的路径，但 Phase 2B 后 feedback / candidate generation 已收敛为 reserved placeholder；当前成熟路径主要是 question / progress tree 与 reserved feedback API shape。Job Match / ResumeAnalysis、Pressure、Report、Review、Asset、Weakness、Training 的 Agent capability 尚不足以直接进入完整业务 graph migration；它们可以先保留 backend stubs、descriptors、DTOs 或 graph placeholders，待对应 dedicated migration PR 再迁移。
...
flowchart TB
  ...
  Polish --> FeedbackSvc["Reserved feedback placeholder"]
...
| Polish question | `PolishUseCases.create_question_task`; `question_generation_service.py`; `question_blueprint.py`; `question_grounding.py` | 已实现 direct application feature | 本地生成 surface question：resolve progress node -> build `EvidenceScope` -> build `QuestionBlueprint` -> validate grounding | `questions`; `ai_tasks`; question metadata, sources, evidence refs；grounding failed 时不写 question | `test_polish_api.py`; `test_polish_question_refactor_phase1.py`; question graph integration / persistence handoff tests | 没有 AgentRun / NodeRun；task 当前同步完成为 succeeded 或 validation_failed；graph interrupt/resume 缺失 |
| Polish feedback and candidates | `PolishUseCases.create_feedback_task`; `feedback_reserved.py`; `polish_candidate` model/repository | feedback API shape 已保留；candidate confirmation repository 保留 | None；当前只返回 reserved placeholder | `feedback`; `ai_tasks`; reserved payload；candidate refs 为空 | `test_polish_api.py`; `test_polish_candidates.py`; sanitizer tests | Feedback generation 与 candidate generation 当前未启用；future graph migration 需重新授权 |
| Polish progress tree | `progress_tree.py`; `progress_v2_prompts.py`; fake transport progress task types | 已实现 planning/runtime helper | 多个 progress-tree task type 通过 transport / fake fallback 执行 | `polish_session_details` progress plan/state JSON | `test_polish_api.py` progress tree and prompt governance tests | ProgressTree 不是 Skill source of truth；graph state/checkpoint boundary 未实现 |
...
flowchart TB
  ...
  PolishFeedbackUc --> FeedbackReserved["build_reserved_feedback_artifacts"]
  FeedbackReserved --> FeedbackWrite["写入 reserved feedback 和 ai_tasks；candidate refs 为空"]
...
| Polish question | `PolishUseCases.create_question_task` -> `QuestionGenerationService.generate` | question metadata 包含 builder、blueprint、surface prompt、grounding metadata 和 evidence refs；grounding passed 后 task 存为 succeeded | progress node 缺失或 grounding failure 返回 validation_failed task；不修复、不生成、不持久化 question |
| Polish feedback | `PolishUseCases.create_feedback_task` -> `build_reserved_feedback_artifacts` | reserved payload metadata 明确 `llm_called=false`、`candidate_extraction_called=false`；candidate refs 为空 | 无 provider 调用；返回 succeeded reserved placeholder；不自动创建 formal objects |
| Progress tree | `PolishProgressTreeQualityFirstPlanner` 通过 `polish_progress_quality_first_menu` 单次生成；该 task_type 是唯一 active initial generator | progress plan/state 持久化到 polish session detail | insufficient context / failed artifacts 使用显式状态表达；无替代 initial generator path |
...
| Polish feedback | `PolishUseCases.create_feedback_task` + `feedback_reserved.py` | Polish Feedback Agent | `polish_feedback_graph` | AI Business Graph Plane | answer/question/session refs | reserved feedback ref、empty candidate refs | Core 只写 reserved feedback；不写 score、loss points、candidate objects | P0 reserved guardrail | feedback graph migration deferred until re-authorized | direct reserved path retained | Placeholder only；full generation maturity must be re-established |
...
| Polish question local path | `QuestionGenerationService.generate` | N/A; no provider call in Phase 2A question generation | `AiTask`、session/progress/evidence refs、contract ids | no `llm_calls`; validation/task trace only | blueprint、surface prompt version、grounding metadata、evidence refs、question ref | grounding failure -> validation_failed；no business write | Phase 2A fact；PR5 adoption must preserve no-provider boundary |
| Polish feedback reserved path | `build_reserved_feedback_artifacts` | N/A; no provider call in Phase 2B feedback flow | `AiTask`、answer/question/session refs、reserved schema id | no `llm_calls`; validation/task trace only | reserved metadata、feedback ref、empty candidate refs | no provider failure path；business validation still fail-closed before write | Phase 2B fact；feedback graph migration deferred |
| Candidate generation | N/A in Phase 2B feedback flow | N/A | N/A | no `llm_calls` | candidate refs remain empty | no partial candidate generation path | Deferred until explicit authorization |
