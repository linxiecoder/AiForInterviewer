...
当前 AI 能力以直接 application use case、LLM transport、local blueprint / grounding validators 和显式 failure states 为主。Polish 是当前唯一具备 first real business graph migration 成熟度的路径；feedback realpath 已接通 `create_feedback_task` -> `FeedbackGenerationService.generate()`，generated feedback 可持久化并读回，provider / validation failure 不返回 reserved success。当前成熟路径包括 question / progress tree、feedback generated direct API shape、`CanonicalEvidencePack.canonical_project_assets` 在 question / feedback context 中的集成、`asset_confirmed` active canonical evidence、`asset_archived` historical/reference-only unless explicitly restored、Phase 4 feedback policy / validation rules，以及 Phase 6A question hardening（`source_support_level` 四态、compact provider request、blocking / warning grounding gate）。Candidate extraction、正式 Weakness / Asset / Training 写回、Phase 5 Fake cleanup、Phase 6B follow-up coverage matrix 和 real provider E2E 仍未执行或未验证。Job Match / ResumeAnalysis、Pressure、Report、Review、Asset、Weakness、Training 的 Agent capability 尚不足以直接进入完整业务 graph migration；它们可以先保留 backend stubs、descriptors、DTOs 或 graph placeholders，待对应 dedicated migration PR 再迁移。
...
flowchart TB
  ...
  Polish --> QuestionSvc["QuestionGenerationService"]
  Polish --> FeedbackSvc["FeedbackGenerationService real path"]
...
| Polish question | `PolishUseCases.create_question_task`; `question_generation_service.py`; `question_blueprint.py`; `question_grounding.py` | 已实现 direct/provider-capable application feature | resolve progress node -> build `EvidenceScope` / `CanonicalEvidencePack` -> compute `source_support_level` four-state -> build compact provider request / `QuestionBlueprint` -> apply blocking/warning grounding gate | `questions`; `ai_tasks`; question metadata, sources, evidence refs, `source_support_level`, canonical project asset refs；blocking grounding failed 时不写正常 question | `test_polish_api.py`; `test_polish_question_refactor_phase1.py`; question graph integration / persistence handoff tests | 没有 AgentRun / NodeRun；task 当前同步完成为 succeeded 或 validation_failed；graph interrupt/resume 缺失；Phase 6B follow-up coverage matrix 未执行 |
| Polish feedback and candidates | `PolishUseCases.create_feedback_task`; `FeedbackGenerationService.generate`; `feedback_reserved.py` legacy compatibility；`polish_candidate` model/repository | feedback real path 已接通；`CanonicalEvidencePack.canonical_project_assets` 已进入 context；Phase 4 feedback rules 由 backend policy / validation 实现；candidate confirmation repository 保留 | feedback generation 通过 `FeedbackGenerationService.generate()` 调用 LLM transport；legacy reserved helper 不作为 success path；`asset_confirmed` 才是 active canonical evidence，`asset_archived` 只作 historical/reference-only unless explicitly restored | `feedback`; `ai_tasks`; generated payload；`asset_consistency_check`、`answer_coverage`、`answer_change_analysis`、`feedback_cards`；candidate refs 仍不写正式对象 | local Phase 2/Phase 4 tests；`test_polish_candidates.py`; sanitizer tests | Candidate extraction、正式 Weakness / Asset / Training 写回、Phase 5 Fake cleanup、Phase 6B follow-up coverage matrix 和 real provider E2E 仍未执行或未验证 |
| Polish progress tree | `progress_tree.py`; `progress_prompts.py`; fake transport progress task types | 已实现 planning/runtime helper | 多个 progress-tree task type 通过 transport / fake fallback 执行 | `polish_session_details` progress plan/state JSON | `test_polish_api.py` progress tree and prompt governance tests | ProgressTree 不是 Skill source of truth；graph state/checkpoint boundary 未实现 |
...
flowchart TB
  ...
  PolishQuestionUc --> QuestionSvc["QuestionGenerationService.generate"]
  QuestionSvc --> QuestionBlueprint["EvidenceScope -> QuestionBlueprint -> surface question"]
  QuestionBlueprint --> QuestionGrounding["validate_question_grounding"]
  ...
  PolishFeedbackUc --> FeedbackSvc["FeedbackGenerationService.generate"]
  FeedbackSvc --> FeedbackWrite["写入 generated feedback 和 ai_tasks；失败写 generation_failed"]
...
| Polish question | `PolishUseCases.create_question_task` -> `QuestionGenerationService.generate` | question metadata 包含 builder、blueprint、compact provider request 摘要、`source_support_level`、grounding metadata、canonical asset refs 和 evidence refs；blocking grounding passed 后 task 存为 succeeded，warning grounding 写入 metadata | progress node 缺失或 blocking grounding failure 返回 validation_failed task；不修复、不生成、不持久化正常 question |
| Polish feedback | `PolishUseCases.create_feedback_task` -> `FeedbackGenerationService.generate` | generated payload metadata 明确 `generated=true`、`llm_called=true`；legacy fields 保持兼容；Phase 4 fields 由 backend policy / validation 补齐或校验 | provider unavailable / exception / validation failed 返回 `generation_failed` 或等价失败语义；不返回 reserved success；不自动创建 formal objects |
| Progress tree | `PolishProgressTreeLlmService` 通过 `polish_progress_quality_first_menu` 单次生成；该 task_type 是唯一 active initial generator | progress plan/state 持久化到 polish session detail | insufficient context / failed artifacts 使用显式状态表达；无替代 initial generator path |
...
| Polish progress tree | `PolishProgressTreeLlmService`、`polish_progress_quality_first_menu` 和 progress plan/state JSON；该 task_type 是 canonical initial generator | Polish Planning Agent | `polish_progress_tree_graph` | AI Business Graph Plane | polish session、完整 resume / job / match context、turns summary、Skill refs | progress plan/state refs、low confidence flags、deferred candidates | Core 只更新 progress state | P0 | PR5 Polish migration first | PR5 first migration target | Active initial generator 已收敛为 quality-first；无替代 initial generator path |
| Polish question | `PolishUseCases.create_question_task` + `QuestionGenerationService` | Polish Question Agent | `polish_question_graph` | AI Business Graph Plane | session、progress node、completed refs、anti-repeat refs、`CanonicalEvidencePack`、`source_support_level` | question ref、evidence refs、task result、grounding gate result | blocking grounding passed 后由 Core 写 `Question`；warning grounding 只写 metadata | P0 | PR5 Polish migration first | PR5 first migration target | Mature question code and tests；Phase 6A question hardening implemented；Phase 6B follow-up coverage matrix not executed |
...
| Polish feedback | `PolishUseCases.create_feedback_task` + `FeedbackGenerationService.generate`；`feedback_reserved.py` 仅 legacy/fallback compatibility | Polish Feedback Agent | `polish_feedback_graph` | AI Business Graph Plane | answer/question/session refs、`CanonicalEvidencePack.canonical_project_assets` compact feedback context | generated feedback ref、legacy-compatible fields、`asset_consistency_check`、`answer_coverage`、`answer_change_analysis`、`feedback_cards`；candidate refs 不写正式对象 | Core 写 generated feedback；provider / validation failure 写失败态，不写 reserved success | P0 feedback realpath guardrail | PR5 graph parity may wrap existing direct generated path; Phase 4 rules must be preserved | direct generated path retained | Feedback generation maturity re-established; Phase 4 feedback rules implemented by backend policy / validation；Phase 5 Fake cleanup, Phase 6B follow-up coverage matrix and real provider E2E not executed/verified |
...
| Weakness candidate | `WeaknessCandidate` model；no current Polish auto extraction | WeaknessAgent / Candidate Agent | `weakness_candidate_graph` placeholder only | AI Business Graph Plane + Asset Plane | feedback/score/review evidence、existing weaknesses | weakness candidates、merge suggestions | formal Weakness 前必须 confirmation interrupt | P3 | PR8 Candidate / Skill / Training closure | Deferred to PR8 closure | Candidate foundation exists；WeaknessAgent immature |
...
| Polish question generated path | `QuestionGenerationService.generate` | future `PersistedLlmTransport` wrapper when migrated | `AiTask`、session/progress/evidence refs、contract ids、`CanonicalEvidencePack`、`source_support_level` | PR2 前无 `llm_calls`；validation/task trace only；provider-facing request remains compact | blueprint、surface prompt version、compact provider request summary、grounding metadata、evidence refs、question ref | blocking grounding failure -> validation_failed；warning grounding -> metadata；no normal question on blocking failure | Phase 6A fact；PR5 adoption must preserve source-support and grounding semantics |
| Polish feedback generated path | `FeedbackGenerationService.generate` | future `PersistedLlmTransport` wrapper when migrated | `AiTask`、answer/question/session refs、generated schema id、`CanonicalEvidencePack.canonical_project_assets` compact feedback context | PR2 前无 `llm_calls`；validation/task trace only | generated metadata、feedback ref、legacy-compatible fields、Phase 4 feedback fields | provider unavailable / exception / validation failed -> `generation_failed` / `validation_failed`；no reserved success | Phase 2 + Phase 4 fact；PR5 graph parity must preserve failure semantics and feedback rule validation |
| Candidate generation | N/A in Phase 2 feedback real path | N/A | N/A | no `llm_calls` | no formal candidate write | no partial candidate generation path | Deferred until explicit authorization |
...
| Removing fallback / test doubles before graph fake tests exist | 破坏 local testing 和 provider gate |
