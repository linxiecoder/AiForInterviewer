...
本文是 LangGraph MultiAgent 实施前的 canonical 技术架构基线。它定义当前真实架构、目标双域架构、AI Runtime 三个 plane、MultiAgent 协议、LLM trace persistence、数据边界、前端运行时边界、迁移路线和后端 function package 的架构约束。
...
## 9. AI Runtime 三个 plane

AI Runtime 拆成三个 plane，使 implementation 可以分阶段评审，避免 control state、business graph logic 和 AI assets 混在一起。
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| Resume source bundle | `build_source_bundle`、resume Markdown chunks；没有 standalone graph | ResumeAnalysisAgent / Source Bundle descriptor | `resume_analysis_graph` placeholder only | AI Business Graph Plane + AI Asset Plane | `ResumeVersion`、markdown digest、section refs | source bundle refs、evidence refs、quality flags | 只给 Job Match / Polish / Report 提供 refs | P2 | PR6 trace-compatible or graph migration if still needed | PR2-PR5 descriptor / DTO / placeholder only；PR6 decision required | Direct helper exists；Agent capability immature |
| Job match analysis | `JobMatchUseCases.create` + `LlmJobMatchAnalyzer.analyze` | JobMatchAgent | `job_match_graph` placeholder only | AI Business Graph Plane | binding、resume/job versions、score rule、source bundle | `JobMatchAnalysis`、`ScoreResult(job_match)`、candidate refs | validation 后由 Core write；candidates 保持 pending | P2 | PR6 trace-compatible or graph migration if still needed | PR2-PR5 trace-compatible wrapper / placeholder only；direct path retained until at least PR7 frontend ready | Direct feature exists；完整 graph migration 不进入 immediate priority |
...
| PR5+ | 按 feature graph adoption 使用 wrapper；PR5 Polish，PR6 JobMatch / ResumeAnalysis if still needed，PR8 remaining closure |

### LLM trace persistence hook matrix

| 当前路径 | 当前调用方 | 未来包装器 | Trace Context 来源 | 调用前写入 | 成功后写入 | 失败后写入 | PR |
|---|---|---|---|---|---|---|---|
| Job Match direct analyzer | `LlmJobMatchAnalyzer.analyze` | 注入的 `LlmTransport` 外包 `PersistedLlmTransport` | Core `AiTask` + legacy `task_type=job_match_analysis`；graph migration 前没有 node ref | `llm_calls(status=running, contract_ids=P-JOBMATCH-*)` | model、usage if available、request/response hash、validation summary、evidence hash | provider failure category、fallback/retry policy、no business write | PR2 schema；PR6 trace-compatible or graph adoption if still needed |
...
| Job Match graph node | `run_job_match_analyzer` node | `PersistedLlmTransport` | `AgentRunContext` + `AgentNodeRun` | node-linked running call | node output refs and validation | node failed；side effect blocked | PR6 if still needed |
...
flowchart LR
  ...
  PR5 --> PR6["PR6 JobMatch / ResumeAnalysis trace-compatible or graph if needed"]
  PR6 --> PR7["PR7 Frontend runtime UI"]
  PR7 --> PR8["PR8 Pressure / Report / Review / Candidate / Skill / Training closure"]
...
| PR6 | JobMatch / ResumeAnalysis trace-compatible or graph migration if still needed | 只在需求仍成立时迁移 `job_match_graph` / `resume_analysis_graph`；否则仅保留 trace-compatible wrapper、descriptors、DTOs 或 graph placeholders | JobMatch / ResumeAnalysis 不作为 immediate graph priority | legacy analyzer 保持 fallback 直到 parity 或保持 direct path | existing API compatibility；score/no probability/evidence/candidate tests pass；无不必要 full graph implementation |
| PR7 | Frontend runtime UI | 在明确 frontend scope 下增加 AI task status、sanitized timeline、interrupt/resume、candidate confirmation、low confidence / validation failed banners；可选 dev/admin debug panel 需单独授权 | UI 消费 sanitized runtime status | existing UI behavior preserved | no raw fields、no normal orchestration page、polling stops terminal、copy boundary preserved |
| PR8 | Pressure / Report / Review / Candidate / Skill / Training closure | 添加 pressure/report/review/candidate/skill/training graphs 和 confirmation closure | MultiAgent 覆盖 late MVP graphs | remaining direct paths either deprecated or documented fallback | report/review/candidate/copy/privacy gates pass；no silent formal write；TrainingTask requires explicit user action |
...
| PR5 / PR6 | PR5 只迁移 Polish graph path；PR6 才评估 JobMatch / ResumeAnalysis trace-compatible wrapper 或 graph path；legacy 仍是 fallback |
...
| PR6 JobMatch / ResumeAnalysis decision | Direct path remains canonical unless PR6 explicitly proves graph still needed；in-flight direct Job Match tasks are not converted to graph runs |
| PR7 frontend ready | Job Match direct path must still be available at least until PR7 frontend ready and graph parity / rollback evidence exists |
...
| `AIFI_JOB_MATCH_GRAPH_ENABLED` | PR6 if still needed | Route new JobMatch / ResumeAnalysis tasks through graph path after parity | false |
...
| `03_JOB_MATCH_AGENT_PACKAGE.md` | Feature-to-Agent map、PR6-if-needed priority、legacy direct path policy | descriptors、DTOs、placeholder graph names；PR6 授权后才定义 graph nodes、idempotency keys、parity tests | Job Match as immediate migration target、separate architecture 或 formal Weakness write |
