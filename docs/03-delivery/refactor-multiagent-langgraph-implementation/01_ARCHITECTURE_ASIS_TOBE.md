...
| Progress tree | `PolishProgressTreeQualityFirstPlanner` 通过 `polish_progress_quality_first_menu` 单次生成；历史 V1 plan 仅 read-only 读取兼容 | progress plan/state 持久化到 polish session detail | insufficient context / failed artifacts 使用显式状态表达；四阶段 `v2_pipeline` 已删除 |
...
| Polish progress tree | `PolishProgressTreeQualityFirstPlanner`、`polish_progress_quality_first_menu` 和 progress plan/state JSON；历史 V1 plan 仅 read-only 兼容 | Polish Planning Agent | `polish_progress_tree_graph` | AI Business Graph Plane | polish session、完整 resume / job / match context、turns summary、Skill refs | progress plan/state refs、low confidence flags、deferred candidates | Core 只更新 progress state | P0 | PR5 Polish migration first | PR5 first migration target | Active initial generator 已收敛为 quality-first；四阶段 `v2_pipeline` 不再是 runtime path |
