...
| AIFI-REL-008 | F8 | M8 | MUST | Feedback rollback and degradation gate plan | 已完成 docs/evidence/checklist-only 依赖收口：登记 feedback-loop default-off、gray、rollback、restore、release gate 和未决项；不声明 release-ready | rollback/degradation gate TODO、发布门槛清单、依赖 closeout evidence | AIFI-PROD-011；AIFI-ARCH-009 / AIFI-QA-003 / AIFI-BE-009 / AIFI-FE-002（参考上下文，非硬依赖） | DONE |
...
| AIFI-QA-005 | F7 | M7 | MUST | Feedback-loop integration and real page QA | 已完成 Step11 集成与真实页面 QA：复现并修复 Step10 limited smoke failure，真实 interview/workbench 页面覆盖 generated、pending、partial、failure、refresh recovery、trusted / untrusted signed action、follow-up surface 和 next action；不进入 Step12 release 或 rollback | implementation commits `9a97458`、`bd3c861`、`8a08cb9`；Step11 implementation scope lock、真实页面 QA / enhanced QA 截图与 action log、focused tests、implementation review-work PASS；limited smoke red/green 证据；C-049 到 C-054 仍保持 Deferred / Open Question | AIFI-QA-004；AIFI-BE-010；AIFI-BE-011；AIFI-BE-012；AIFI-BE-015；AIFI-BE-013；AIFI-BE-014；AIFI-BE-017；AIFI-FE-003；AIFI-FE-004 | DONE |
| AIFI-REL-009 | F8 | M8 | MUST | Feedback-loop release gate and rollback checklist | 已完成 feedback-loop 发布门禁、回滚 checklist、QA evidence 归档和 release notes draft；文档收口不代表 production release-ready | `FEEDBACK_LOOP_RELEASE_CHECKLIST.md`、`FEEDBACK_LOOP_RELEASE_RUNBOOK.md`、`FEEDBACK_LOOP_QA_EVIDENCE.md`、`FEEDBACK_LOOP_CHANGELOG_DRAFT.md` | AIFI-QA-004；AIFI-BE-010；AIFI-BE-011；AIFI-BE-012；AIFI-BE-015；AIFI-BE-013；AIFI-BE-014；AIFI-BE-017；AIFI-FE-003；AIFI-FE-004；AIFI-QA-005；AIFI-REL-008 | DONE |
...
- 依赖：AIFI-PROD-011；AIFI-ARCH-009、AIFI-QA-003、AIFI-BE-009、AIFI-FE-002 仅作为参考上下文，非本 docs/evidence/checklist-only closeout 的硬依赖。替代证据为 AIFI-QA-004、AIFI-BE-010、AIFI-BE-011、AIFI-BE-012、AIFI-BE-015、AIFI-BE-013、AIFI-BE-014、AIFI-BE-017、AIFI-FE-003、AIFI-FE-004、AIFI-QA-005 的已完成实现 / QA closeout，以及本节登记的 release gate 待验证项。
- closeout 分类：VALID_REQUIRED_WORK；本轮只完成 active docs / evidence / checklist 级规划登记，不执行生产发布、灰度、回滚、降级、迁移、配置、依赖或 runtime 改动。
- closeout evidence：`.omo/evidence/plan/aifi-rel-008-dependency-closeout.md`；`.omo/evidence/plan/step12-active-docs-unblock.md`。
- default-off / gray / rollback / restore / release gate 口径：仅作为 Step12 release-readiness checklist 的待验证门槛输入；不得写成已具备生产开关、真实 env var、部署脚本或回滚命令。
- residual risk 分类：Step11 真实页面 QA 的 mobile DOM 文本依赖、404 console、Ant Design deprecated warning、Vite chunk size warning、inherited large file issue 和 LSP diagnostics `Transport closed` 均进入 Step12 known risks / release gate 输入；当前不把它们升级为已解决。
- C-049 到 C-054：保持 Deferred / Open Question；AIFI-REL-008 closeout 不定义相似度阈值、考察点绑定模型、失败折叠最终样式、错误枚举最终映射、刷新恢复最终状态机或下一题具体算法。
- 是否阻塞后续实现：不再阻塞 Step12 / AIFI-REL-009 implementation scope lock 的重新执行；仍不得绕过 Step12 scope lock 直接进入 release、rollback、production rollout、migration、config、dependency changes 或业务 / 测试代码。
...
#### Step 6 closeout and read-only dependency status
...
- 背景：Step8 initial scope lock 已确认 active docs 缺少唯一 canonical backend API response envelope AIFI；本任务已补齐 Step8 后端 API schema / response envelope 汇总入口，并在重新执行 Step8 implementation scope lock 后完成实现。
...
- 当前状态：DONE。Step8 implementation scope lock 已生成 `execution_mode=AUTHORIZED`、`active_aifi=AIFI-BE-017`、`implementation_allowed=true` 的证据；本任务完成后不自动授权 Step9 implementation。
...
- 历史解锁状态：Step9 initial scope lock（初始范围锁）曾因 AIFI-FE-002=NOT_STARTED 硬依赖口径被阻塞；active-docs unblock（当前有效文档解锁）已把 AIFI-FE-002 调整为参考上下文，并仅允许重新执行 Step9 implementation scope lock（实现范围锁）。scope lock 生成 `execution_mode=AUTHORIZED` 前，不得开始 implementation（实现）。
...
- 当前状态：DONE。Step9 implementation（实现）已通过 scope lock（范围锁）、RED→GREEN FE contract test（前端契约测试）、runtime contract（运行时契约）、Step2-Step8 backend canary（后端金丝雀测试）和 implementation review-work（实现审查）；implementation commit（实现提交）为 `7f5f889`。
- 完成范围：补齐 Step8 response-only metadata（响应专用元数据）的前端类型边界；新增 `FeedbackResponseContractViewModel` 投影；对 policy-signed next action（策略签名下一步动作）执行 fail-closed（失败关闭）信任判断；投影 progress（进展）、follow-up（追问）、same-node（同节点）和 next-question contract（下一题契约）；失败状态不伪装为 generated feedback（已生成反馈）；pending（等待中）状态保留 pending copy（等待文案）；legacy / missing payload（旧载荷 / 缺失载荷）保持安全输出。
- 完成证据：`.omo/evidence/plan/step9-implementation-scope-lock.md`；`.omo/evidence/plan/step9-implementation-closeout.md`；`.omo/evidence/plan/step9-view-model-runtime.txt`；`.omo/evidence/plan/step9-implementation-review-work.md`。
- 验证结果：`npm.cmd run web:test` 通过；runtime JS contract（运行时 JS 契约）断言通过；Step2-Step8 backend canary（后端金丝雀测试）为 `91 passed in 3.51s`；implementation review-work（实现审查）PASS；forbidden path diff（禁止路径差异）为空。
- 未完成且不得被误认为完成的范围：Step10 / Step11 页面交互与刷新恢复；AIFI-FE-004；AIFI-REL-009；backend（后端）行为；API schema source（API 结构来源）；migration（迁移）；release（发布）；C-049 到 C-054 关闭。
...
- docs authorization commit：`612eef81e01cd1487c54cdc5655e9c64eb2c645e`（`docs(aifi): authorize step10 workbench interaction scope`）。
- implementation commit：`8aa4d01bcf36b4929d477a76582848f7d6545d2d`（`feat(aifi): close step10 workbench interaction`）。
- scope lock 证据：`.omo/evidence/plan/step10-scope-lock.md`、`.omo/evidence/plan/step10-active-docs-unblock.md`、`.omo/evidence/plan/step10-implementation-scope-lock.md`。
- implementation review-work（实现审查）：PASS；证据为 `.omo/evidence/plan/step10-implementation-review-work.md`。
- 完成范围：`InterviewPage.tsx` 页面层消费 Step9 view model（视图模型），使右侧反馈面板选中的回答驱动 fixed next action bar（固定下一步动作条）和 feedback next-question（基于反馈生成下一题）入口；刷新恢复或点选旧回答后不再误用当前题目的最新回答。
- 完成修改路径：`apps/web/src/pages/interview/InterviewPage.tsx`；`apps/web/src/pages/interview/InterviewPage.test.ts`；`tests/web/test_interview_actions.py`。
- 验证结果：`npm.cmd run web:test` 通过；`python -m pytest -p no:cacheprovider tests/web/test_interview_actions.py -q` 结果为 `14 passed in 8.11s`；focused backend regression 结果为 `77 passed in 4.68s`；`npm.cmd run web:build` 通过并保留既有 large chunk warning；`git diff --check` 通过，仅 LF -> CRLF warning。
...
- 当前状态：DONE / CLOSED。Step11 initial scope lock 证据为 `.omo/evidence/plan/step11-scope-lock.md`，结论为 `execution_mode=BLOCKED`；active-docs unblock 后已重新执行 implementation scope lock，`.omo/evidence/plan/step11-implementation-scope-lock.md` 结论为 `execution_mode=AUTHORIZED`、`implementation_allowed=true`。
...
- 完成证据：
  - implementation commits：`9a97458`、`bd3c861`、`8a08cb9`。
  - Step11 implementation scope lock：`.omo/evidence/plan/step11-implementation-scope-lock.md`。
  - Step10 residual red evidence：`.omo/evidence/step11-polish-feedback-frontend-smoke-red`，失败点为 `failed answer should expose generation_failed payload`。
  - Step11 green smoke evidence：`.omo/evidence/step11-polish-feedback-frontend-smoke-green`，pending / generated / generation_failed 样本均通过。
  - 真实页面 QA：`.omo/evidence/step11-real-page-qa/action-log.json`，截图为 `mobile-375.png`、`tablet-768.png`、`desktop-1280.png`。
  ...
  - implementation review-work：`.omo/evidence/plan/step11-implementation-review-work.md`，API contract、backend test、real page QA、governance scope、security / privacy 均为 PASS。
  - 测试：`npm.cmd run web:test`、`python -m pytest -p no:cacheprovider tests/web/test_interview_actions.py -q`、focused backend regression（`81 passed`）、`npm.cmd run web:build`、`git diff --check` 均通过；`npm.cmd run web:build` 保留 Vite chunk size warning。
...
- final_status：DONE / CLOSED（仅 Step11 integration / real page QA closeout，不代表 Step12 release / rollback 已完成）。
...
- 当前状态：DONE / CLOSED；仅关闭 AIFI-REL-009 的 release docs / runbook / QA evidence / release notes draft 文档范围，不代表 production release-ready，不授权生产发布、灰度、回滚、降级、migration、config、dependency、runtime、业务代码或测试代码。
- implementation docs commit：`7abd932630f81c40642dbc0a6834b7a580bbf585`（`docs(aifi): close step12 release readiness docs`）。
- active docs：`docs/03-implementation/FEEDBACK_LOOP_RELEASE_CHECKLIST.md`；`docs/03-implementation/FEEDBACK_LOOP_RELEASE_RUNBOOK.md`；`docs/03-implementation/FEEDBACK_LOOP_QA_EVIDENCE.md`；`docs/03-implementation/FEEDBACK_LOOP_CHANGELOG_DRAFT.md`；已登记 `docs/00-governance/DOCS_INDEX.md`。
- 前置 unblock evidence：`.omo/evidence/plan/aifi-rel-008-dependency-closeout.md`；`.omo/evidence/plan/step12-active-docs-unblock.md`；unblock commit `b0540dba57adcc419eb45931688fea02e2520530`。
- release decision：AIFI-REL-009 文档范围 DONE；生产发布 NO-GO；灰度发布 NO-GO；回滚执行 NO-GO；降级执行 NO-GO；F8 / M8 full production release 仍受 `DELIVERY_PLAN.md` 和后续 release review 约束。
- residual risks：Step1 `ACCEPTED_RED`、mobile DOM 文本依赖、404 console、Ant Design deprecated warning、Vite chunk size warning、inherited large file issue、LSP diagnostics `Transport closed` 均保留为后续生产前复核或豁免输入，不写成 resolved。
