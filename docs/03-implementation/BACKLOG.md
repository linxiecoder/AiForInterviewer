...
| AIFI-FE-004 | F6 | M6 | MUST | Interview workbench interaction and refresh recovery | 实现面试工作台反馈交互、刷新恢复和重试/降级呈现；当前仅允许重新执行 Step10 implementation scope lock（实现范围锁），不直接授权 implementation（实现） | `InterviewPage.tsx` 行为和相关 FE tests；Step10 scope lock（范围锁）证据 | AIFI-FE-003；AIFI-BE-010；AIFI-BE-011；AIFI-BE-012；AIFI-BE-015；AIFI-BE-013；AIFI-BE-014；AIFI-QA-004 | READY_TO_START |
...
- 当前状态：READY_TO_START。Step10 initial scope lock（初始范围锁）因 AIFI-FE-004=NOT_STARTED 被阻塞；本次 active-docs unblock（当前有效文档解锁）仅允许重新执行 Step10 implementation scope lock（实现范围锁）。scope lock 生成 `execution_mode=AUTHORIZED` 前，不得开始 implementation（实现）。
- 边界：Step10 implementation scope lock（实现范围锁）若通过，只能授权 `InterviewPage.tsx` 页面层消费 Step9 view model（视图模型）、feedback 展示状态、刷新恢复、pending / partial / failure（等待 / 部分 / 失败）展示流、policy-signed next action（策略签名下一步动作）展示、follow-up action surface（追问动作展示面）和相关 FE tests（前端测试）；不得修改后端语义、`apps/web/src/entities/polish/**` 中 AIFI-FE-003 contract（契约）、Step11 integration（集成）/ real page QA（真实页面验收）、Step12 release（发布）或 C-049 到 C-054。
