...
用户已确认 `OQ-124` 方案 A：把 facts-only Preview 放到 `docs/governance/` 直下重新验证；Preview 严格全绿后，再执行 facts-only 正式 State Update。
...
| --- | --- | --- |
| docs/governance 直下 Preview | `ok=true,error=0,warning=0` | `ok=true,error=0,warning=0,documents_blocked_count=0,modules_blocked_count=1,subtasks_blocked_count=25` |
