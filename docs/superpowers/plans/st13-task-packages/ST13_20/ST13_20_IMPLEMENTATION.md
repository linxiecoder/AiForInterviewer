...
当前 W13-E8 不允许创建数据库、migration 或 ORM。
...
2. 固定 PostgreSQL schema、实体关系、索引和约束。
3. 制定 migration up/down 和数据回退策略。
4. 实现 persistence 层并补齐测试。
5. 与 `ST13_24` 对齐数据 DoD。
...
`ST13_20_IMPLEMENTATION.md` 的存在不等于 implementation-ready。当前不创建数据库，不创建 migration，不创建 ORM，不生成 implementation packet，不打开 formal window。
