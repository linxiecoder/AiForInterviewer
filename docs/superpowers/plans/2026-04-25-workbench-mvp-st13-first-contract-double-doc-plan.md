...
2. `TASK_INDEX.md`：在 `ST13_21 / ST13_20 / ST13_24 / ST13_25` 下引用双文档路径，状态只能从 `task_packet_draft_created` 过渡到 `double_doc_created` 或 `double_doc_path_planned`，不得写 implementation-ready。
...
6. `DOC_STATE.yaml`：本窗口不修改。后续若要减少 `missing_required_doc_slot` blocker，应另开 State Update 窗口写入 required doc slot，并跑 validate/evaluate。
...
9. 避免误当实现完成：所有父索引都必须写明 `double_doc_path_planned` 或 `double_doc_created` 不等于 implementation-ready。
...
| W13-E10 | 第一批 ST13 readiness 复核窗口；检查 required doc slot、验收标准、测试要求、允许 / 禁止修改范围 | 仍不实现，若需 state 更新另开窗口 |
