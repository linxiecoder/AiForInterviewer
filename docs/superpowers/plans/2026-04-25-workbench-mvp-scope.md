...
- 历史记录能力：面试过程、回答、复盘结果和关键状态需要服务端保存。
...
- 真实 LLM、登录 / 权限、服务端保存、评分系统和复盘记录都进入一期范围，但实现方案尚未完成设计确认。
...
- 原型中的 mock LLM、无登录、会话内临时数据、无数值评分、不导出的边界已被 W13-A 取代。
...
- 工作台一级导航与页面信息架构。
- 核心用户旅程：登录后进入工作台、创建或选择岗位、绑定简历、发起面试、查看历史、生成复盘、导出 Markdown。
- 页面对象关系：工作台、岗位、简历、面试、复盘、评分、导出。
- W10 原型可参考和不可继承的界面清单。

当前 W13-B 已补充：

- 一期工作台 IA：登录 / 权限、工作台首页、岗位、简历、模拟面试、反馈评分、历史复盘、复制 / Markdown 下载、用户 / 权限入口。
- 页面集合：区分一期必须页面、一期可合并页面和后续占位页面。
- 核心用户旅程：覆盖登录、创建岗位、创建 / 上传 / 粘贴简历、发起面试、真实 LLM 出题、回答、评分反馈、历史复盘、Markdown 导出和回到工作台继续训练。
- 页面到对象映射：为 `W13-C` 的对象模型设计提供输入。
- 后续确认卡：账号来源、角色层级、岗位 / 简历列表化、复盘主对象、评分报告页面形态、Markdown 导出入口、工作台统计范围和后续占位区位置。
