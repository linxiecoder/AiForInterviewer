...
PR1.6 caveat：Option C 仍是 `Proposed` 候选长期架构决策。PR2 runtime implementation blocked until PR1.6 blockers are resolved or explicitly accepted by the main Agent. AI/Core directory shape remains open until `AIFI-ARCH-008` or an equivalent authorized task closes.
...
本 ADR 不把 `docs/tmp` 或 `docs/03-delivery/refactor-multiagent-langgraph/` 提升为 canonical active docs。本 ADR 只记录长期架构决策候选；API、数据、Prompt、安全和应用编排事实仍需按 `16_DESIGN_DOCS_REFACTOR_PLAN.md` 回写到 active docs。
...
3. LangGraph concrete import 只能出现在 infrastructure adapter 或明确的 runtime adapter 层。
...
7. PR2 只有在 PR1.6 blockers 关闭或主 Agent 显式接受风险后，才可落 AI Runtime data / trace / checkpoint ref / rollback 基础；PR3 再落 facade / runner port；PR4 再落 concrete LangGraph adapter、fake graph、interrupt/resume 和 timeline；PR5-PR8 逐 graph 迁移。
...
| Option C：LangGraph-first Agentic Workflow Runtime | Proposed | 最能支撑多 graph、runtime timeline、interrupt/resume、raw-off、安全审计和候选确认边界 |
...
- AI/Core directory shape 在 `AIFI-ARCH-008` 或等价任务关闭前仍是 open issue；不得把 `application/ai`、`application/agents`、`application/ai_runtime` 或 `infrastructure/agent_runtime` / `infrastructure/ai_runtime` 的并存状态当作最终目录决策。
- Option B fallback 必须显式登记，不能以临时兼容为由长期保留双入口。

## Boundaries

本 ADR 在 PR1.6 blockers 关闭或主 Agent 显式接受风险后允许：

- 在 PR2 设计和实现 `agent_runs`、`agent_node_runs`、`agent_interrupts`、`llm_calls`、`agent_checkpoint_refs` 等 AI Runtime 基础对象。
...
- 在 `AIFI-ARCH-008` 或等价任务关闭前，将 AI/Core 目录形态、LangGraph import scan 规则或业务 graph 目录视为已冻结。
...
| Active doc | 回写内容 | PR |
|---|---|---|
| `docs/02-design/PERSISTENCE_MODEL.md` | AI Runtime Tables、checkpoint ref、migration / rollback / in-flight task policy | PR2 |
| `docs/02-design/DATA_MODEL.md` | `AgentRun`、`AgentNodeRun`、`AgentInterrupt`、`LlmCall`、`AgentCheckpointRef` 逻辑对象和状态域 | PR2 |
| `docs/02-design/SECURITY_PRIVACY.md` | raw-off payload、checkpoint / timeline / debug 可见性、redaction、retention、audit 边界 | PR2/PR4 |
