...
Polish execution authority backfill：2026-06-19 已将 `refactor-for-arch-01` 中验证通过的 Polish question / feedback execution authority 事实回写到本 ADR 和 active design docs。本回写不把 `docs/goals/` 或 `tmp/` 证据升级为事实源，不改变本 ADR 的 `Proposed` 状态，也不授权完整 PR5+ business graph productization；它只冻结当前 Polish formal write 必须遵守的 execution authority / snapshot / executor 边界。
...
9. Polish question / feedback formal execution 必须遵守 Execution Minimal Model：backend authority 先决定 `execution_target`，allowed decision 再冻结为 immutable `ExecutionSnapshot`，executor handler 只消费 snapshot 并持久化结果。
10. frontend 只提交 intent；LLM 只输出 recommendation / candidate；graph、fallback 和 provider path 只作为 adapter / availability / transport 证据，不能产生 formal business write 授权。
11. `decision_ref` 只用于 authority decision、snapshot、日志、结果关联和调试排查；它不是 durable idempotency、running task lifecycle、resume / cancel / deadline recovery 的长期契约。若后续需要 durable binding，必须另行回写 ADR / active design docs。
12. Progress canonical write 与 projection refresh 必须拆分：canonical write 才能改变进展树或会话进度事实；projection refresh 只是派生读模型 / 展示状态刷新，不得反向授权 question / feedback execution。
...
- Polish 旧 direct path、旧 compat mirror payload 和旧 contract assertion 不再作为长期兼容保护；后续调用方必须依赖 backend authority、snapshot 和 intent-only API。
- 剩余 skeleton / placeholder entrypoint 只能以 temporary exception 存在，必须具备 blocking condition、delete condition、cleanup task、owner check 和 unavailable safe response；不能以 default-off、provider missing 或 fallback wording 暗示产品能力已启用。
...
- 让 frontend request field、LLM output、graph descriptor、fallback branch、provider transport success、legacy mapper 或 checkpoint payload 决定 Polish `execution_target`。
- 将 `decision_ref` 暗示为幂等键、任务恢复键、running task lifecycle key 或跨请求 durable contract。
- 通过 projection refresh、read model rebuild 或 display status 写入 question、answer、feedback 或 progress canonical state。
...
| `docs/02-design/APPLICATION_FLOW_SPEC.md` | Polish Execution Minimal Model、authority / snapshot / executor、Progress canonical write / projection refresh split | Completed 2026-06-19 for `refactor-for-arch-01` active docs backfill |
| `docs/02-design/API_SPEC.md` | Polish intent-only request contract、old compat mirror payload removal、`decision_ref` trace-only response boundary | Completed 2026-06-19 for `refactor-for-arch-01` active docs backfill |
| `docs/02-design/TECH_DESIGN.md` | Polish execution authority architecture、repository dependency boundary、use-case split direction | Completed 2026-06-19 for `refactor-for-arch-01` active docs backfill |
| `docs/02-design/DATA_MODEL.md` / `PERSISTENCE_MODEL.md` | `decision_ref` / `execution_target` trace metadata, not durable idempotency; projection refresh not canonical write | Completed 2026-06-19 for `refactor-for-arch-01` active docs backfill |
| `docs/02-design/SECURITY_PRIVACY.md` / `PROMPT_SPEC.md` | provider / graph / fallback / LLM recommendation cannot authorize formal business writes | Completed 2026-06-19 for `refactor-for-arch-01` active docs backfill |
