...
PR1.6 caveat：Option C 仍是 `Proposed` 候选长期架构决策。AIFI-ARCH-008 已关闭 AI/Core directory shape open issue：最终目录形态为 `application/ai_runtime/**` + `infrastructure/ai_runtime/langgraph/**`。PR2 runtime implementation 仍 blocked until remaining PR1.6 blockers are resolved or explicitly accepted by the main Agent.
