...
| Step 11 | AIFI-QA-005 | 只授权 integration QA（集成质量验证）和 real page QA（真实页面验收）所必需的最小修复；必须复现、修复或分流 Step10 limited smoke residual risk；不授权 release、rollback、migration、config、dependency、新评分/策略逻辑、新题生成或关闭 C-049 到 C-054 |
...
### 5.14 Step 11 / AIFI-QA-005 当前有效文档解锁状态登记

本节登记 `.omo/plans/plan.md` Step 11 的 active-docs unblock（当前有效文档解锁）状态。该状态只说明 Step11 integration（集成）/ real page QA（真实页面验收）可在重新 scope lock 后启动，不改变 PRD 需求事实源，不关闭 C-049 到 C-054，也不授权 Step12 release（发布）、rollback（回滚）、migration（迁移）、config（配置）或 dependency（依赖）改动。

| 项 | 内容 |
|---|---|
| plan.md Step | Step 11：integration（集成）与真实页面 QA |
| BACKLOG 授权入口 | AIFI-QA-005 |
| 当前状态 | READY_TO_START；仍需重新执行 implementation scope lock |
...
| Step10 residual risk | limited smoke 失败于 `failed answer should expose generation_failed payload`，必须复现、修复或分流；不得写成 Step11 PASS |
| 下一步授权 | 只允许重新执行 Step11 implementation scope lock；得到 `execution_mode=AUTHORIZED` 前不得开始 implementation |
