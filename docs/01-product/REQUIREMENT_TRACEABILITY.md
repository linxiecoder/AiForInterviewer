...
| Step 8 backend API schema and response envelope boundary | AIFI-BE-017 | 只允许后端 API response envelope / schema contract 汇总，包括 feedback / progress / follow-up / signed next action payload schema alignment、effective feedback / failure response contract、backwards-compatible API projection 和 backend contract tests；不授权 FE、migration、release、Step9-12 或关闭 C-049 到 C-054；implementation 必须先重新执行 scope lock |
| Step 9 | AIFI-FE-003 | 只允许 `apps/web/src/entities/polish/**` view model、failure folding 和相关 FE tests |
...
### 5.11 Step 8 / AIFI-BE-017 Scope Readiness 状态登记

本节登记 `.omo/plans/plan.md` Step 8 的 active-docs unblock 结果。该状态只说明 Step8 backend API schema and response envelope boundary 已有唯一 BACKLOG 授权入口，不改变 PRD 需求事实源，不关闭 C-049 到 C-054，也不直接授权 implementation。
...
| implementation_allowed | false，直到新的 Step8 implementation scope lock 输出 `execution_mode=AUTHORIZED` |
...
| 下一步授权 | 仅允许重新执行 Step8 implementation scope lock；本节不授权 Step9 implementation |
