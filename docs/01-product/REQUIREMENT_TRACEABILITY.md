...
| C-049 到 C-054 | Deferred / Open Question | `PRD.md` §12、相关 active design docs 的待确认项 | AIFI-TRACE-001、AIFI-ARCH-009、AIFI-QA-003、AIFI-QA-004、AIFI-BE-010 到 AIFI-BE-014、AIFI-FE-003、AIFI-FE-004、AIFI-REL-009 | 仍保持 Deferred；后续只允许按 BACKLOG 对应 AIFI 执行测试护栏、兼容投影、fail-closed、页面恢复和 release gate，不得关闭这些 C-ID |
...
| Lazycodex 输出边界要写硬 | `PRD.md` §12、本文件、`BACKLOG.md` §2、各 active design docs 回写段落；AIFI-QA-004、AIFI-BE-010 到 AIFI-BE-014、AIFI-FE-003、AIFI-FE-004、AIFI-REL-009 | 已登记，`.omo/plans/plan.md` 只作为工程规划来源；可执行范围以 BACKLOG 对应 AIFI 的允许/禁止路径为准 |
...
`BACKLOG.md` 已将 `AIFI-BE-010` 调整为 `READY_TO_START`；进入 Step 2 实现前必须重新执行 scope lock，并确认对应 AIFI 授权入口、允许修改路径和禁止修改路径。`AIFI-BE-009` 仅作为参考上下文，`AIFI-BE-009=NOT_STARTED` 不再硬阻塞 `AIFI-BE-010`。
...
### 5.7 Step 4 / AIFI-BE-012 Readiness Reconciliation 状态登记

本节登记 Step 4 unblock 的 active-doc 修复结果。该状态只说明 `AIFI-BE-012` 可以作为重新执行 Step 4 scope lock 的下一步入口，不直接授权 Step 4 实现落地；实现前仍必须重新执行 scope lock，并以 `BACKLOG.md` 的允许路径、禁止路径和验收边界为准。
...
| unblock evidence | `.omo/evidence/plan/step4-unblock.md` |
| 允许重新执行 Step4 scope lock | YES |
| 当前授权范围 | same-answer stability、reference-answer replay、scoring normalization、related tests/api |
| 可创建测试文件 | `tests/api/test_polish_feedback_stability.py` |
| 明确不授权 | Step5 improvement trend、Step6 progress mastery、Step7 question generation、FE、migration、release |

#### Step 4 覆盖范围

AIFI-BE-012 只覆盖 Step 4 的受控启动入口：
