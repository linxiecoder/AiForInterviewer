...
| Step 9 | AIFI-FE-003 | 当前为 READY_TO_START，只允许重新执行 Step9 implementation scope lock（实现范围锁）；若 scope lock（范围锁）授权，范围仍仅限 `apps/web/src/entities/polish/**` view model（视图模型）、failure folding（失败折叠）、response contract projection（响应契约投影）和相关 FE tests（前端测试），不授权 Step10 / Step11 / Step12 或关闭 C-049 到 C-054 |
...
本节登记 `.omo/plans/plan.md` Step 9 的 active-docs unblock（当前有效文档解锁）状态。该状态只允许重新执行 Step9 implementation scope lock（实现范围锁），不直接授权 implementation（实现），不改变 PRD 需求事实源，不关闭 C-049 到 C-054，也不授权 Step10、Step11、Step12、backend（后端）、migration（迁移）、release（发布）或页面交互实现。
...
| 下一步授权 | 仅允许重新执行 Step9 implementation scope lock（实现范围锁）；scope lock 生成 `execution_mode=AUTHORIZED` 前，不得开始 implementation（实现） |
