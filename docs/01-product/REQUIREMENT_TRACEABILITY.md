...
| Step 6 | AIFI-BE-013 | 只允许 progress mastery、manual completion consistency 和相关 tests/api；依赖 AIFI-BE-015，不得绕过 Step5 |
...
### 5.9 Step 6 / AIFI-BE-013 Canonicalization 状态登记

本节登记 `.omo/plans/plan.md` Step 6 的 canonical BE 入口裁决。该状态只解决 BE mapping、Step5 / Step6 / BACKLOG 漂移和 AIFI-ID 冲突；不进入 Step6 scope lock，不修改 `apps/**`、`tests/**`、FE、migration、配置或依赖。
...
| Step6 authorization_status | READY_TO_START |
| 允许重新执行 Step6 scope lock | YES |
| 当前授权边界 | 只允许重新执行 Step6 scope lock；scope lock 返回 `execution_mode=AUTHORIZED` 前，不授权代码、测试、FE、migration、release 或 Step7 行为 |
...
- AIFI-BE-013：ACCEPTED，作为 Step6 canonical BE，承接 progress mastery、manual completion consistency 和相关 tests/api。
...
#### step6_authorization_status

`READY_TO_START_SCOPE_LOCK_ONLY`

含义：允许重新执行 Step6 scope lock；不表示 Step6 implementation 已授权。
