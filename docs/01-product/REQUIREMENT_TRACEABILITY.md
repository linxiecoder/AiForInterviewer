...
| C-049 到 C-054 | Deferred / Open Question | `PRD.md` §12、相关 active design docs 的待确认项 | AIFI-TRACE-001、AIFI-ARCH-009、AIFI-QA-003、AIFI-QA-004、AIFI-BE-010 到 AIFI-BE-015、AIFI-FE-003、AIFI-FE-004、AIFI-REL-009 | 仍保持 Deferred；后续只允许按 BACKLOG 对应 AIFI 执行测试护栏、兼容投影、fail-closed、Step5 趋势一致性、页面恢复和 release gate，不得关闭这些 C-ID |
...
| C-051 | Deferred / Open Question | §11 Open Questions、§12 Lazycodex Planning Inputs、§12.1 Architecture Inputs | `PRD.md` §12、相关 active design docs 的待确认项 | AIFI-TRACE-001、AIFI-ARCH-009、AIFI-QA-003、AIFI-QA-004、AIFI-FE-003、AIFI-FE-004、AIFI-REL-009 | 仍保持 Deferred；后续实现只允许 view model、失败折叠和页面恢复护栏，不得冻结最终 UI 形态或关闭该 C-ID |
| C-052 | Deferred / Open Question | §11 Open Questions、§12 Lazycodex Planning Inputs、§12.1 Architecture Inputs | `PRD.md` §12、相关 active design docs 的待确认项 | AIFI-TRACE-001、AIFI-ARCH-009、AIFI-QA-003、AIFI-QA-004、AIFI-BE-011、AIFI-FE-003、AIFI-REL-009 | 仍保持 Deferred；后续实现只允许 fail-closed validation / failure folding，不得新增最终错误码或关闭该 C-ID |
| C-053 | Deferred / Open Question | §11 Open Questions、§12 Lazycodex Planning Inputs、§12.1 Architecture Inputs | `PRD.md` §12、相关 active design docs 的待确认项 | AIFI-TRACE-001、AIFI-ARCH-009、AIFI-QA-003、AIFI-QA-004、AIFI-BE-013、AIFI-FE-004、AIFI-REL-009 | 仍保持 Deferred；后续实现只允许 progress/manual completion 一致性与刷新恢复护栏，不得决定最终前端状态机或关闭该 C-ID |
| C-054 | Deferred / Open Question | §11 Open Questions、§12 Lazycodex Planning Inputs、§12.1 Architecture Inputs | `PRD.md` §12、相关 active design docs 的待确认项 | AIFI-TRACE-001、AIFI-ARCH-009、AIFI-QA-003、AIFI-QA-004、AIFI-BE-014、AIFI-REL-009 | 仍保持 Deferred；后续实现只允许基于 BR-024 的产品排序护栏和 QA 断言，不得选择最终下一题算法或关闭该 C-ID |
...
| 兼容性矩阵不足 | `API_SPEC.md`、`DATA_MODEL.md`、`PERSISTENCE_MODEL.md`、`UX_SPEC.md`、`RELEASE_HANDOFF_SPEC.md`；AIFI-BE-009、AIFI-FE-002、AIFI-BE-010、AIFI-BE-011、AIFI-FE-003、AIFI-FE-004 | 已拆分为后端兼容投影、fail-closed validation、前端 view model 和刷新恢复入口；不得新增迁移或关闭 Deferred C-ID |
| Lazycodex 输出边界要写硬 | `PRD.md` §12、本文件、`BACKLOG.md` §2、各 active design docs 回写段落；AIFI-QA-004、AIFI-BE-010 到 AIFI-BE-015、AIFI-FE-003、AIFI-FE-004、AIFI-REL-009 | 已登记，`.omo/plans/plan.md` 只作为工程规划来源；可执行范围以 BACKLOG 对应 AIFI 的允许/禁止路径为准 |
...
| Step 7 | AIFI-BE-014 | 当前只允许重新执行 Step7 implementation authorization scope lock；后续实现若被新 scope lock 授权，也只允许 follow_up_intent_classification、same_node_follow_up_contract、same_node_next_question_candidate_selection、next_question_response_contract、policy_signed_next_action、no_auto_generation_without_policy_signature；不关闭 C-049 / C-054 |
| Step 8、Step 9 | AIFI-FE-003 | 只允许 `apps/web/src/entities/polish/**` view model、failure folding 和相关 FE tests |
...
本节登记 `.omo/plans/plan.md` Step 6 的 canonical BE closeout。该状态只说明 Step6 progress mastery 已完成并关闭，不改变 PRD 需求事实源，不关闭 C-049 到 C-054，也不直接授权 Step7 implementation；Step7 仍必须重新执行 implementation authorization scope lock。
...
| 下一步授权 | 本节只允许 Step7 重新执行 implementation authorization scope lock；不直接授权 Step7 implementation |
...
### 5.10 Step 7 / AIFI-BE-014 Implementation Scope-Lock Readiness 登记

本节登记 `.omo/plans/plan.md` Step 7 的 implementation scope-lock readiness。该状态只允许重新执行 Step7 implementation authorization scope lock；不等于 implementation 已授权，不得复用旧 boundary lock 或旧 BLOCKED lock 作为授权依据。

| 项 | 内容 |
|---|---|
| plan.md Step | Step 7：受策略签名约束的同节点追问 / 同节点下一题候选契约 |
| BACKLOG 授权入口 | AIFI-BE-014 |
| readiness_status | READY_TO_START_SCOPE_LOCK |
...
| prior implementation lock | 当前 `step7_implementation_scope_lock.json` 是 BLOCKED 结果，不得作为 implementation 授权依据 |
| 允许下一步 | 重新执行 Step7 implementation authorization scope lock |
| implementation 进入条件 | 新 scope lock 返回 `execution_mode=AUTHORIZED` 且 `implementation_allowed=true` |

#### Step7 allowed capabilities
...
C-049 到 C-054 仍保持 `Deferred / Open Question`。AIFI-BE-014 只允许在后续授权 implementation 中建立受控 contract / candidate selection / policy-signed next action，不得定义 C-049 相似度阈值，不得选择或关闭 C-054 的最终下一题算法。
