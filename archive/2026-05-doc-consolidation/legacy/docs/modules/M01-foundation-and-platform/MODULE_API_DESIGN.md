...
type: note
permalink: ai-for-interviewer/docs/modules/m01-foundation-and-platform/module-api-design
---
...
| 接口 | 类型 | 消费方 | 作用 | 当前成熟度 |
| --- | --- | --- | --- | --- |
| `GET /api/v1/health` | HTTP API | 本地开发、CI、后续模块联调 | 返回最小存活确认 | 可作为下游设计输入 |
| `GET /api/v1/storage-objects/{object_id}/download` | HTTP API | M03、M05、M06、M08 等业务模块 | 统一实际文件下载入口 | 可作为下游设计输入 |
| `StorageObjectWritePort` | 平台内部接口 | M03 上传 / 导出、M05 归档、M08 导出 | 统一对象写入与元数据回填边界 | 可作为下游设计输入 |
| `getMessages()` + locale seed | 前端文案入口 | Web 页面、共享组件 | 统一读取可见文案，禁止硬编码 | 可作为下游设计输入 |
...
- 历史 ST13_21 最小骨架仅覆盖健康检查与未注册占位；当前 `apps/api` 已存在 interviews、records、review、export 等读写面，当前事实以 `apps/api/**` 和 `docs/development/**` 为准。
- HTTPException 最小错误响应已收敛为 `{"error": {"code": "HTTP_<status>", "message": "<detail>"}}`，不定义业务错误 taxonomy。
- 最小配置边界历史上只读取 `API_TITLE`、`API_VERSION`、`ENVIRONMENT`、`API_PREFIX`、`API_HOST`、`API_PORT`；当前数据库事实为 PostgreSQL runtime + SQLite fallback，Redis、MinIO、LLM、RAG 或对象存储不作为 R0 必需 runtime。
...
### 8.4 request adapter / shared adapter 最小承接边界

- 页面容器持有 canonical `ListQueryState`，并负责触发 request adapter。
- request adapter 只负责输出两类稳定结果：
  - URL query patch
  - 服务请求参数
- 上述两类结果共享同一最小键集：`page / page_size / q / status / sort / order`。
- shared adapter 只负责把服务响应、页面态与 i18n 消费结果投影为 `FilterBar`、`DataTable`、`Pagination` 可消费输入，不向共享原语暴露 router、fetch client 或模块私有 service。
- 不在 M01 冻结：callback 名称、hook 组织方式、resolved copy 承载形态与模块私有扩展 DTO。
...
## 11. L5 / downstream_ready 后仍后置的实现级条件
...
- 将列表查询状态的默认冻结口径继续吸收到下游模块或子任务的实现级 prop / callback contract，并保持共享最小映射只覆盖 `page / page_size / q / status / sort / order`。
- 冻结根目录最小验证命令矩阵与调用入口。
