...
| OQ-016 | 薄弱项聚合 key、消减规则和停练规则如何定稿 | confirmed | M09 / M07 / M08 | 已由 `FC-13` 覆盖：`WeaknessItem` 是可训练、可累计、可消减、可停练的中粒度训练主题，消减建议由用户确认后生效 | 否 | `OPEN_QUESTIONS.md`、对象模型 / RAG / 多轮 / 后端边界输入 |
