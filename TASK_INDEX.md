...
> 当前已进入“最低位压缩轮”，不再按 11 个源任务直接执行；具体窗口编排以 `DOCUMENT_PROGRESS.md` 为准。
...
- `M01`：当前无白名单观察面；本轮只允许围绕 `MQ-001 / MQ-003 / MQ-005` 做最低位压缩，`ST01_*` 仍只作为规划入口，不得在当前阶段直开。
...
- `M03`：当前观察入口以 `MT03_01`、`MT03_02A`、`MT03_02B`、`MT03_03` 等蓝本为准；只允许把 `MT03_01 / MT03_03` 记为条件性白名单观察面，本轮只允许压缩共享最小映射与 `OQ-025` 最小输入表述，上传 / 导出链继续不得视为可直开子任务。
