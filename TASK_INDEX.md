...
> 当前处于“阶段 3 / 总控澄清 + 模块候选白名单准备轮”的收口阶段；“最低位压缩”只是本轮模块窗口采用的方法名，不单独构成阶段切换，全局阶段口径以 `DOCUMENT_PROGRESS.md` 为准。
...
> - `M02`、`M03` 的新微任务蓝本见各自模块目录下的 `MODULE_TASK_INDEX.md`；在 `OQ-024` 完成总控正式映射前，这些蓝本仍不是正式子任务 ID。
...
- `M01`：当前无白名单观察面；本轮只允许围绕 `MQ-001 / MQ-003 / MQ-005` 做保守收口复核，`ST01_*` 仍只作为规划入口，不得在当前阶段直开。
- `M02`：当前观察入口以 `MT02_01 ~ MT02_08` 为蓝本，但只允许把 `MT02_01 / MT02_02` 记为条件性白名单观察面；本轮只允许压缩 `/members` 契约、映射引用与 `MT02_02` 权限边界，`MT02_03 / MT02_07` 仅保留为后续顺位。
- `M03`：当前观察入口以 `MT03_01`、`MT03_02A`、`MT03_02B`、`MT03_03` 等蓝本为准；只允许把 `MT03_01 / MT03_03` 记为条件性白名单观察面，且应理解为“已吸收但未放行”；本轮只允许继续收紧共享最小映射与 `OQ-025` 最小输入表述，上传 / 导出链继续不得视为可直开子任务。
