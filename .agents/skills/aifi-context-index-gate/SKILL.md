...
description: Use Understand-Anything and CodeGraph before large AiForInterviewer refactors to refresh/query indexed context, inspect impact, and reduce blind Read/Grep exploration.
---
...
在大型重构或跨模块分析前，先用 Understand-Anything 与 CodeGraph 获取压缩上下文、调用关系和影响面证据，减少盲目 `Read` / `Grep` / `Glob`。本 Skill 只负责上下文索引、影响面分析和调用链压缩，不负责业务实现、修复、规划或提交。
