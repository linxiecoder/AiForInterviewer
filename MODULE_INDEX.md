...
- 当前正式 requirement 入口固定为 `RQ01`；以下映射只用于首切片关系层，不改变各模块成熟度结论。
...
| `M02 / M05 / M08 / M09 / M10` | 本轮明确排除 | 登录与成员治理、资产 / RAG、复盘、训练中心、管理台 / 运维不进入首切片主链 | 相关开放问题可保留默认口径或待确认，但不转化为本轮实施入口 |
...
>
> 全量待确认模式补充：`FC-01~FC-19` 已完成用户确认；当前只允许进入 W13-F 全局确认回写与实施包评估准备。M01-M10 均不因确认卡完成而自动升级成熟度或放行下游，正式开窗仍需 `TASK_INDEX.md` 写入明确任务 ID。
>
> 当前轮次（W10-C 首切片关系补齐）结论：
> - 当前最低成熟度模块为 `M04-M10`
> - `M03 / M04 / M06 / M07` 已被固定为首切片直接模块，`M01` 只作为条件性支撑
> - `M02 / M05 / M08 / M09 / M10` 当前明确排除在首切片主链外，不是遗漏
> - `MT03_01 / MT03_03` 仍只是观察蓝本，`ST04_01 / ST04_02 / ST06_01 / ST06_02 / ST07_03` 仍只是后续承接对象
> - `OQ-024` 的正式映射已被总控写死，当前正式开窗层仍为空
...
| --- | --- | --- | --- | --- |
| P1 | `M03` | 首切片输入侧直接模块 | 继续以关系层为准，不改模块正文；仅承接 `MT03_01 / MT03_03` 观察蓝本与下游最小输入 | 首切片必须先稳定 JD / 简历输入与最小对齐，不得把上传 / 导出链误写回主链 |
| P1 | `M04` | 首切片问题生成与最小证据模块 | 只补 `ST04_01 / ST04_02` 的后续承接关系，不开正式窗口 | 首轮问题与最小证据需要明确模块承接，但当前不确认完整评分标准 |
| P1 | `M06` | 首切片单轮会话模块 | 只补 `ST06_01 / ST06_02` 的后续承接关系，不扩到 `ST06_03` | 单轮问答记录与最小上下文需要明确模块承接，但不进入多轮 / 导出链 |
| P1 | `M07` | 首切片反馈输出模块 | 只补 `ST07_03` 的后续承接关系，不进入训练中心 / 长期进度 | 简版反馈摘要与最小改进建议需要明确模块承接，但不代表统一评估框架已确认 |
| P2 | `M01` | 条件性支撑 | 只保留为总控后备条件，不进入当前主链 | 仅当 `W10-D` 后续被总控放行时才可能需要最小壳层 / 运行时骨架 |
| P3 | `M02 / M05 / M08 / M09 / M10` | 本轮排除 | 不开新的关系补齐窗 | 不属于首切片主链；相关问题保留在 OQ 层处理 |
...
> - `M03 / M04 / M06 / M07` 只是首切片的直接模块承接，不等于正式子任务设计候选
> - `M03` 仍只有 `MT03_01 / MT03_03` 作为观察蓝本；`M04 / M06 / M07` 当前只登记后续承接对象 `ST04_01 / ST04_02 / ST06_01 / ST06_02 / ST07_03`
> - `M01` 继续只作为条件性支撑；`M02 / M05 / M08 / M09 / M10` 继续排除在首切片主链外
> - 本轮只证明关系层已闭合，未证明任何正式候选条件已闭合

- 暂无

| 候选观察顺序 | Module ID | 当前判断 | 尚缺条件 |
| --- | --- | --- | --- |
| 1 | `M03` | `MT03_01 / MT03_03` 只属于观察蓝本，且应明确为“已吸收但未放行” | 正式开窗层仍为空、当前阶段仍关窗、上传 / 导出链依赖继续未变 |
| 2 | `M04` | `ST04_01 / ST04_02` 仅为后续承接对象，不是本轮正式开窗任务 | 仍缺正式开窗决定，且完整评分标准与训练入口不在本轮确认范围 |
| 3 | `M06` | `ST06_01 / ST06_02` 仅为后续承接对象，不是本轮正式开窗任务 | 仍缺正式开窗决定，且多轮会话、search snapshot、导出链不进入本轮 |
| 4 | `M07` | `ST07_03` 仅为后续承接对象，不是本轮正式开窗任务 | 仍缺正式开窗决定，且训练中心 / 长期进度 / 跨场景评估框架不进入本轮 |
| 5 | `M01` | 当前只作为条件性支撑，不进入首切片主链 | 需等总控判断是否需要最小壳层 / 运行时骨架 |
