...
- 当前一期工作台 MVP 模块设计应回到 `PLAN_LATEST.md` 与 `docs/design/workbench-mvp/` 正式设计事实源。
...
| `M02 / M05 / M08 / M09 / M10` | W10 历史排除 | 登录与成员治理、资产 / RAG、复盘、训练中心、管理台 / 运维不进入 W10 首切片主链 | 不得外推为 W13 一期工作台 MVP 排除项；当前范围以 `docs/design/workbench-mvp/` 为准 |
...
| P1 | `M06 / M08 / M09` | 与多轮、复盘、评分、训练闭环直接相关 | 后续由独立模块窗口按 Workbench MVP 正式设计事实源做模块侧同步，不开子任务窗口 | 这些能力已进入一期设计范围，但模块文档仍停留在 L1 或骨架状态 |
...
| --- | --- | --- | --- |
| 1 | `global` | 先完成 W13 工作台级任务重映射 | `TASK_INDEX.md` 尚未按 `docs/design/workbench-mvp/` 正式设计事实源写入正式任务 ID 与开窗资格 |
...
| M03 | `ST03_01~ST03_03` | historical-reference / superseded | `ST13_03`、`ST13_04`、`ST13_06`、`ST13_19`、`ST13_20`、`ST13_23` | 旧岗位 / 简历 / 导出链路需按 `docs/design/workbench-mvp/` 重切。 |
...
| M08 | `ST08_01~ST08_03` | historical-reference / superseded | `ST13_14~ST13_16`、`ST13_18`、`ST13_19` | 旧复盘骨架只作历史参考，当前以 `docs/design/workbench-mvp/scoring-review-export-dod.md` 为准。 |
...
### 6.8 W13-E10 第一批 readiness 复核模块影响

> W13-E10 只复核四个 `contract_refined` 双文档能否进入 formal window candidate 评估，不改变 M01-M10 成熟度，不解除 M02 模块 blocker，不打开 formal window，不生成 implementation packet，不进入实现。

| ST13 | WT13 alias | 模块映射 | W13-E10 状态建议 | 模块层结论 |
| --- | --- | --- | --- | --- |
| `ST13_21` | `WT13-21` | M01-M10；M02 权限边界仍需后续同步 | `near_ready_for_formal_window_candidate` | API contract 可进入 W13-E11 候选评估，但仍不能创建 `apps/api/**`。 |
| `ST13_20` | `WT13-20` | M01-M10；权限、RAG、资产、导出和复盘均会消费数据 contract | `near_ready_for_formal_window_candidate` | 数据 contract 可进入 W13-E11 候选评估，但仍不能创建数据库、migration、ORM 或 SQL。 |
| `ST13_24` | `WT13-24` | M01、M10、全模块 | `ready_for_formal_window_candidate` | 测试 / DoD contract 可进入 W13-E11 候选评估，但仍不能创建 `tests/**`。 |
| `ST13_25` | `WT13-25` | global、M01、M10 | `ready_for_formal_window_candidate` | 治理收口 contract 可进入 W13-E11 候选评估，但当前不写 Basic Memory，不做 Superpowers 外部写回。 |

W13-E10 不把任一模块升级为实现就绪，也不把 formal window candidate 建议写成状态层 candidate。

### 6.9 W13-E11 第一批 formal window candidate 评估模块影响

> W13-E11 已吸收用户确认 `OQ-114=A`、`OQ-115=B`、`OQ-116=A`、`OQ-117=A`。本节只记录文档层 candidate 评估，不改变 M01-M10 成熟度，不解除 M02 模块 blocker，不打开 formal window，不生成 implementation packet，不进入实现。

| ST13 | WT13 alias | 模块映射 | W13-E11 文档层结论 | 模块层结论 |
| --- | --- | --- | --- | --- |
| `ST13_21` | `WT13-21` | M01-M10；M02 权限边界仍需后续同步 | `near_ready_for_formal_window_candidate_confirmed` | API contract 保持 near-ready；不建议在 M02 blocker 未处理前写成状态层 candidate。 |
| `ST13_20` | `WT13-20` | M01-M10；权限、RAG、资产、导出和复盘均会消费数据 contract | `near_ready_for_formal_window_candidate_confirmed` | 数据 contract 保持 near-ready；不建议在 M02 blocker 和 schema 授权未处理前写成状态层 candidate。 |
| `ST13_24` | `WT13-24` | M01、M10、全模块 | `formal_window_candidate_recommended` | 测试 / DoD contract 可作为后续 State Update 优先候选，但仍不能创建 `tests/**`。 |
| `ST13_25` | `WT13-25` | global、M01、M10 | `formal_window_candidate_recommended` | 治理收口 contract 可作为后续 State Update 优先候选，但当前不写 Basic Memory，不做 Superpowers 外部写回。 |

W13-E11 不把任一模块升级为实现就绪，也不把文档层 recommended 写成 `DOC_STATE.yaml` 已更新。

### 6.10 W13-E12 State Update 准备模块影响

> W13-E12 只准备状态层 candidate / readiness 写入方案和 preview 路径，不改变 M01-M10 成熟度，不解除 M02 blocker，不打开 formal window，不生成 implementation packet，不进入实现。

| ST13 | WT13 alias | 模块映射 | W13-E12 结论 | 模块层结论 |
| --- | --- | --- | --- | --- |
| `ST13_21` | `WT13-21` | M01-M10；M02 权限边界仍需后续同步 | 保持 near-ready，不建议写 `candidate_status=candidate` | API contract 仍是后续 candidate 候选输入；M02 blocker、OpenAPI / `apps/api/**` 授权未闭合前不升级。 |
| `ST13_20` | `WT13-20` | M01-M10；权限、RAG、资产、导出和复盘均会消费数据 contract | 保持 near-ready，不建议写 `candidate_status=candidate` | 数据 contract 仍是后续 candidate 候选输入；M02 blocker、schema / migration / ORM 授权未闭合前不升级。 |
| `ST13_24` | `WT13-24` | M01、M10、全模块 | 可进入 candidate State Update preview | 测试 / DoD contract 可作为 W13-E13 preview 的优先对象，但仍不能创建 `tests/**` 或测试代码。 |
| `ST13_25` | `WT13-25` | global、M01、M10 | 可进入 candidate State Update preview | 治理收口 contract 可作为 W13-E13 preview 的优先对象，但当前不写 Basic Memory，不做 Superpowers 外部写回。 |

W13-E12 不把任一模块升级为实现就绪，也不把 candidate recommended 写成正式 `DOC_STATE.yaml` 已更新。

### 6.11 W13-E13 candidate State Preview 模块影响

> W13-E13 已按用户确认 `OQ-118=B`、`OQ-119=A`、`OQ-120=B` 创建 candidate state preview。该 preview 不改变 M01-M10 成熟度，不解除 M02 blocker，不打开 formal window，不生成 implementation packet，不进入实现。

| ST13 | WT13 alias | 模块映射 | Preview 结果 | 模块层结论 |
| --- | --- | --- | --- | --- |
| `ST13_21` | `WT13-21` | M01-M10；M02 权限边界仍需后续同步 | 保持正式状态原样；near-ready 只保留文档层 | 不写状态层 candidate，不创建 `apps/api/**`。 |
| `ST13_20` | `WT13-20` | M01-M10；权限、RAG、资产、导出和复盘均会消费数据 contract | 保持正式状态原样；near-ready 只保留文档层 | 不写状态层 candidate，不创建 schema、migration、ORM 或数据库代码。 |
| `ST13_24` | `WT13-24` | M01、M10、全模块 | Preview 尝试 candidate 组合但验证失败：`formal_window_open=false` 时禁止 candidate，且 `downstream_ready` 要求 `maturity` 非空 | 不创建 `tests/**`，不升级为正式 candidate。 |
| `ST13_25` | `WT13-25` | global、M01、M10 | Preview 尝试 candidate 组合但验证失败：`formal_window_open=false` 时禁止 candidate，且 `downstream_ready` 要求 `maturity` 非空 | 不写 Basic Memory，不做 Superpowers 外部写回，不升级为正式 candidate。 |

W13-E13 不把任一模块升级为实现就绪，也不把 preview candidate 写成正式 `DOC_STATE.yaml` 已更新。

### 6.12 W13-E13.5 Candidate State 表达策略模块影响

> W13-E13.5 已吸收 `OQ-121=A`，只修正 candidate 状态表达策略，不修改 `DOC_STATE.yaml`，不打开 formal window，不生成 implementation packet，不进入实现。本节只记录模块影响，不改变 M01-M10 成熟度，不解除 M02 blocker。

| ST13 | WT13 alias | 模块映射 | W13-E13.5 策略 | 模块层结论 |
| --- | --- | --- | --- | --- |
| `ST13_21` | `WT13-21` | M01-M10；M02 权限边界仍需后续同步 | 继续保留文档层 `near_ready_for_formal_window_candidate_confirmed`；不写 `candidate_status`，不写 `readiness=downstream_ready` | 不创建 `apps/api/**`，不升级为 formal window candidate。 |
| `ST13_20` | `WT13-20` | M01-M10；权限、RAG、资产、导出和复盘均会消费数据 contract | 继续保留文档层 `near_ready_for_formal_window_candidate_confirmed`；不写 `candidate_status`，不写 `readiness=downstream_ready` | 不创建 schema、migration、ORM、数据库代码或 SQL，不升级为 formal window candidate。 |
| `ST13_24` | `WT13-24` | M01、M10、全模块 | 继续保留文档层 `formal_window_candidate_recommended`；正式状态层暂不写 `candidate_status=candidate`；下一 Preview 优先 facts-only，备选 `candidate_status=observe` | 不创建 `tests/**`，不升级为正式状态层 candidate。 |
| `ST13_25` | `WT13-25` | global、M01、M10 | 继续保留文档层 `formal_window_candidate_recommended`；正式状态层暂不写 `candidate_status=candidate`；下一 Preview 优先 facts-only，备选 `candidate_status=observe` | 不写 Basic Memory，不做 Superpowers 外部写回，不升级为正式状态层 candidate。 |

W13-E13.5 不把任一模块升级为实现就绪，也不把文档层 recommended 写成正式 `DOC_STATE.yaml` 已更新。当前不建议并行窗口；应先串行创建并验证修正后的 Candidate Preview。

### 6.13 W13-E13.6 facts-only Candidate Preview 模块影响

> W13-E13.6 已吸收 `OQ-122=A`、`OQ-123=A`，只创建 facts-only Candidate Preview，不修改 `DOC_STATE.yaml`，不打开 formal window，不生成 implementation packet，不进入实现。本节只记录模块影响，不改变 M01-M10 成熟度，不解除 M02 blocker。

| ST13 | WT13 alias | 模块映射 | W13-E13.6 Preview 结果 | 模块层结论 |
| --- | --- | --- | --- | --- |
| `ST13_21` | `WT13-21` | M01-M10；M02 权限边界仍需后续同步 | 保持正式状态原样；不写 candidate facts | 不创建 `apps/api/**`，不升级为 formal window candidate。 |
| `ST13_20` | `WT13-20` | M01-M10；权限、RAG、资产、导出和复盘均会消费数据 contract | 保持正式状态原样；不写 candidate facts | 不创建 schema、migration、ORM、数据库代码或 SQL，不升级为 formal window candidate。 |
| `ST13_24` | `WT13-24` | M01、M10、全模块 | Preview 仅写入 facts-only `formal_window_candidate_recommended=true`；未写 `candidate_status=candidate` 或 `readiness=downstream_ready` | 不创建 `tests/**`，不升级为正式状态层 candidate。 |
| `ST13_25` | `WT13-25` | global、M01、M10 | Preview 仅写入 facts-only `formal_window_candidate_recommended=true`；未写 `candidate_status=candidate` 或 `readiness=downstream_ready` | 不写 Basic Memory，不做 Superpowers 外部写回，不升级为正式状态层 candidate。 |

Preview facts-only 字段未导致 implementation-ready 或 formal window 误判；但完整 Preview 仍有 `documents_blocked_count=1` 的 path-scan blocker，因此当前不建议并行窗口，也不建议进入 W13-E14。若后续并行，必须由总控先冻结范围，合并窗口负责冲突检查和验证。
...
| --- | --- | --- | --- | --- |
| `ST13_20` | `WT13-20` | M01、M03、M05、M06、M08、M10 | 保持原样；不写 candidate facts，不写 near-ready 状态 | 仍为文档层 near-ready，不创建数据库、migration、ORM 或 SQL。 |
| `ST13_21` | `WT13-21` | M01、M02、M03、M04、M05、M06、M07、M08、M09、M10 | 保持原样；不写 candidate facts，不写 near-ready 状态 | 仍为文档层 near-ready，不创建 `apps/api/**`、OpenAPI 或 schema。 |
| `ST13_24` | `WT13-24` | M01、M10、全模块 | 正式 `facts.formal_window_candidate_recommended=true`，并记录来源、review status、document layer state 和禁止 packet/实现说明 | 只表示状态层记录 candidate 推荐事实；不创建 `tests/**`，不升级为 formal window 或 implementation-ready。 |
| `ST13_25` | `WT13-25` | global、M01、M10 | 正式 `facts.formal_window_candidate_recommended=true`，并记录来源、review status、document layer state 和禁止 packet/实现说明 | 只表示状态层记录 candidate 推荐事实；不写 Basic Memory，不做 Superpowers 外部写回，不升级为 implementation-ready。 |
