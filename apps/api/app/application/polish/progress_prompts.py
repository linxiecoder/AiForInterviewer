    ...
    prompt_asset = {
        ...
        "prompt": "\n".join(
            [
                ...
                "请在保证推理准确的前提下，用最精简的步骤完成思考，避免不必要的分点展开和重复验证。最终输出必须是一个合法、完整的 JSON 对象，不要包含任何额外说明。",
                "目标：优先 6-7 个主训练节点，最多 9 个；resume_deep_dive 优先 4-5 个；jd_gap_learning 优先 2 个。",
                "优先级高于数量，训练路径高于菜单完整度；不为凑数生成低价值节点。",
                "节点必须可直接生成首题并支撑 2-3 轮追问。",
                "planner_summary 控制在 120 字以内。",
                "priority_reason、depth_goal、first_question 均使用短句，不输出长段解释。",
                "follow_up_focus 最多 3 项，每项短句。",
                "low_confidence_flags 使用短 code 风格，不输出长句解释。",
                "deferred_candidates 最多 5 个。",
                "不要输出解释性长段落、分析过程或推理过程。",
                ...
                "如果简历中存在多个有明确贡献项的项目，应尽量让每个强项目进入主训练路径或其子结构。",
                "不要把多个项目全部压缩到一个泛化节点；无法进入主树的项目可写入 deferred_candidates。",
                "resume_deep_dive 顶层节点必须优先代表具体项目或项目内核心方向。",
                "如果 selected_evidence_chunks 中存在 resume_project_contribution，必须生成 children；children 应绑定同项目贡献项。",
                "children 必须是具体核心贡献项、关键技术点或可连续追问的子能力。",
                "至少 1 个 resume_deep_dive 节点应包含 children；如果 evidence 中有多个项目贡献，尽量让多个强项目都有 children。",
                "children 的 evidence_refs 必须引用对应 resume_project_contribution_* 或同项目 resume_project_* ref。",
                "coverage_points/sub_points 只能补充 children，不能替代 children。",
                "禁止把多个项目贡献压缩成一个无 children 的泛化节点。",
                "单个项目包含多个高价值贡献项时，优先用 children 表达项目内技术点，不要把所有技术点平铺成顶层节点。",
                "顶层节点仍需控制数量，项目内部技术点优先进入子结构或兼容字段。",
                "输出标题应体现具体能力方向，避免只写项目名、业务背景或泛化技术栈。",
        ...
        "output_schema": {
            ...
            "optional_leaf_fields": ["children", "coverage_points", "sub_points", "evidence_notes"],
