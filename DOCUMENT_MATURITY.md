...
| --- | --- | --- | --- | --- |
| M01 | L4 | `MODULE_TASK_INDEX.md` / `MODULE_DEPENDENCIES.md` / `MODULE_OPEN_QUESTIONS.md`（高 `L4`） | `OQ-019~OQ-022` 已全部完成 `proposed-default` 回写，模块核心文档维持可评审闭环；但共享下载 / 对象存储口径与页面 / 列表 / i18n 的实现级 adapter 边界仍未形成稳定下游输入，因此暂不登记为 `L5` 候选 | 否 |
| M02 | L4 | `MODULE_API_DESIGN.md`（高 `L4`） | 模块重切已把旧 `ST02_*` 入口回退为历史容器，`requirements/design/schema/logic/task/dependencies` 已接近 `L5` 候选；但 `/members` 读 API 仍受 `OQ-021` 实现级承接、`OQ-022` 页面吸收与旧入口映射策略约束，因此暂不登记为 `L5` 候选 | 否 |
| M03 | L4 | `MODULE_SCHEMA_DESIGN.md`（`L4`） | 模块重切已完成旧入口清理，但 `jobs.requirement_items_json` 的最小 item 结构、空值语义、排序规则与写入责任仍未冻结；当前只有“简历聚合根与当前版本指针”链路局部接近 `L5` 候选，岗位链和上传 / 导出链仍未 ready | 否 |
...
> 本轮（`R-Refactor-01` / 计划重构执行轮）总控判断：
> - 当前最低成熟度模块仍为 `M04-M10`，整体停留在 `L1`，本轮无变化
> - 当前最值得优先推进的仍是 `M01-M03`，但重点已收敛为：`M01` 共享下载 / 对象存储与 shared adapter 收口、`M02` readiness 收紧、`M03` 输入契约上提
> - 当前接近“可进入子任务设计候选”的观察顺序调整为：`M02` > `M03`（仅简历聚合根链局部接近） > `M01`，但本轮仍不开放任何子任务 Codex
...
| 本轮共享契约主模块 | M01 | 当前仍应按共享契约承接中心推进，而不是按普通模块 `L5` 候选推进 | 下一轮优先收口共享下载 / 对象存储与 shared adapter 边界；在这些共享口径未成为稳定上游输入前，不进入子任务候选 | `MODULE_TASK_INDEX.md` / `MODULE_DEPENDENCIES.md` / `MODULE_OPEN_QUESTIONS.md`（高 `L4`） | 否 |
| 接近 `L5` 候选但未 ready | M02 | 模块整体更接近 `L5` 候选，但页面 adapter 与入口映射仍未 ready | 下一轮优先收紧 `MODULE_API_DESIGN.md`、同步 `OQ-024` 映射口径，并继续保持页面类微任务后置 | `MODULE_API_DESIGN.md`（高 `L4`） | 否 |
| 局部接近候选 | M03 | 只有“简历聚合根与当前版本指针”链路局部接近候选，模块整体仍未 ready | 下一轮优先上提 `jobs.requirement_items_json` 输入契约，继续拆细岗位链与编辑器链；上传 / 导出链仍等待 `M01` 共享下载口径 | `MODULE_SCHEMA_DESIGN.md`（`L4`） | 否 |
...
> 说明：
> - `M02` 是当前最接近升入本节的模块；`M03` 只有简历聚合根链局部接近；`M01` 仍先承担共享契约冻结主模块角色。
> - 由于本轮定义为“计划重构执行轮”，总控在 `OQ-020/OQ-021` 收口、`OQ-022` 完成默认口径回写与模块重切完成前，暂不把任何模块文档正式登记为“可作为下游输入”。
...
- 本轮 readiness 判断：
  ...
  - `M02` 是当前最接近整体“可进入子任务设计候选”的模块，但页面 adapter 与旧入口映射仍未 ready。
  - `M03` 只有简历聚合根链局部接近候选；岗位链仍受 `OQ-025` 阻塞，上传 / 导出链仍受 `M01` 共享下载口径约束。
  - `M01` 仍是共享契约主模块，在共享下载 / 对象存储与 shared adapter 边界形成稳定上游输入前不进入候选。
