...
class TaskPatchPreviewTests(ManagedTempArtifactsTestCase):
    managed_temp_dir_label = "task-patch-preview"
    ...
    def setUp(self) -> None:
        ...
        _write_text(
            ...
            (
                ...
                "- 输出 patch preview。\n\n"
                "## 7. 测试与验证\n"
                "- `python -m pytest tests/doc_governor/test_task_patch_preview.py -q`\n"
            ),
        )
        _write_text(
            self.temp_root,
    ...
    def test_generates_requirement_seed_patch_preview(self) -> None:
        preview = build_task_patch_preview(
            state_path=self.state_path,
            evaluate_payload=_evaluate_payload(),
        )
        seed_map = {item["task_id"]: item for item in preview["requirement_seed_patch_preview"]}
        st09 = seed_map["ST09_03"]
        self.assertEqual(st09["suggested_requirement_id"], "RQ01")
        self.assertTrue(st09["needs_manual_confirmation"])
        self.assertEqual(st09["target_path"], "requirements.RQ01.facts.task_ids")
        self.assertEqual(st09["patch_preview"]["op"], "append_if_missing")
        self.assertIn("ST09_03", st09["patch_preview"]["value_preview"])

    def test_generates_task_document_patch_preview(self) -> None:
        preview = build_task_patch_preview(
            state_path=self.state_path,
            evaluate_payload=_evaluate_payload(),
        )
        task_map = {item["task_id"]: item for item in preview["task_document_patch_previews"]}
        st09 = task_map["ST09_03"]
        st01 = task_map["ST01_01"]

        st09_sections = st09["implementation_doc_patch_preview"]["section_patch_preview"]
        headings = [item["proposed_heading"] for item in st09_sections]
        self.assertIn("# 子任务实施文档", headings)
        ...
        self.assertTrue(st01["design_doc_patch_preview"]["enabled"])

    def test_defers_module_inherited_task_instead_of_emitting_task_patch(self) -> None:
        preview = build_task_patch_preview(
            state_path=self.state_path,
            evaluate_payload=_evaluate_payload(),
        )
        task_ids = {item["task_id"] for item in preview["task_document_patch_previews"]}
        self.assertNotIn("ST02_03", task_ids)
        deferred_map = {item["task_id"]: item for item in preview["batch_preview"]["deferred_tasks"]}
