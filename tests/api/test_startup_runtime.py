...
from tools.testing.temp_artifacts import ManagedTempArtifacts  # noqa: E402


@contextmanager
def managed_workspace(label: str) -> Iterator[Path]:
    artifacts = ManagedTempArtifacts(test_id=f"tests.api.test_startup_runtime.{label}")
    try:
...
def test_settings_reads_local_dotenv_without_mutating_environment(
    ...
    with managed_workspace("local-dotenv") as workspace:
...
def test_settings_ignores_dotenv_when_environment_is_test(
    ...
    with managed_workspace("test-env-dotenv") as workspace:
        env_file = workspace / ".env"
        env_file.write_text(
            "DATABASE_URL=postgresql://dotenv_user:dotenv_password@localhost:55432/interviewer",
            encoding="utf-8",
        )
        sqlite_path = workspace / "api.sqlite3"
...
def test_process_sqlite_path_overrides_dotenv_database_url(
    ...
    with managed_workspace("sqlite-overrides-dotenv") as workspace:
        env_file = workspace / ".env"
        env_file.write_text(
            "DATABASE_URL=postgresql://dotenv_user:dotenv_password@localhost:55432/interviewer",
            encoding="utf-8",
        )
        sqlite_path = workspace / "fixture.sqlite3"
        monkeypatch.delenv(DATABASE_URL_ENV, raising=False)
        monkeypatch.delenv(ENVIRONMENT_ENV, raising=False)
        monkeypatch.setenv(API_DATABASE_PATH_ENV, str(sqlite_path))
...
def test_startup_logs_docs_database_and_bootstraps_sqlite(
    ...
    with managed_workspace("startup-bootstrap") as workspace:
        database_path = workspace / "startup.sqlite3"
...
def test_startup_can_disable_schema_bootstrap(
    ...
    with managed_workspace("startup-disabled") as workspace:
        database_path = workspace / "startup-disabled.sqlite3"
        settings = ApiSettings(
            title="startup-test",
            version="0.1.0",
            environment="test",
