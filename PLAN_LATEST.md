...
- 当前状态：已进入阶段 3 的“最低位压缩轮”。
- 当前目标：统一 `OQ-021 / OQ-024 / OQ-025` 的正式映射与状态分层，并把 `M01 / M02 / M03` 的窗口作用域压缩到最低位文档与最小阻塞集合。
...
- 当前轮次优先处理总控状态分层与最低位文档压缩，而不是继续按源任务顺序平推。
- 单次实施单位应收敛到一个子任务。
...
1. `GC-03`：总控统一 `OQ-021 / OQ-024 / OQ-025` 的状态分层与正式映射
2. `MR-08`：`M01` 仅围绕 `MQ-001 / MQ-003 / MQ-005` 做最低位压缩
3. `MR-09`：`M02` 仅围绕 `/members` 契约、映射引用与 `MT02_02` 权限边界做最低位压缩
4. `MR-10`：`M03` 仅围绕共享最小映射、`OQ-025` 最小输入与 route / callback 边界做最低位压缩
5. `RV-04`：正式候选前交叉复核
6. 收包后再决定是否继续模块轮，或进入正式候选复评；本轮仍不开放任何子任务窗口
