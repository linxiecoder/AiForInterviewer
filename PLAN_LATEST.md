...
- 当前真正待定的已收缩为：`W10-C` 能否在不重开正式开窗层的前提下，把 `RQ01 -> module -> subtask/观察蓝本` 关系按冻结口径闭合；`W10-D` 仍保持禁止自动进入。
...
4. `W10-C` 下一步只负责补齐 `RQ01 -> M03/M04/M06/M07 (+ 条件性 M01)` 的关系链与观察蓝本映射，保持正式开窗层为空，不得回改 `W10-A / W10-B` 已冻结口径。
5. `W10-D` 必须等待 `W10-B / W10-C` 完成且 `validate-state` / `evaluate-state` 仍为干净结果后，才允许总控重新判断是否需要最小代码骨架；当前仍不默认触发。
