...
- 当前活动窗口：`W10` 首切片边界与关系层收口；尚未进入业务代码实施。
...
- 当前真正待定已收缩为：下一窗口需先提交 `W10-D-Gate` 的文档写回结果，再复核是否仅放行 `apps/web/**` 最小原型骨架；在该复核通过前仍不创建业务代码目录。
...
5. `W10-D-Gate` 已完成用户确认写回；下一窗口应先提交 `OPEN_QUESTIONS.md`、`DESIGN_DECISIONS.md`、`PLAN_LATEST.md`、`EXECUTION_LOG.md` 与当前执行计划文档的同步结果。
6. 提交后，才允许复核是否进入 `W10-D` 的 `apps/web/**` 最小原型骨架；`apps/api/**`、`infra/**` 继续禁止，且 `W10-D` 不等于完整业务实施。
7. `W10-E / W10-F` 必须跟随 `W10-D` 进入测试验证与收口，不允许直接扩大范围。
