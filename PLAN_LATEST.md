...
- 当前活动窗口：`W13-B` 一期工作台 MVP 信息架构与用户旅程设计；当前暂停代码开发，回到设计文档补齐。
...
### 5.11 W13-B 工作台 IA 与用户旅程设计

- 已新增 `docs/superpowers/plans/2026-04-25-workbench-mvp-ia-user-journey.md`，补齐一期工作台 MVP 信息架构、页面集合、核心用户旅程、页面到对象映射和后续确认卡。
- 一期 IA 明确包含登录 / 权限入口、工作台首页、岗位管理、简历管理、模拟面试会话、反馈与评分、历史记录 / 复盘、复制 / Markdown 下载、用户 / 权限区域。
- 后续能力只作为占位：资产库、RAG / 知识库、管理台、训练中心、多轮高级面试、更完整导出、运维 / 配置不得被写成一期已实现能力。
...
- 当前一期 MVP 已冻结到工作台级范围，且 `W13-B` 已补齐 IA / 用户旅程草案；但对象模型、技术方案、评分 / 复盘 / 导出 DoD 仍未完成，不具备实施开工条件。
...
3. `W13-B` 已补工作台 IA、核心用户旅程、页面对象关系和一期工作台信息架构。
